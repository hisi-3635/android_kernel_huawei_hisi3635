...
extern void callback_demo_main(char *uuid);
...
static void tc_notify_fn(struct work_struct *dummy)
{
    ...
    TC_TIME_DEBUG("step into ipi_secure_notify\n");

    if(!notify_data) {
        TCERR("notify_data is NULL\n");
        return;
    }
    TC_TIME_DEBUG("notify_data timer type is 0x%x, timer ID is 0x%x\n", notify_data->property.type, notify_data->property.timer_id);
    ...
    list_for_each_entry(callback_func_t, &ta_callback_func_list.callback_list, head){
        if(memcmp(callback_func_t->uuid, notify_data->uuid, 16) == 0){
            if (notify_data->property.timer_class == TIMER_RTC) {
                TC_TIME_DEBUG("start to call callback func\n");
                callback_func = (rtc_timer_callback_func)(callback_func_t->callback_func);
                (void)(callback_func)(&(notify_data->property));
                TC_TIME_DEBUG("end to call callback func\n");
            }
            else if (notify_data->property.timer_class == TIMER_GENERIC) {
    ...
    if (notify_data->property.timer_class == TIMER_GENERIC) {
...
find_session:
    ...
    //TCVERBOSE("before send smc call to non_secure in TC_NS_SEND_CMD\n");
    //send smc
	mutex_lock(&session->ta_session_lock);
...
int TC_NS_ClientOpen(TC_NS_DEV_File **dev_file, uint8_t kernel_api)
{
    ...
    if(!notify_data){
       notify_data = (struct tc_notify_data*)kmalloc(
        ...
        if(ret != TEEC_SUCCESS) {
            TCERR("Shared memory registration for secure world notification failed\n");
	    mutex_unlock(&notify_data_lock);
            return ret;
            }
    }
...
int TC_NS_ClientClose(TC_NS_DEV_File *dev){
	...
	if(tc_ns_dev_list.dev_file_cnt != 0){
		//TCVERBOSE("dev list  dev file cnt:%d\n",tc_ns_dev_list.dev_file_cnt);
		tc_ns_dev_list.dev_file_cnt--;
		TCDEBUG("dev list  dev file cnt:%d\n",tc_ns_dev_list.dev_file_cnt);
	}
	else
...
static int tc_client_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	//TCVERBOSE("kernel_addr = %08x, user_addr = %08x\n", addr, (void*)vma->vm_start);
	mutex_lock(&tc_ns_dev_list.dev_lock);
	dev_file->shared_mem_cnt++;
			    ...
			    unsigned long arg)
{
	...
	/* TCDEBUG("cmd = %08x, TC_NS_CLIENT_IOCTL_SES_OPEN_REQ = %08x\n", 
	...
	case TC_NS_CLIENT_IOCTL_REGISTER_AGENT:{
		...
		mutex_unlock(&tc_ns_dev_list.dev_lock);
		ret = TC_NS_register_agent(dev_file, arg, shared_mem);
		break;
	}
			    ...
			    unsigned long arg)
{
	...
	case TC_NS_CLIENT_IOCTL_CANCEL_CMD_REQ:
		...
		ret = TC_NS_Send_CMD(dev_file, argp);
...
#ifdef CONFIG_COMPAT
int tc_compat_client_ioctl(struct file *flie, unsigned int cmd, 
			   unsigned long arg)
{
