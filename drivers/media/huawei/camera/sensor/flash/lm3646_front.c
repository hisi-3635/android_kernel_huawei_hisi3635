...
static int hw_lm3646_front_set_mode(struct hw_flash_ctrl_t *flash_ctrl, void *data)
{
    ...
    unsigned char max_current = 0, led1_torch_current = 0, led1_flash_current = 0;
    u8 level = 0;
    ...
    /* torch mode:level 0 ~ level 7 , flash mode:level 0 ~ level 15 */
    level = (u8)(cdata->data);
    if (FLASH_MODE == cdata->mode) {
        mode = INDUCTOR_CURRENT_LIMMIT|MODE_FLASH;
        led1_flash_current = STROBE_DISABLE;
        max_current = (level > 0xf) ? 0xf : level;
    } else {
        mode = INDUCTOR_CURRENT_LIMMIT|MODE_TORCH;
        led1_torch_current = TORCH_DISABLE;
        max_current = ((level > 0x7) ? 0x7 : level)<<4;
    }
    ...
    i2c_func->i2c_write(i2c_client, REG_MAX_CURRENT, max_current | ENABLE_SHORT_PROTECT);
    /* LED1 disable */
    i2c_func->i2c_write(i2c_client, REG_LED1_FLASH_CURRENT_CONTROL, led1_flash_current);
    i2c_func->i2c_write(i2c_client, REG_LED1_TORCH_CURRENT_CONTROL, led1_torch_current);
    i2c_func->i2c_write(i2c_client, REG_ENABLE, mode);

    cam_info("%s mode=%s level=%d max_current=%02x", __func__,
            (FLASH_MODE == cdata->mode) ? "flash" : "torch",
            level, max_current);
    return 0;
}
