...
/*lint --e{944,506,525,64,119,101,132,537,958,438,830,752,762,713,732,569,830,525,539,650,652,69,}*/
 ...
 */
apPL131_eClockMode g_ulSciClkTimeStpFlag = apPL131_CLOCK_STOP;
...
SCI_CARD_HPD_STATUS g_sci_card_hpd_status = CARD_HPD_STATUS_NORMAL;
...
int pwrctrl_sci_low_power_exit(void)
{
	...
	{
		result = BSP_PWRCTRL_SleepVoteLock(g_stSciHwCfg.sleep_vote);/* [false alarm]:fortify */
        /* [false alarm]:fortify */
        if(result != 0)
	    {
...
s32 sci_detect_cb(u32 channel_id, u32 len, void *context)
{
    ...
    if(g_sci_detect_state == SIM_CARD_LEAVE && pstrState->eCurrentState >= PL131_STATE_WAITATR )
    {
        g_sci_card_hpd_status = CARD_HPD_STATUS_LEAVE;
    ...
    /* send to usim */
    if(NULL != g_omSciFuncPtr && g_stSciHwCfg.slot_switch_flag != SCI_SLOT_SWITCH_CHANGE)
	{
...
void sci_init(void)
{
	...
	ret = bsp_icc_event_register(g_stSciHwCfg.ac_icc_chnl, sci_detect_cb, NULL, NULL, NULL);/*[false alarm]:*/
    
	ret = bsp_icc_event_register(g_stSciHwCfg.m3c_icc_chnl, sci_detect_cb, NULL, NULL, NULL);/*[false alarm]:*/
	if(ret != BSP_OK)
	{
...
s32 sci_buf_add(u8 * pucRecbyte)
{
    ...
    if(g_sci_card_hpd_status == CARD_HPD_STATUS_LEAVE)
    {
        g_sciRxStGblStru.RxDataRecFlag = SCI_RX_DATA_REC_ERROR;       
...
s32 bsp_sci_reset(RESET_MODE rstMode)
{
     ...
     {
         if(g_sci_detect_state == SIM_CARD_LEAVE && (((g_icc_channel >> 16) & 0x0000FFFF) == ICC_CHN_MCORE_CCORE))
         {
             SCI_RECORD_EVENT(SCI_EVENT_DETECT_IND_M3, pSciInOut, __LINE__);
             bsp_icc_send(ICC_CPU_MCU,g_stSciHwCfg.m3c_icc_chnl, (u8*)&pSciInOut, sizeof(u32));
    ...
    for (;;)
    {
        ...
        if (g_SciDeactivateFlag)
        {
            ...
            if(g_sci_card_hpd_status == CARD_HPD_STATUS_LEAVE)
            {
                ...
                sci_print_error("card leave deal\n");
                g_u32ResetSuccess = SCI_DEF_NOT_RESET;
                g_sci_card_hpd_status = CARD_HPD_STATUS_NORMAL;
                if (g_ulSciClkTimeStpFlag != apPL131_CLOCK_START)
                {
            ...
            else if(g_sciRxStGblStru.RxDataRecFlag == SCI_RX_DATA_REC_ERROR) 
            {
    ...
    switch (g_sci_voltage_state.sciClassKind)
    {
	    ...
	    case (PL131_SCI_Class_C):
	    {
	        ...
	        if (PL131_STATE_READY <= pstrState->eCurrentState)
	        {
	            ...
	            lErroRet = OK;/* [false alarm]:fortify */
	        }
	        else
	        {
	    ...
	    case (PL131_SCI_Class_B):
	    {
	        sci_print_info("sciPSClassSwitch & PL131_SCI_Class_B: The current vltg is PL131_SCI_Class_B, not do class switch.\n\n",
	                  0, 0, 0, 0, 0, 0);/* [false alarm]:fortify */
	        lErroRet = BSP_ERR_SCI_VLTG_HIGHEST;/* [false alarm]:fortify */
	        break;
	    }
	    default:
...
s32 sci_set_voltage(SCI_VOLTAGE_SWITCH_DIRECTION direction)
{
       ...
       case SCI_VOLTAGE_SWITCH_CLASS_ZERO2CB:
            {
                 ...
                 if (g_SciExcResetTimes <= 2)/* [false alarm]:fortify */
                 {
                     g_sci_voltage_state.sciClassKind = PL131_SCI_Class_C;
                     /* [false alarm]:fortify */
                     ...
                     if(retVal != 0)
                     {
                        SCI_RECORD_EVENT(SCI_EVENT_REGULATOR_ERR,retVal , __LINE__);/* [false alarm]:fortify */
                     }
                 }
                 else
                 {
    ...
    /* [false alarm]:fortify */
        ...
        {
            SCI_RECORD_EVENT(SCI_EVENT_REGULATOR_ERR,0 , __LINE__);/* [false alarm]:fortify */
...
void sci_set_cfg( SCI_CFG_STRU * p_sci_cfg,SCI_SLOT sci_slot,u32 card_id)
{
    ...
    p_sci_cfg->m3c_icc_chnl     = ((ICC_CHN_MCORE_CCORE << 16) | ((sci_slot == SCI_ID_0)? MCORE_CCORE_FUNC_SIM0 : MCORE_CCORE_FUNC_SIM1));

    p_sci_cfg->ac_icc_chnl     = ((ICC_CHN_IFC<< 16) | ((sci_slot == SCI_ID_0) ? IFC_RECV_FUNC_SIM0 : IFC_RECV_FUNC_SIM1));
...
void sci_uninit(SCI_CFG_STRU * p_sci_cfg)
{
    ...
    bsp_icc_event_unregister(p_sci_cfg->m3c_icc_chnl);
    
    bsp_icc_event_unregister(p_sci_cfg->ac_icc_chnl);
...
s32 bsp_sci_slot_switch(SCI_SLOT sci_slot0,SCI_SLOT sci_slot1)
{
    ...
    if(SCI_DUAL != ((NV_SCI_CFG_UNION)g_SciNVValue).cfg.card_num)/*lint !e69*/
    {
...
s32 bsp_sci_get_slot_state(SCI_SLOT* sci_slot0,SCI_SLOT* sci_slot1)
{
    ...
    SCI_CFG_STRU * p_sci1_cfg = NULL;
     ...
     /*lint -e69*/
    p_sci0_cfg = (SCI_CFG_STRU *)((int)&_sci_cfg_data_start + \
                                (int)(sizeof(SCI_CFG_STRU) * CARD0));/*lint !e69*/
    p_sci1_cfg = (SCI_CFG_STRU *)((int)&_sci_cfg_data_start + \
                                (int)(sizeof(SCI_CFG_STRU) *  CARD1));/*lint !e69*/
    /*lint +e69*/
