...
NV_PA_POWER_UNIT_STRU pa_power_unit;
...
{
	/*清除状态寄存器*/
	return pmu_hi6561_reg_write(HI6561_STATUS1_OFFSET,1<<type,chip_id);	
}
/*lint -save -e801*/
...
static int pmu_hi6561_power_on_local(HI6561_POWER_ID power_id ,HI6561_ID_ENUM chip_id)
{
	...
	if(PMU_HI6561_BUCK_PA == power_id)
	{
	    ret=pmu_hi6561_exc_clear((EXCEPTION_TYPE_E)power_id,chip_id);/*[false alarm]*/
		if(MIPI_OK!=ret){
...
int pmu_hi6561_init_phase1(void)
{
	...
	ret |= (int)bsp_nvm_read((u32)en_NV_Item_PA_POWER_UNIT,(u8 *)&pa_power_unit,(u32)sizeof(NV_PA_POWER_UNIT_STRU));
	if(NV_OK != ret){
...
{
	(void)pmu_hi6561_power_off(PMU_HI6561_LDO1,HI6561_0);/*close LDO1*/
}
...
unsigned int pmu_hi6561_reg_resume(void)
{
	...
	for(i = 0;i < re_num; i++)
	{
	    ret = pmu_hi6561_reg_write(hi6561_save_reg_list[i],hi6561_0_save_reg_data[i],(HI6561_ID_ENUM)0);
		if(ret){
        ...
        if(rf_power_unit.rfpower_m1 == RF_POWER_FROM_HI6561)
		{
		    ret = pmu_hi6561_reg_write(hi6561_save_reg_list[i],hi6561_1_save_reg_data[i],(HI6561_ID_ENUM)1);
			if(ret){
...
int pmu_hi6561_debug(HI6561_ID_ENUM chip_id)
{
	...
	/*lint -save -e18 -e718 -e746 -e628*//*pclint for printf*/
	pastar_print("\n******************pastar[%d] work status show******************\n",chip_id);
	pastar_print("power id:\tLDO1--0\tLDO2--1\tBUCK0--2\tBUCK1--3\tBUCK2--4\n");
	pastar_print("power status:\t0--power off\tnon 0--power on\n\n");
	for(i=0;i<PMU_HI6561_POWER_ID_BUTT;i++)
	{
		...
		pastar_print("voltage:%dmv\n",voltage_mv[i]);
	}

	ret = pmu_hi6561_limit_current_get(&limit_current, chip_id);
	if(MIPI_ERROR == ret){
		pastar_print("Current is limited get failed\n");
		return MIPI_ERROR;
	}
	...
	if(MIPI_ERROR == ret){
		pastar_print("pmu_hi6561_apt_status_get failed\n");
	}
