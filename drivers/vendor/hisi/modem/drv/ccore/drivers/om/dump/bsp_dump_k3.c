...
/*lint --e{124,830,539,752,778,506,729,774,578,830,525,732,737,718,,762,628,959}*/
...
void dump_stack_hook(s32 where)
{
	...
	task->offset += sprintf((char *)(task->dump_stack+task->offset), "[%08x] %pS\n", where, (char *)where);
...
void dump_wait_and_reboot(void)
{
#define DUMP_WAIT_MAX   3000   /* 30S 因需要保存modem镜像和其他镜像时间加长 */
    ...
    {
        /* K3 DUMP复位由AP完成，此函数只能单独复位CP，如果AP不能复位系统 CP单独复位无意义, 不再调用此函数 */
...
void dump_map_init(void)
{
    ...
    strcpy((char*)p_head->name, "CP");          /* [false alarm]:屏蔽Fortify错误 */
    /* coverity[secure_coding] */
    strcpy((char*)p_head->version, "K300V300"); /* [false alarm]:屏蔽Fortify错误 */
...
void dump_base_info_init(void)
{
    ...
    /* coverity[check_return] */
    strncpy((char*)g_dump_base_info->version, (char*)bsp_version_get_firmware(), 30);
    /* coverity[check_return] */
    strncpy((char*)g_dump_base_info->compile_time, (char*)bsp_version_get_build_date_time(), strlen(bsp_version_get_build_date_time()));
}
...
s32 bsp_dump_register_hook(dump_save_modid_t mod_id, dump_save_hook func)
{
    ...
    dump_field_hook_t* pfieldhook = (dump_field_hook_t*)bsp_malloc(sizeof(dump_field_hook_t), 0);/* [false alarm]:fortify */
    if(pfieldhook == NULL)
    {
        return BSP_ERROR;
    }
	   ...
	   case DUMP_SAVE_MOD_LP_MCORE_BUSERROR:
        ...
        case DUMP_SAVE_MOD_TLPS:
             *length = (u32)DUMP_EXT_TLPS_SIZE;
             break;
        case DUMP_SAVE_MOD_GUAS:
             *length = (u32)DUMP_EXT_GUAS_SIZE;
             break;
        default:
...
s32 dump_show_map(void)
{
    ...
    logMsg("DUMP_CP_RESERVE_CPINFO_ADDR: addr=0x%x, offset=0x%x, size=0x%x\n", DUMP_CP_RESERVE_CPINFO_ADDR,  0,                                                 DUMP_CP_RESERVE_CPINFO_SIZE, 0, 0, 0);
    return 0;
}
...
{ 
    ...
    /* 初始化BBE16区域为0xFE */
    memset((void*)DUMP_BBE16_TCM_ADDR, 0xFE, DUMP_BBE16_TCM_SIZE);                                 /* [false alarm]:屏蔽Fortify错误 */
	...
	if((1 == bsp_bbe_is_tcm_accessible()) && (0 == dump_is_deadlock()))
    {
        ...
        memcpy((void*)DUMP_BBE16_TCM_ADDR, (void*)HI_BBE16DMEM_BASE_ADDR, HI_BBE16DMEM_SIZE);        /* [false alarm]:屏蔽Fortify错误 */

        /* 保存ITCM前256K, 并在前4字节置TCM标志 */
        memcpy((void*)DUMP_BBE16_TCM_ADDR+HI_BBE16DMEM_SIZE, (void*)HI_BBE16IMEM_BASE_ADDR,  SZ_256K),/* [false alarm]:屏蔽Fortify错误 */
        memcpy((void*)DUMP_BBE16_TCM_ADDR, "_MCT", 4);                                                /* [false alarm]:屏蔽Fortify错误 */
    }
    else
    {
        /* 低功耗时，保存dsp镜像 */
        ...
        memcpy((void*)dst, (void*)src, LPHY_BBE16_PUB_DTCM_LOAD_SIZE);/* [false alarm]:屏蔽Fortify错误 */
        ...
        memcpy((void*)dst, (void*)src,  LPHY_BBE16_MOD_DTCM_LOAD_SIZE);/* [false alarm]:屏蔽Fortify错误 */
