...
void Mbx_BbeIpcProc(void)
{
    ...
    if(0 == usMode)
    {
        ...
        *(pAddr++) = 0;
        ...
        /* 保存DSP的异常信息 */
        memcpy((u8*)pAddr, (u8*)SRAM_DSP_MNTN_INFO_ADDR, SRAM_DSP_MNTN_SIZE); /* [false alarm]:fortify */
...
void bsp_mailbox_init(void)
{
    ...
    memset(&g_stMbxCtrl, 0, sizeof(MAILBOX_MAINCTRL_STRU));
    ...
    ret = (s32)BSP_IPC_IntConnect(IPC_CCPU_INT_SRC_DSP_RCM_PUB_MBX, (VOIDFUNCPTR)Mbx_ComPsIntProc, 0);
    if(BSP_OK != ret)
    {
        Mbx_Printf("BSP_IPC_IntConnect failed.\n");
        return ;
    ...
    ret = (s32)BSP_IPC_IntEnable(IPC_CCPU_INT_SRC_DSP_RCM_PUB_MBX);
    if(BSP_OK != ret)
    {
        Mbx_Printf("BSP_IPC_IntEnable failed.\n");
        return ;
...
void BSP_MailBox_DspAwakeProc(MAILBOX_CALLBACK_TYPE cbType)
{
    ...
    if(g_stMbxCtrl.pfnRcmNotifyProc)
    {
        ...
        g_stMbxMntn.stSleepCbInfo.astCbInfo[ulPtr].ulSliceStart = BSP_GetSliceValue();
        g_stMbxCtrl.pfnRcmNotifyProc(cbType);
        g_stMbxMntn.stSleepCbInfo.astCbInfo[ulPtr].ulSliceEnd   = BSP_GetSliceValue();;
        g_stMbxMntn.stSleepCbInfo.ulPtr = (g_stMbxMntn.stSleepCbInfo.ulPtr+1)%MAILBOX_MNTN_NUMBER;
    }
    /* 只处理邮箱模块发起的唤醒 */
...
void Mbx_DumpSave(u8* pData, u32 ulSize)
{
    ...
    {
        /* (callback timer) */
        memcpy(&pData[ultmp], (u8*)&(g_stMbxMntn.stCallback.ulSlice[ptr]), sizeof(u32));
        ultmp += sizeof(u32);
        ptr = (ptr+1)%MAILBOX_MNTN_NUMBER;
    }
...
void Mbx_SystemError(u32 errcode, u32 ulPara1, u32 ulPara2, u32 ulPara3)
{
    ...
    pData = malloc(MBX_SYSTEM_ERROR_SIZE);/*lint !e746*/
    if(NULL == pData)
    {
    ...
    memset(pData, 0, MBX_SYSTEM_ERROR_SIZE);
