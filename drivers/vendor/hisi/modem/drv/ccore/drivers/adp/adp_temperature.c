...
* 其它说明  : 无
*****************************************************************************/
BSP_S32 drv_hkadc_get_phy_temp(unsigned short usPhyChannel, unsigned short *pusVolt, short *pstTemp, HKADC_CONV_MODE_E ulMode)
{
    DRV_HKADC_DATA_AREA *p_area = (DRV_HKADC_DATA_AREA *)TEMPERATURE_VIRT_ADDR;
...
void tem_volt_null_test(void)
{
    ...
    if(TEMPERATURE_OK != drv_hkadc_get_temp((HKADC_TEMP_PROTECT_E)0,NULL,&temp, HKADC_CONV_DELAY))
    {
        tem_print_error("volt null is fail.\n");
    }
    if(TEMPERATURE_OK != drv_hkadc_get_temp((HKADC_TEMP_PROTECT_E)0,&volt,NULL, HKADC_CONV_DELAY))
    {
