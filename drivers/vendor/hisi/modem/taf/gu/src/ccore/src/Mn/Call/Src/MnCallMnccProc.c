...
)
{
    ...
    #if 0
    /* 当前信道打开,且信道类型未发生变化,不进行设置 */
    if ( ( VOS_TRUE == f_stCallTchStatus.bChannelEnable )
...
LOCAL VOS_UINT32 MN_CALL_ProcNotifySsCode(
    VOS_UINT8                           ucCallId,
    const MN_CALL_SS_NOTIFYSS_ARG_STRU  *pstNotifySs,
    MN_CALL_SS_NOTIFY_STRU              *pstSsInfo
...
)
{
    ...
    */
    if ((VOS_TRUE == MN_CALL_GetTchStatus()) || (NAS_IE_IS_PRESENT(&pstSetup->stSignal)))
    {
...
{
    if ((NAS_IE_IS_PRESENT(&pstDisc->stCause))
     && (CCA_SS_DIAG_CCBS_POSSIBLE == (pstDisc->stCause.diagnostic[0] & 0x7))
     && (NAS_IE_IS_PRESENT(&pstDisc->stAllowedAct))
...
)
{
    ...
    if (NAS_IE_IS_PRESENT(&stBc1CcEstCnf) && NAS_IE_IS_PRESENT(&stBc2CcEstCnf))
    {
...
)
{
    ...
    if ((MN_CALL_TIMER_STATUS_RUNING == MN_CALL_GetTimerStatus(MN_CALL_TID_WAIT_CALL_REDAIL_INTERVAL))
     && (VOS_TRUE == pstBufferdMsg->bitOpBufferedRejIndMsg)
...
)
{
    ...
    if (VOS_TRUE == pstMsg->unParam.stSyncInd.ulTchAvail)
    {
        ...
        if (VOS_FALSE == MN_CALL_GetTchStatus())
        {
            ...
            MN_CALL_SetTchPara(&(pstMsg->unParam.stSyncInd));
            MN_CALL_GetWaitSendAlertStatus(&bWaitSendAlertStatus, &uccallId) ;
        ...
        {
            bResult = MN_CALL_SetTchPara(&(pstMsg->unParam.stSyncInd));
...
)
{
    ...
    {
        ucCause = pstRel->stCause.Octet4.CauseValue;
        MN_CALL_UpdateCcCause(pstMsg->ucCallId, ucCause);
    }
...
)
{
    ...
    {
        ucCause = pstRelComp->stCause.Octet4.CauseValue;
        MN_CALL_UpdateCcCause(pstMsg->ucCallId, ucCause);
    }
...
)
{
    ...
    if ((VOS_TRUE == pstBufferdMsg->bitOpBufferedSetupMsg)
     && (pstBufferdMsg->stBufferedSetupMsg.ucCallId == pstMsg->ucCallId))
    {
        MN_CALL_ClearBufferedMsg();
        MN_CALL_StopTimer(MN_CALL_TID_WAIT_CALL_REDIAL_PERIOD);
        MN_CALL_StopTimer(MN_CALL_TID_WAIT_CALL_REDAIL_INTERVAL);
...
)
{
    ...
    pstChannelInfoMsg->enMsgName = (VOS_UINT16)enEventType;
    VOS_MemCpy(&pstChannelInfoMsg->stChannelInfo,
               &f_stCallTchStatus,
               sizeof(MN_CALL_CHANNEL_INFO_STRU));
...
)
{
    ...
    */
    MN_INFO_LOG1("MN_CALL_ReportCoeceEvent: event.", (VOS_INT32)enEventType);

    MN_CALL_SendVcMsg(enEventType);

    /* 处理CHANNEL 信息并向AT上报 */

    MN_CALL_ProcChannelInfo(enEventType, &f_stCallTchStatus);
}
...
)
{
    ...
    {
        MN_CALL_SndAtChannelInfoInd(enChannelEvent, pstChannelInfo, VOS_FALSE);
    ...
    if ( MN_CALL_EVT_CHANNEL_PARA_CHANGE == enChannelEvent )
    {
        /* 默认是窄带，SRVCC宽带切窄带时，没有给AT报CHANNEL信息，
           修改为不变化也上报，经huwen确认，如果有重复上报，AP和CODEC都能处理 */
        MN_CALL_SndAtChannelInfoInd(enChannelEvent, pstChannelInfo, VOS_FALSE);
