...
VOS_UINT32 Sta_ProcSetMsClassTypeforDetach(MMC_MMA_DETACH_CNF_STRU *pDetachCnf )
{
    ...
    ClientId =  MMA_CLIENTID_NONE;
    Id       = MMA_OP_ID_INTERNAL;
...
VOS_UINT8 TAF_SDC_IsSpecDomainUsimStatusInvalid(
    MMA_MMC_SRVDOMAIN_ENUM_UINT32       ulCnDomainId
)
{    
...
VOS_UINT8 TAF_MMA_IsDelayRptFeatureActive(VOS_VOID)
{
    ...
    if (VOS_FALSE == TAF_SDC_IsUsimStausValid())
    {
        /*卡无效直接上报，无需启用延时上报功能*/
        STA_TRACE (STA_WARNING_PRINT,
            "TAF_MMA_IsDelayRptFeatureActive():WARNING: The Status of USIM is invaild");
    ...
    if (STA_FSM_ENABLE != g_StatusContext.ulFsmState)
    {
        ...
        STA_TRACE (STA_WARNING_PRINT,
            "TAF_MMA_IsDelayRptFeatureActive():INFOMATION: Service status changed by User operated");
...
VOS_UINT8 TAF_MMA_IsAppServiceStatusChanged(
    TAF_SDC_SERVICE_STATUS_ENUM_UINT8   ulServiceStatus, 
    MMA_MMC_SRVDOMAIN_ENUM_UINT32       ulCnDomainId
)
{
    TAF_SDC_SERVICE_STATUS_ENUM_UINT8   ucOldCsAppServiceStatus;
    TAF_SDC_SERVICE_STATUS_ENUM_UINT8   ucOldPsAppServiceStatus;
    VOS_UINT8                           ucIsAppSrvStatusChg;

    ucOldCsAppServiceStatus = TAF_SDC_GetAppCsServiceStatus();
    ucOldPsAppServiceStatus = TAF_SDC_GetAppPsServiceStatus();
    ucIsAppSrvStatusChg     = VOS_FALSE;


    switch (ulCnDomainId)
    {
        case MMA_MMC_SRVDOMAIN_CS:
            if (ucOldCsAppServiceStatus != ulServiceStatus)
            {
                ucIsAppSrvStatusChg = VOS_TRUE;
            }
            break;
        case MMA_MMC_SRVDOMAIN_PS:
            if (ucOldPsAppServiceStatus != ulServiceStatus)
            {
                ucIsAppSrvStatusChg = VOS_TRUE;
            }
            break;
        ...
        case MMA_MMC_SRVDOMAIN_NOT_REG_PLMN_SEARCHING:
            if ( (ucOldCsAppServiceStatus != ulServiceStatus)
               ||(ucOldPsAppServiceStatus != ulServiceStatus))
            {
                ucIsAppSrvStatusChg = VOS_TRUE;
            }
            break;
...
)
{
    ...
    /*仅单域处于非normal service，需要要直接上报*/
    if (VOS_TRUE == TAF_SDC_IsCurrentNormalServiceStatus())
    {
        return VOS_FALSE;
    }
    if (MMA_MMC_SRVDOMAIN_NOT_REG_PLMN_SEARCHING == enCnDomainId)
    {
        /*搜网出发的服务状态改变，需要延时上报*/
        return VOS_TRUE;
    }
    if ((MMA_MMC_SERVICE_STATUS_LIMITED_SERVICE == enServiceStatus)
     || (MMA_MMC_SERVICE_STATUS_LIMITED_SERVICE_REGION == enServiceStatus))
    {
        ...
        /* 从NO SERVICE 到 limit service, 立即上报*/
        if ((TAF_SDC_SERVICE_STATUS_NO_SERVICE ==TAF_SDC_GetAppCsServiceStatus())
         && ((MMA_MMC_SRVDOMAIN_CS    == enCnDomainId)
          || (MMA_MMC_SRVDOMAIN_CS_PS == enCnDomainId)))
        {
            return VOS_FALSE;
        }
        ...
        if ((TAF_SDC_SERVICE_STATUS_NO_SERVICE ==TAF_SDC_GetAppPsServiceStatus())
         && ((MMA_MMC_SRVDOMAIN_PS    == enCnDomainId)
          || (MMA_MMC_SRVDOMAIN_CS_PS == enCnDomainId)))
        {
            return VOS_FALSE;
        }
...
)
{
    ...
    if (MMA_MMC_SERVICE_STATUS_NORMAL_SERVICE == enServiceStatus)
    {
        return VOS_TRUE;
    }
    ...
    {
        /*延时定时器运行时,APP中还是Normal Service,内部保存的服务状态都是非正常服务，
         若上报回到值上层立刻显示无服务，则不能上报*/
        if ((TAF_SDC_SERVICE_STATUS_NORMAL_SERVICE != enPsSrvSta)
         && (TAF_SDC_SERVICE_STATUS_NORMAL_SERVICE != enCsSrvSta))
        {
            return VOS_FALSE;
        }
    ...
    /*从无服务，到限制服务，应该上报*/
    if (((TAF_SDC_REPORT_SRVSTA_NO_SERVICE      == enAppReportSrvsta)
      || (TAF_SDC_REPORT_SRVSTA_BUTT            == enAppReportSrvsta))
     && ((TAF_SDC_REPORT_SRVSTA_LIMITED_SERVICE         == enReportSrvsta)
      || (TAF_SDC_REPORT_SRVSTA_REGIONAL_LIMITED_SERVICE == enReportSrvsta)))
    {
        return VOS_TRUE;
    }
...
)
{
    ...
    TAF_MMA_TIMER_STATUS_ENUM_UINT8     emDelayTimerStatus;
    

    ucEnableDaleyRptSrvSta = VOS_FALSE;
    emDelayTimerStatus     = TAF_MMA_TIMER_STATUS_BUTT;
    ...
    {
        emDelayTimerStatus = TAF_MMA_GetTimerStatus(TI_TAF_MMA_DELAY_REPORT_SERVICE_STATUS);
    }

    /*与当前APP保存状态不相同则应需要上报*/
    if (VOS_TRUE == TAF_MMA_IsAppServiceStatusChanged((TAF_SDC_SERVICE_STATUS_ENUM_UINT8)enServiceStatus, enCnDomainId))
    {
        /*仅当延时上报功能启用，延时定时器开启更新的服务状态为非normal，且非无卡则无需上报*/
        if ((VOS_FALSE == TAF_SDC_IsSpecDomainUsimStatusInvalid(enCnDomainId))
         && (TAF_MMA_TIMER_STATUS_RUNING == emDelayTimerStatus)
         && (MMA_MMC_SERVICE_STATUS_NORMAL_SERVICE != enServiceStatus))
        {
            /*延时定时器运行期间，确保域注册状态保持一致*/
            if (VOS_FALSE == TAF_MMA_IsNeedReportServiceStatus_DelayRptTimerRun(enCnDomainId, enServiceStatus))
            {
                return VOS_FALSE;
            }
            else
...
VOS_VOID TAF_MMA_ProcMmcServiceStatusReport(
...
)
{
    ...
    VOS_UINT8                           ucNewSrvSta;
    VOS_UINT8                           ucOldSrvSta;
    ...
    ucOldSrvSta            = TAF_SDC_GetServiceStatus();

    ucIsNeedRptSrvSta      = VOS_FALSE;
    ucIsNeedDelayRptSrvSta = VOS_FALSE;

    /* PS服务状态发生变化通知IMSA或SPM模块 */
    TAF_MMA_NotifySrvInfoImsa(pstServiceStatus->enCnDomainId, pstServiceStatus->enServiceStatus);
    ...
    TAF_MMA_ProcBastetServiceStatusInd(ucOldCsSrvSta, ucOldPsSrvSta);
     ...
     /*获取新的服务状态*/
    ucNewSrvSta     = TAF_SDC_GetServiceStatus();
    ...
    if (VOS_TRUE == ucIsNeedRptSrvSta)
    {
        ...
        else
        {
            ...
            TAF_MMA_ProcMmcServiceStatusReport(ucNewSrvSta, TAF_SDC_GetAppServiceStatus());

            if (MMA_MMC_SERVICE_STATUS_NORMAL_SERVICE == pstServiceStatus->enServiceStatus)
            {
    ...
    if (VOS_TRUE == TAF_MMA_IsNeedReportRegStatus_ServiceStatusInd(&stOldAppPlmnInfo))
    {
        /* CREG为2时 */
        if (TAF_SDC_CREG_TYPE_ENTIRE == TAF_SDC_GetCregType())
        {
            TAF_MMA_SndAtRegStatus(MMA_MMC_SRVDOMAIN_CS, TAF_SDC_GetCsRegStatus());
        }
        ...
        if ((TAF_SDC_CGREG_TYPE_ENTIRE == TAF_SDC_GetCgregType())
         ...
         {
            TAF_MMA_SndAtRegStatus(MMA_MMC_SRVDOMAIN_PS, TAF_SDC_GetPsRegStatus());
         }

        /*同步更新驻留信息到APPINFO*/
        TAF_MMA_UpdateAppRegStatusInfo(MMA_MMC_SRVDOMAIN_CS_PS);
    }

    /* 系统模式发生变化时，无条件上报一次信号强度;由其他状态变到normal service时，无条件报一次信号强度 */
    if ((enOldAppSysMode != TAF_SDC_GetAppSysMode())
     || ((TAF_SDC_REPORT_SRVSTA_NORMAL_SERVICE == ucNewSrvSta)
      && (ucOldSrvSta != ucNewSrvSta)))
    {
...
VOS_UINT32 TAF_MMA_IsNeedDelayReportRegStatus(
    MMA_MMC_SRVDOMAIN_ENUM_UINT32       enCnDomainId,
    MMA_MMC_REG_STATE_ENUM_UINT8        enNewRegState
)
{
    ...
    switch(enNewRegState)
    {
        ...
        case NAS_MML_REG_NOT_REGISTERED_SEARCHING:
            if(VOS_FALSE == TAF_SDC_IsCurrentNormalRegStatus())
            {
                /*从oos到搜网不需要延时*/
                ucIsNeedDelayRpt = VOS_FALSE;
            }
        ...
        case NAS_MML_REG_REGISTERED_DENIED:
            ...
            {
                /*仅单域处于未注册状态，需直接上报，不影响界面显示*/
                ucIsNeedDelayRpt = VOS_FALSE;
            }
            else if (VOS_TRUE == TAF_SDC_IsSpecDomainUsimStatusInvalid(enCnDomainId))
            {
                ucIsNeedDelayRpt = VOS_FALSE;
            }
            else 
            {
                /*no service change limit service, need report*/
                if (((TAF_SDC_REG_NOT_REGISTERED_NOT_SEARCH == enAppCsRegStatus)
                  || (TAF_SDC_REG_NOT_REGISTERED_SEARCHING  == enAppCsRegStatus))
                 && (MMA_MMC_SRVDOMAIN_CS                  == enCnDomainId)
                 && (NAS_MML_REG_REGISTERED_DENIED         == enNewRegState))
                {
                    ucIsNeedDelayRpt = VOS_FALSE;
                }
                
                if (((TAF_SDC_REG_NOT_REGISTERED_NOT_SEARCH == enAppPsRegStatus)
                  || (TAF_SDC_REG_NOT_REGISTERED_SEARCHING  == enAppPsRegStatus))
                 && (MMA_MMC_SRVDOMAIN_PS                  == enCnDomainId)
                 && (NAS_MML_REG_REGISTERED_DENIED         == enNewRegState))
                {
                    ucIsNeedDelayRpt = VOS_FALSE;
...
VOS_VOID TAF_MMA_ProcMmcRegStatusInd(
    MMC_MMA_SERVICE_STATUS_IND_STRU    *pstServiceStatus
)
{
    ...
    /*更新内部变量保存的注册状态*/
    if (MMA_MMC_SRVDOMAIN_CS == pstServiceStatus->enCnDomainId)
    {
        TAF_SDC_SetCsRegStatus(pstServiceStatus->enRegState);
    }
    else if (MMA_MMC_SRVDOMAIN_PS == pstServiceStatus->enCnDomainId)
    {
    ...
    {
        STA_TRACE (STA_WARNING_PRINT,
            "TAF_MMA_ProcMmcRegStatusInd():WARNING:enCurRegStatus is TAF_SDC_REG_STATUS_BUTT");
    }

    /*判断是否需要上报*/
    ucIsNeedRptRegStatus = TAF_MMA_IsNeedReportRegStatus_RegStatusInd(pstServiceStatus->enCnDomainId,
                                                                      enOldCsRegState, 
                                                                      enOldPsRegState,
    ...
    {
        /*需要开启延时上报定时器*/
        if (VOS_TRUE == TAF_MMA_IsNeedDelayReportRegStatus(pstServiceStatus->enCnDomainId, pstServiceStatus->enRegState))
        {
            /*开启延时上报定时器*/
            if ( TAF_MMA_TIMER_STATUS_RUNING != TAF_MMA_GetTimerStatus(TI_TAF_MMA_DELAY_REPORT_SERVICE_STATUS))
            {
        ...
        {
            /*上报注册状态*/
            TAF_MMA_SndAtRegStatus(pstServiceStatus->enCnDomainId, pstServiceStatus->enRegState);

            /*停止定时器*/
            if ((NAS_MML_REG_REGISTERED_HOME_NETWORK == pstServiceStatus->enRegState)
             || (NAS_MML_REG_REGISTERED_ROAM         == pstServiceStatus->enRegState))
            {
                TAF_MMA_StopTimer(TI_TAF_MMA_DELAY_REPORT_SERVICE_STATUS);
            }
            
            /*更新APP中保存的注册状态*/
            TAF_MMA_UpdateAppRegStatusInfo(pstServiceStatus->enCnDomainId);
        }
    }
    else
    {
...
)
{
    ...
    {
        TAF_MMA_ProcMmcRegStatusInd(pServiceStatus);
    }
...
VOS_VOID TAF_MMA_UpdateAppNetworkInfo(VOS_VOID)
{
    ...
    PS_MEM_CPY(&(pstAppNwInfo->stServPlmnInfo.stServicePlmnId), &(pstCurNwInfo->stCampPlmnInfo.stPlmnId), sizeof(TAF_SDC_PLMN_ID_STRU));
    pstAppNwInfo->stServPlmnInfo.enServiceSysMode       = pstCurNwInfo->stCampPlmnInfo.enSysMode;
    pstAppNwInfo->stServPlmnInfo.ucServiceUtranMode     = NAS_UTRANCTRL_GetCurrUtranMode();
    pstAppNwInfo->stServPlmnInfo.enServiceSysSubMode    = pstCurNwInfo->stCampPlmnInfo.enSysSubMode;
    pstAppNwInfo->stServPlmnInfo.usServiceLac           = pstCurNwInfo->stCampPlmnInfo.usLac;
    pstAppNwInfo->stServPlmnInfo.ucServiceRac           = pstCurNwInfo->stCampPlmnInfo.ucRac;
    pstAppNwInfo->stServPlmnInfo.ulServiceCellId        = pstCurNwInfo->stCampPlmnInfo.ulCellId;
    pstAppNwInfo->stServPlmnInfo.ucServiceRoamFlg       = pstCurNwInfo->stCampPlmnInfo.ucRoamFlag;
...
)
{
    ...
    else if(MMA_MMC_SRVDOMAIN_CS_PS == enCnDomainId)
    {
        TAF_SDC_SetAppCsRegStatus(pstCurNwInfo->stCsDomainInfo.enCsRegStatus);
        TAF_SDC_SetAppPsRegStatus(pstCurNwInfo->stPsDomainInfo.enPsRegStatus);
    }
...
VOS_VOID TAF_MMA_NotifySrvInfoImsa(
    MMA_MMC_SRVDOMAIN_ENUM_UINT32                enCnDomainId,
    MMA_MMC_SERVICE_STATUS_ENUM_UINT32           enServiceStatus
)
{
    ...
    {
        /* PS服务状态发生变化通知IMSA或SPM模块 */       
        if (VOS_TRUE == TAF_MMA_IsPsSrvStatusChanged(enCnDomainId, enServiceStatus))
        {
            ...
            if ((MMA_MMC_SRVDOMAIN_PS    == enCnDomainId)
             || (MMA_MMC_SRVDOMAIN_CS_PS == enCnDomainId))
            { 
                TAF_MMA_SndImsaSrvInfoNotify(enServiceStatus);             
            }
        }
    }
#endif
...
)
{
    ...
    TAF_SDC_CAMP_PLMN_INFO_STRU        *pstCurCampInfo = VOS_NULL_PTR;

    pstCurCampInfo = TAF_SDC_GetCampPlmnInfo();
    ...
    /* PLMN是否变化 */
    if ( (pstOldPlmnInfo->stServicePlmnId.ulMcc) != (pstCurCampInfo->stPlmnId.ulMcc)
      || (pstOldPlmnInfo->stServicePlmnId.ulMnc) != (pstCurCampInfo->stPlmnId.ulMnc) )
    {
        return VOS_TRUE;
    }

    /* lac是否变化 */
    if ((pstOldPlmnInfo->usServiceLac) != (pstCurCampInfo->usLac))
    {
        return VOS_TRUE;
    }

    /* rac是否变化 */
    if ((pstOldPlmnInfo->ucServiceRac) != (pstCurCampInfo->ucRac))
    {
        return VOS_TRUE;
    }

    /* cellid是否变化 */
    if ((pstOldPlmnInfo->ulServiceCellId) != (pstCurCampInfo->ulCellId))
    {
        return VOS_TRUE;
    }
...
{
    VOS_UINT8                           ucIsSpecDomainUsimStatus;
    ...
    /*获取指定域的卡状态是否有效*/
    ucIsSpecDomainUsimStatus = TAF_SDC_IsSpecDomainUsimStatusInvalid(enCnDomainId);
    ...
    if (VOS_TRUE == TAF_SDC_IsAppSepcDomainRegStaChg((TAF_SDC_SERVICE_DOMAIN_ENUM_UINT8)enCnDomainId,enRegState))
    {
        ...
        /* 仅当卡有效时延时上报定时器运行中 且不是正常注册状态则，继续等待定时器，暂不上报*/
        else if ((VOS_FALSE == TAF_MMA_IsNeedReportRegState_DelayRptTimerRun(enCnDomainId,enRegState))
               &&(VOS_FALSE == ucIsSpecDomainUsimStatus)
               &&(TAF_MMA_TIMER_STATUS_RUNING == enDelayTimerStatus))
        {
            return VOS_FALSE;
        }
        else
