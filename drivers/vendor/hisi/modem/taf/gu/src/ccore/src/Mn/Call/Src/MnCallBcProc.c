...
VOS_UINT32 MN_CALL_CapaProcInit()
{
...
)
{
    ...
    if (f_stMnCallSphVers.ucCnt > 0)
    {
        ...
        for (i=0; i<f_stMnCallSphVers.ucCnt; i++)
        {
            ...
            {
                /* Octet3a的赋值 */
                pstBc->Octet3a.SpeechVer = f_stMnCallSphVers.aucVers[i];
                pstBc->Octet3a.Spare = 0;
                pstBc->Octet3a.Ctm   = 0;
                pstBc->Octet3a.Coding= 0;

                /* 根据有没有后续内容，设定ext的值 */
                ucExtVal = (f_stMnCallSphVers.ucCnt > 1)?MN_CALL_BC_VAL_EXT_EXTEND : MN_CALL_BC_VAL_EXT_NO_EXT;
            ...
            else
            {
                ...
                pstOctet3x->SpeechVer = f_stMnCallSphVers.aucVers[i];
                pstOctet3x->Spare = 0;
                pstOctet3x->Coding= 0;

                /* 根据有没有后续内容，设定ext的值 */
                ucExtVal = (f_stMnCallSphVers.ucCnt == (i + 1))? MN_CALL_BC_VAL_EXT_NO_EXT : MN_CALL_BC_VAL_EXT_EXTEND ;
...
)
{
    ...
    else
    {
        ...
        {
            MN_CALL_GetWaiurFromFunr(pstBc->Octet6d.FNUR);
        }
        else
        {
...
)
{
    ...
    if (f_stMnCallSphVers.ucCnt > 0)
    {
        ...
        for (i=0; i<f_stMnCallSphVers.ucCnt; i++)
        {
            ...
            {
                /* Octet3a的赋值 */
                pstBcOfConf->Octet3a.SpeechVer = f_stMnCallSphVers.aucVers[i];
                pstBcOfConf->Octet3a.Spare = 0;
                pstBcOfConf->Octet3a.Ctm = 0;
                pstBcOfConf->Octet3a.Coding = 0;

                /* 根据有没有后续内容，设定ext的值 */
                ucExtVal = (f_stMnCallSphVers.ucCnt > 1)?
            ...
            else
            {
                ...
                /* 根据有没有后续内容，设定ext的值 */
                ucExtVal = (f_stMnCallSphVers.ucCnt == (i + 1))?
