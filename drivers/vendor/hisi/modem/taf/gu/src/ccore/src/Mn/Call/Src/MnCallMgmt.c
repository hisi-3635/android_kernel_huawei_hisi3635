...
)
{
    ...
    /* 删除对应关注的事件 */
    ucNumOfKeyEvt = f_stCallSupsCmdMgmt.ucNumOfKeyEvt;
...
VOS_UINT32 NAS_CALL_SndOutsideContextData()
{
...
VOS_UINT32 NAS_CALL_RestoreContextData(struct MsgCB * pMsg)
{
    ...
    if (EVT_NAS_CALL_OUTSIDE_RUNNING_CONTEXT_FOR_PC_REPLAY == pRcvMsgCB->MsgHeader.ulMsgName)
    {
        ...
        PS_LOG(WUEPS_PID_TAF, VOS_NULL, PS_PRINT_INFO,"CALL: NAS_CALL_RestoreContextData - data is restored.");
        return VOS_TRUE;
    }
...
)
{
    ...
    ulCallRedialPeriodRemainLen = 0;
    ulCallRedialSupportFlg      = MN_CALL_GetCallRedialSupportFlg();
    ulCallRedialIntervalLen     = MN_CALL_GetTimerLen(MN_CALL_TID_WAIT_CALL_REDAIL_INTERVAL);

    if (MN_CALL_TIMER_STATUS_RUNING == MN_CALL_GetTimerStatus(MN_CALL_TID_WAIT_CALL_REDIAL_PERIOD))
    {
        ulCallRedialPeriodRemainLen = MN_CALL_GetTimerRemainLen(MN_CALL_TID_WAIT_CALL_REDIAL_PERIOD);
    }
     ...
     && (VOS_TRUE == ulCallRedialSupportFlg))
    {
        if ((VOS_TRUE == TAF_CALL_IsCallRedialCause(enCause))
         && (ulCallRedialPeriodRemainLen > ulCallRedialIntervalLen))
        {
            /* 主叫且呼叫处于dialing状态且支持呼叫重发且原因值满足重建条件且剩余时间大于重建间隔时长 */
            return VOS_TRUE;
        }
    }
...
)
{
    ...
    if ( (callId == 0)
      || ( callId > MN_CALL_MAX_NUM ))
    {
        stCallInfo.clientId   = clientId;
        stCallInfo.opId       = opId;
    ...
    else
    {
        ...
        f_astCallMgmtEntities[callId - 1].stCallInfo.opId       = opId;
        f_astCallMgmtEntities[callId - 1].stCallInfo.callId     = callId;
        f_astCallMgmtEntities[callId - 1].stCallInfo.enCause    = enCause;
        ...
        PS_MEM_CPY(&stCallInfo, &f_astCallMgmtEntities[callId - 1].stCallInfo, sizeof(stCallInfo));
...
)
{
    ...
    {
        stCallInfo.ucAtaReportOkAsyncFlag = TAF_CALL_GetAtaReportOkAsyncFlag();
    }
...
)
{
    ...
    for (i = 0; i < ucCallNum; i++)
    {
        ...
        /* 通过ucLocalAlertedFlag与呼叫状态标记判断是否打开声码器:
           active,hold状态:声码器肯定打开

           call delivered状态：根据call info中是否处于本地震铃过程中，如果不是则打开否则未打开 */
        if (MN_CALL_IS_OPEN_CODEC(pstSrvccCallInfo[i].enCallState, pstSrvccCallInfo[i].ucLocalAlertedFlag))
        {
...
)
{
    ...
    if (NAS_CC_CAUSE_CM_SRV_REJ_NETWORK_FAILURE == enCause)
    {
        return VOS_TRUE;
    }
...
)
{
    ...
    switch (enCause)
    {
        case NAS_CC_CAUSE_MM_INTER_ERR_FORB_LA:
