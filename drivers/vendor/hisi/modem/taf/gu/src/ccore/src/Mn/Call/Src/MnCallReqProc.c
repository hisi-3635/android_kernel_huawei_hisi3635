...
)
{
    ...
    if (0 == ulNumOfCalls)
    {
        ...
        if (VOS_TRUE == MN_CALL_GetCallRedialSupportFlg())
        {
     ...
     else
     {
        ...
        /*挂断指定的呼叫*/
        MN_CALL_SendCcDiscReq(ucCallId, MN_CALL_USER_BUSY);
...
)
{
    ...
    MN_CALL_SendCcDiscReq(callId, MN_CALL_USER_BUSY);
...
)
{
    ...
    if (0 < ulNumOfCalls)
    {
        ...
        for (i = 0; i < ulNumOfCalls; i++)
        {
            ...
            MN_CALL_SendCcDiscReq(aCallIds[i], MN_CALL_NORMAL_CALL_CLEARING);
...
)
{
    ...
    {
        MN_CALL_SendCcDiscReq(pstCallSupsParam->callId, MN_CALL_USER_BUSY);

        /* 记录主动挂断的原因值 */
        MN_CALL_UpdateCcCause(pstCallSupsParam->callId, MN_CALL_USER_BUSY);
    ...
    else if ((MN_CALL_TIMER_STATUS_RUNING == MN_CALL_GetTimerStatus(MN_CALL_TID_WAIT_CALL_REDAIL_INTERVAL))
    ...
    {
        /*挂断指定的呼叫*/
        MN_CALL_SendCcDiscReq(pstCallSupsParam->callId, MN_CALL_NORMAL_CALL_CLEARING);
...
LOCAL VOS_VOID MN_CALL_CallSupsCmdRelAllCallReqProc(
    MN_CLIENT_ID_T                      clientId,
    MN_OPERATION_ID_T                   opId,
    const MN_CALL_SUPS_PARAM_STRU       *pstCallSupsParam
)
    ...
    if (0 != ulNumOfCalls)
    {
        ...
        for (i = 0; i < ulNumOfCalls; i++)
        {
            ...
            if ((MN_CALL_S_INCOMING == enCallState)
             ...
             || (MN_CALL_S_CCBS_WAITING_RECALL == enCallState))
            {
                MN_CALL_SendCcDiscReq(aCallIds[i], MN_CALL_USER_BUSY);

                /* 记录主动挂断的原因值 */
                MN_CALL_UpdateCcCause(aCallIds[i], MN_CALL_USER_BUSY);
...
)
{
    ...
    /* 把 MN_CALL_ReportEvent(callId, MN_CALL_EVT_NO_ERROR)
       MN_SendClientResponse(clientId, opId, TAF_CS_CAUSE_SUCCESS)
       这二条上报的消息合成一条，发送给SPM */
...
)
{
    ...
    else
    {
        ...
        VOS_MemCpy(pstCustomEccNum->aucEccNum,
...
)
{
    ...
    {
        VOS_MemSet(&stNvimCustomEccNumList, 0, sizeof(MN_CALL_NVIM_CUSTOM_ECC_NUM_LIST_STRU));
        ...
        for (i = 0; i < pstCustomCallEmerNumCtx->stCustomEccNumList.ucEccNumCount; i++)
        {
            ...
            VOS_MemCpy(stNvimCustomEccNumList.astCustomEccNumList[i].aucEccNum,
...
VOS_VOID MN_CALL_GetEccNumList(MN_CALL_ECC_NUM_INFO_STRU          *pstEccNumInfo)
{
    ...
    if (VOS_FALSE == ulListFulled)
    {
       MN_CALL_AddCustomEccNumsToReportList(pstEccNumInfo);
    }
...
VOS_UINT32 TAF_CALL_CheckDtmfKey(
    VOS_CHAR                           *pckey
)
{
    /* 转换为大写字符 */
...
VOS_VOID  TAF_CALL_RcvStartDtmfReq(struct MsgCB * pstStartDtmf)
{
    ...
    /* DTMF信息合法性判断 */
    if ( (VOS_OK != TAF_CALL_CheckDtmfKey(&stDtmfInfo.cKey))
