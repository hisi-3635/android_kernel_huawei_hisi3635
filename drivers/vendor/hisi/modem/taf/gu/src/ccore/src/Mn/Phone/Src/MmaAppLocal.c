...
VOS_VOID MMA_CPnnReport()
{
    ...
    else
    {
        ...
        ucRoamFlag  = MMA_IsRoam(TAF_SDC_GetRoamFlag());
...
)
{
    ...
    VOS_UINT8                           stOldRac;
    VOS_UINT32                          stOldCellId;
    TAF_SDC_SYS_MODE_ENUM_UINT8         stOldSysMode;
    TAF_SDC_SYS_SUBMODE_ENUM_UINT8      enOldSysSubMode;

    TAF_SDC_PLMN_ID_STRU               *pstAppPlmnId = VOS_NULL_PTR;
    VOS_UINT16                          stAppLac;
    VOS_UINT8                           stAppRac;
    VOS_UINT32                          stAppCellId;
    TAF_SDC_SYS_MODE_ENUM_UINT8         stAppSysMode;
    ...
    stOldRac     = TAF_SDC_GetCurrCampRac();
    stOldCellId  = TAF_SDC_GetCurrCampCellId();
    stOldSysMode = TAF_SDC_GetSysMode();

    enOldSysSubMode = TAF_SDC_GetSysSubMode();

    pstAppPlmnId = &(pstAppServPlmnInfo->stServicePlmnId);
    stAppLac     = pstAppServPlmnInfo->usServiceLac;
    stAppRac     = pstAppServPlmnInfo->ucServiceRac;
    stAppCellId  = pstAppServPlmnInfo->ulServiceCellId;
    stAppSysMode = TAF_SDC_GetAppSysMode();
    ...
    if ((pstOldPlmnId->ulMcc != pstAppPlmnId->ulMcc)
        ...
        || (stOldLac            != stAppLac)
        || (stOldRac            != stAppRac)
        || (stOldCellId         != stAppCellId)
        || (stOldSysMode        != stAppSysMode)
...
VOS_VOID MMA_UpdateRegInfo( TAF_PH_REG_STATE_STRU *pstCurRegInf )
{
    ...
    pstCurRegInf->ucPsRegState = enPsRegStatus;
                                 ...
                                 VOS_UINT8           OpId)
{
    ...
    /* Modified by l00305157 for Service_State_Optimize_PhaseI, 2014-11-29, begin */
    stOperatorNameTmp.RaMode            = TAF_SDC_GetAppSysMode();
    /* Modified by l00305157 for Service_State_Optimize_PhaseI, 2014-11-29, end */
...
VOS_VOID MMA_PhoneGetSrvStatus(VOS_UINT16 ClientId, VOS_UINT8 OpId)
{
    ...
    /*服务状态装入缓冲区*/
    stSrvTmp.CsSrvSta = (TAF_PHONE_SERVICE_STATUS)(TAF_SDC_GetAppCsServiceStatus());
    stSrvTmp.PsSrvSta = (TAF_PHONE_SERVICE_STATUS)(TAF_SDC_GetAppPsServiceStatus());
                           ...
                           VOS_UINT8      OpId)
{
    ...
    switch(TAF_SDC_GetAppSysMode())
    {
        case TAF_SDC_SYS_MODE_GSM:
            ...
            sys_info.ucSysSubMode = TAF_SDC_GetAppSysSubMode();
            break;
    /* Modified by l00305157 for Service_State_Optimize_PhaseI, 2014-11-29, end */
        case TAF_SDC_SYS_MODE_WCDMA:
            if (NAS_UTRANCTRL_UTRAN_MODE_FDD == NAS_UTRANCTRL_GetCurrUtranMode())
...
VOS_UINT32 Taf_PhoneGetOperNameInfo( TAF_PH_OPERATOR_NAME_FORMAT_OP_STRU *pstOperName )
{
    ...
    switch ( pstOperName->ListOp )
    {
        ...
        case TAF_PH_OPER_NAME_PLMN2ALPHANUMERICNAME :
            ...
            if((TAF_PH_NO_ROAM == MMA_IsRoam(TAF_SDC_GetAppRoamFlg()))
...
VOS_VOID MMA_WriteVersion(VOS_VOID)
{
    ...
    ulRslt = (VOS_UINT32)DRV_MEM_VERCTRL((char *)acVersionBuffer,
...
VOS_VOID MMA_UsimGetOPLFileRspForCPNN(PS_USIM_GET_FILE_CNF_STRU *pUsimTafMsg)
{
    ...
    {
        /*重新组合MCC和MNC*/
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[0]  =  (pucContent[0]&0x0F);                  /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[1]  =  ((pucContent[0]&0xF0)>>4);             /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[2]  =  (pucContent[1]&0x0F);                  /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[3]  =  (pucContent[2]&0x0F);                  /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[4]  =  ((pucContent[2]&0xF0)>>4);             /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[5]  =  ((pucContent[1]&0xF0)>>4);             /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PlmnLen  =  6;                                     /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].LACLow   =  (*(pucContent+3)<<8)+(*(pucContent+4)); /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].LACHigh  =  (*(pucContent+5)<<8)+(*(pucContent+6)); /* [false alarm]: 数组已动态分配 */
        gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PNNIndex =  pucContent[7];                         /* [false alarm]: 数组已动态分配 */

        if((0xF >= gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[5])                                /* [false alarm]: 数组已动态分配 */
          &&(0xA <= gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[5]))                              /* [false alarm]: 数组已动态分配 */
        {
            gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PlmnLen = 5;                                   /* [false alarm]: 数组已动态分配 */
            gstMmaValue.stCPNN.stCpnnCnf.pstOplCnf->OPLRecord[i].PLMN[5] = 0xF;                                 /* [false alarm]: 数组已动态分配 */
        }
        pucContent += ucEfLen;
    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            /*重新组合MCC和MNC*/
                            pstOPL->OPLRecord[i].PLMN[0]  =  (pucContent[0]&0x0F);                  /* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].PLMN[1]  =  ((pucContent[0]&0xF0)>>4);             /* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].PLMN[2]  =  (pucContent[1]&0x0F);                  /* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].PLMN[3]  =  (pucContent[2]&0x0F);                  /* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].PLMN[4]  =  ((pucContent[2]&0xF0)>>4);             /* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].PLMN[5]  =  ((pucContent[1]&0xF0)>>4);             /* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].PlmnLen  =  6;                                     /* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].LACLow   =  (*(pucContent+3)<<8)+(*(pucContent+4));/* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].LACHigh  =  (*(pucContent+5)<<8)+(*(pucContent+6));/* [false alarm]: 数组已动态分配 */
                            pstOPL->OPLRecord[i].PNNIndex =  pucContent[7];                         /* [false alarm]: 数组已动态分配 */
                            if((0xF >= pstOPL->OPLRecord[i].PLMN[5])                                /* [false alarm]: 数组已动态分配 */
                              &&(0xA <= pstOPL->OPLRecord[i].PLMN[5]))                              /* [false alarm]: 数组已动态分配 */
                            {
                                pstOPL->OPLRecord[i].PlmnLen = 5;                                   /* [false alarm]: 数组已动态分配 */
                                pstOPL->OPLRecord[i].PLMN[5] = 0xF;                                 /* [false alarm]: 数组已动态分配 */
                            }
...
VOS_VOID MMA_AnQueryProc(VOS_UINT16 ClientId, VOS_UINT8 OpId)
{
    ...
    {
        MMA_WARNINGLOG("MMA_AnQueryProc():RAT IS INVALID");
    }
    return;
}
...
)
{
    ...
    pstCerssiInfoQryCnf->ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstCerssiInfoQryCnf->ulSenderPid       = WUEPS_PID_MMA;
    pstCerssiInfoQryCnf->ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstCerssiInfoQryCnf->ulLength          = sizeof(TAF_MMA_CERSSI_INFO_QUERY_CNF_STRU) - VOS_MSG_HEAD_LENGTH;
    ...
    switch (enRatType)
    {
        ...
        /*还未驻留成功，返回无效值*/
            ...
            MMA_WARNINGLOG1("TAF_MMA_RcvTafMsgCerssiInfoQueryReq():WARNING:TAF_SDC_GetAppSysMode():", enRatType);
         break;
    }
...
MMA_MMC_ALLOWED_REG_DOMAIN_ENUM_UINT8 TAF_MMA_GetRegDomain(VOS_VOID)
{
    ...
    VOS_UINT16                      usSrcLen)
 {
    ...
    if((VOS_UINT32)(2 * usSrcLen)>= ulDestLength)
    {
...
VOS_VOID TAF_MMA_SndBastetServiceStatusInd(VOS_VOID)
{
    ...
    if(MMA_MMC_SERVICE_STATUS_NORMAL_SERVICE == TAF_SDC_GetCsServiceStatus())
    {
    ...
    if(MMA_MMC_SERVICE_STATUS_NORMAL_SERVICE == TAF_SDC_GetPsServiceStatus())
    {
