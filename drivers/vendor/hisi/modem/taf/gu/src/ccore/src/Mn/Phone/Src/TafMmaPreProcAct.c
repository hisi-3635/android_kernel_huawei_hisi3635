...
)
{
    ...
    /* 转换MTC卡状态 */
    MMA_MTC_ConvertCardStatus(pstUsimMsg->enCardStatus, &enMtcCardState);

    /* 发送消息给MTC模块 */
    TAF_MTC_SetModemUsimmState(&stCtrl,enMtcCardState);
#endif
...
)
{
    ...
    /* 将连接状态报到MTC */
    TAF_MTC_SetModemServiceConnState(&stCtrl, &stMmaMtcConnStInfo);
#endif
...
)
{
    ...
    {
        TAF_MMA_ProcMmcServiceStatusReport(enRptSrvSta, TAF_SDC_GetAppServiceStatus());
    }

    /* 注册状态改变,上报注册状态*/
    TAF_MMA_ReportRegStatus(enCsRegStatus, MMA_MMC_SRVDOMAIN_CS);

    TAF_MMA_ReportRegStatus(enPsRegStatus, MMA_MMC_SRVDOMAIN_PS);
...
)
{
    ...
    TAF_MMA_SndMmcAutoReselSetReq(pstRefreshStubReq->ucActiveFlg);
...
VOS_UINT32 TAF_MMA_RcvMmcPsServiceConnStatusInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MMC_MMA_PS_SERVICE_CONN_STATUS_IND_STRU                *pstPsServiceConnStatusInd = VOS_NULL_PTR;
    TAF_MTC_SRV_CONN_STATE_INFO_STRU                        stMmaMtcConnStInfo;
    TAF_CTRL_STRU                                           stCtrl;
    TAF_SDC_SYS_MODE_ENUM_UINT8                             enSysMode;

    pstPsServiceConnStatusInd = (MMC_MMA_PS_SERVICE_CONN_STATUS_IND_STRU *)pstMsg;
    ...
    switch(enSysMode)
    {
        ...
        default:
            ...
            MMA_WARNINGLOG("TAF_MMA_RcvMmcPsServiceConnStatusInd_PreProc():WORNING:SysMode is invalid.");
            break;
    }
...
VOS_UINT32 TAF_MMA_RcvMmcLmmCellSignReportInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    TAF_SDC_CTX_STRU                           *pstSdcCtx     = VOS_NULL_PTR;
    MMC_MMA_LMM_CELL_SIGN_INFO_REPORT_IND_STRU *pstRcvMsg          = VOS_NULL_PTR;

    pstRcvMsg        = (MMC_MMA_LMM_CELL_SIGN_INFO_REPORT_IND_STRU*)pstMsg;
    ...
    if (MMA_MMC_SCELL_MEAS_TYPE_RXLEV == (pstRcvMsg->unMeasReportType.ucMeasReportType & MMA_MMC_SCELL_MEAS_TYPE_RXLEV))
    {
...
#endif
VOS_UINT32 TAF_MMA_RcvMmcRssiInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 TAF_MMA_RcvMmcSrchedPlmnInfoInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MMC_MMA_SRCHED_PLMN_INFO_IND_STRU  *pstSrchedPlmnInfoInd = VOS_NULL_PTR;
    TAF_SDC_CURC_RPT_CTRL_STRU         *pstCurcRptCtrl       = VOS_NULL_PTR;

    pstSrchedPlmnInfoInd = (MMC_MMA_SRCHED_PLMN_INFO_IND_STRU *) pstMsg;
...
/* Added by c00318887 for file refresh需要触发背景搜, 2015-3-9, begin */
...
)
{
    ...
    {
        ulTotalLen = sizeof(PS_USIM_REFRESH_IND_STRU) * (pstRefreshStubReq->ucTotalNum - 1);
    }
    
    pstUsimRefreshInd = (PS_USIM_REFRESH_IND_STRU*)PS_ALLOC_MSG(MAPS_PIH_PID, ((sizeof(PS_USIM_REFRESH_IND_STRU)-VOS_MSG_HEAD_LENGTH) + ulTotalLen));
    ...
    PS_MEM_SET((VOS_INT8*)pstUsimRefreshInd + VOS_MSG_HEAD_LENGTH, 0, ((sizeof(PS_USIM_REFRESH_IND_STRU) - VOS_MSG_HEAD_LENGTH) + ulTotalLen));

    pstUsimRefreshInd->ulMsgName         = PS_USIM_REFRESH_IND;
    pstUsimRefreshInd->ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstUsimRefreshInd->ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstUsimRefreshInd->ulLength          = (sizeof(PS_USIM_REFRESH_IND_STRU) - VOS_MSG_HEAD_LENGTH);
    pstUsimRefreshInd->ulReceiverPid     = pstRefreshStubReq->ulReceivePid;
    pstUsimRefreshInd->ulSenderPid       = MAPS_PIH_PID;
    ...
    for (i = 0; i < pstRefreshStubReq->ucTotalNum; i++)
    {
        ...
        pstUsimRefreshInd->astEfId[i].enAppType = USIMM_UMTS_APP;                   /* [false alarm]:通过动态申请内存的结果  */

    }

    (VOS_VOID)PS_SEND_MSG(MAPS_PIH_PID, pstUsimRefreshInd);

    return VOS_TRUE;
}


/* Added by c00318887 for file refresh需要触发背景搜, 2015-3-9, end */
