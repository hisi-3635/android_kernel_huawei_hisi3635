...
VOS_VOID TAF_MMA_SndMmcAutoReselSetReq(
    VOS_UINT8                           ucActiveFlg
)
{
    VOS_UINT32                          ulRet;
    MMA_MMC_AUTO_RESEL_SET_REQ_STRU    *pstMsg = VOS_NULL_PTR;

    /* 申请内存  */
    pstMsg = (MMA_MMC_AUTO_RESEL_SET_REQ_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMA,
                           sizeof(MMA_MMC_AUTO_RESEL_SET_REQ_STRU) - VOS_MSG_HEAD_LENGTH);

    if ( VOS_NULL_PTR == pstMsg )
    {
        MN_ERR_LOG("TAF_MMA_SndMmcAutoReselSetReq:ERROR: Memory Alloc Error for pMsg");
    ...
    pstMsg->stMsgHeader.ulSenderCpuId    = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid      = WUEPS_PID_MMA;
    pstMsg->stMsgHeader.ulLength         = sizeof(MMA_MMC_AUTO_RESEL_SET_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName        = ID_MMA_MMC_AUTO_RESEL_SET_REQ;
    ...
    if ( VOS_OK != ulRet )
    {
        MN_ERR_LOG("TAF_MMA_SndMmcAutoReselSetReq:ERROR:PS_SEND_MSG FAILURE");
    }
