...
/*lint --e{830,628,639,539,525,774,740,958}*/
...
VOS_UINT32 l4a_PktActivateCnf2Aps(L4A_PS_MSG_STRU* pData,VOS_UINT32 ulAtCnfMsgId,VOS_UINT16 usClientId)
{
    ...
    VOS_MemCpy(&(stCnf.stIpAddrInfo), &(pstCnf->stPDNAddrInfo), sizeof(APS_L4A_PDP_ADDR_STRU));
    VOS_MemCpy(&(stCnf.stEpsQosInfo), &(pstCnf->stSdfQosInfo), sizeof(APS_L4A_EPS_QOS_STRU));

    if((1 == pstCnf->bitOpDnsPrim) && (APP_ESM_PDN_TYPE_IPV4 == pstCnf->stDnsPrimAddrInfo.ucIpType))
    {
        stCnf.stDnsAddrInfo.bitOpPrimDnsAddr= 1;
        VOS_MemCpy(stCnf.stDnsAddrInfo.aucPrimDnsAddr, pstCnf->stDnsPrimAddrInfo.aucIpV4Addr, APP_MAX_IPV4_ADDR_LEN);
    }
    ...
    if((1 == pstCnf->bitOpDnsPrim) && (APP_ESM_PDN_TYPE_IPV6 == pstCnf->stDnsPrimAddrInfo.ucIpType))
    {
        ...
        VOS_MemCpy(stCnf.stIpv6DnsAddrInfo.aucPrimDnsAddr, pstCnf->stDnsPrimAddrInfo.aucIpV6Addr, APP_MAX_IPV6_ADDR_LEN);
    }

    if((1 == pstCnf->bitOpDnsSec) && (APP_ESM_PDN_TYPE_IPV4 == pstCnf->stDnsSecAddrInfo.ucIpType))
    {
        stCnf.stDnsAddrInfo.bitOpSecDnsAddr = 1;
        VOS_MemCpy(stCnf.stDnsAddrInfo.aucSecDnsAddr, pstCnf->stDnsSecAddrInfo.aucIpV4Addr, APP_MAX_IPV4_ADDR_LEN);
    }

    if((1 == pstCnf->bitOpDnsSec) && (APP_ESM_PDN_TYPE_IPV6== pstCnf->stDnsSecAddrInfo.ucIpType))
    {
        stCnf.stIpv6DnsAddrInfo.bitOpSecDnsAddr = 1;
        VOS_MemCpy(stCnf.stIpv6DnsAddrInfo.aucSecDnsAddr, pstCnf->stDnsSecAddrInfo.aucIpV6Addr, APP_MAX_IPV6_ADDR_LEN);
    }

    /*stPdpActCnf.tNbnsAddrInfo暂时可以不提供*/

    stCnf.stGateWayAddrInfo.bitOpGateWayAddr= pstCnf->bitOpGateWayAddrInfo;
    VOS_MemCpy(stCnf.stGateWayAddrInfo.aucGateWayAddr, pstCnf->stGateWayAddrInfo.aucIpV4Addr, 4*sizeof(VOS_UINT8));
    ...
    VOS_MemCpy(stCnf.stPcscfAddrInfo.aucPrimPcscfAddr, pstCnf->stPCscfPrimAddrInfo.aucIpV4Addr, 4*sizeof(VOS_UINT8));
    VOS_MemCpy(stCnf.stPcscfAddrInfo.aucSecPcscfAddr, pstCnf->stPCscfSecAddrInfo.aucIpV4Addr, 4*sizeof(VOS_UINT8));
...
VOS_UINT32 l4a_PktDeactivateCnf2Aps(L4A_PS_MSG_STRU* pData, VOS_UINT32 ulAtCnfMsgId,VOS_UINT16 usClientId)
{
    ...
    VOS_MemCpy(stCnf.stIpAddrInfo.aucIpv4Addr, pstCnf->stPDNAddrInfo.aucIpV4Addr, APP_MAX_IPV4_ADDR_LEN*sizeof(VOS_UINT8));
    VOS_MemCpy(stCnf.stIpAddrInfo.aucIpv6Addr, pstCnf->stPDNAddrInfo.aucIpV6Addr, APP_MAX_IPV6_ADDR_LEN*sizeof(VOS_UINT8));
...
VOS_UINT32 l4a_FunPsLwclashReadCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    VOS_MemCpy(&stCnf.stLwclashInfo, &pstCnf->stLWClashPara, sizeof(RRC_APP_LWCLASH_PARA_STRU));
...
VOS_UINT32 l4a_FunPsLcacellReadCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    stCnf.ulErrorCode = ERR_MSP_SUCCESS;
    VOS_MemCpy(stCnf.stLcacellInfo, pstCnf->astCellStatus, sizeof(L4A_CACELL_INFO_STRU)*CA_MAX_CELL_NUM);
...
VOS_UINT32 l4a_FunLwclashInd(L4A_PS_MSG_STRU* pData)
{
    ...
    if(L4A_GetRptCmdStatus(L4A_RPT_CMD_LWURC))
    {
        ...
        VOS_MemCpy(&stLwclash.stLwclashInfo, &pstLwclashInd->stLWClashPara, sizeof(RRC_APP_LWCLASH_PARA_STRU));
...
VOS_UINT32 l4a_FunLcacellInd(L4A_PS_MSG_STRU* pData)
{
    ...
    if(L4A_GetRptCmdStatus(L4A_RPT_CMD_LCACELLURC))
    {
        ...
        VOS_MemCpy(stLcacell.stLcacellInfo, pstLcacellInd->astCellStatus, sizeof(L4A_CACELL_INFO_STRU)*CA_MAX_CELL_NUM);
...
VOS_UINT32 l4a_FunLteWifiReq(L4A_DATA_MSG_STRU* pData,VOS_UINT32 ulPsReqMsgId,VOS_UINT32 ulPsReqPid)
{
    ...
    if(ulRet != BSP_OK)
    {
        ...
        VOS_MemFree(MSP_L4_L4A_PID, pstReqToDsp);
...
VOS_UINT32 l4a_FunPsCnmrReadCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    VOS_MemCpy(&(stCnf.stLteCelInfo),&(pstCnf->stLteCelInfo),sizeof(RRC_APP_LTE_CELL_INFO_STRU));
...
VOS_UINT32 l4a_FunPsCellInfoReadCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    stCnf.ulErrorCode = pstCnf->enResult;
    VOS_MemCpy(&(stCnf.stNcellListInfo), &pstCnf->stNcellListInfo, sizeof(LRRC_APP_NCELL_LIST_INFO_STRU));
...
VOS_UINT32 l4a_FunPsNdisconnSetReq(L4A_DATA_MSG_STRU* pData,VOS_UINT32 ulPsReqMsgId,VOS_UINT32 ulPsReqPid)
{
    ...
    g_ucL4aPdpType              = pstReq->stDialPara.enPdpType;
    ...
    VOS_MemCpy(stReq.stApnInfo.aucApnName, pstReq->stDialPara.aucAPN, APP_MAX_APN_LEN);

    if(0 != pstReq->stDialPara.usAuthType)
    {
       /* USER NAME*/
       stReq.stGwAuthInfo.ucUserNameLen = pstReq->stDialPara.ucUserNameLen;
       VOS_MemCpy(stReq.stGwAuthInfo.aucUserName,pstReq->stDialPara.aucUserName,APP_ESM_MAX_USER_NAME_LEN); /* [false alarm]: 屏蔽Fortify错误 */

       /* PASSWORD*/
       stReq.stGwAuthInfo.ucPwdLen = pstReq->stDialPara.ucPwdLen;
       VOS_MemCpy(stReq.stGwAuthInfo.aucPwd,pstReq->stDialPara.aucPassword,APP_ESM_MAX_PASSWORD_LEN); /* [false alarm]: 屏蔽Fortify错误 */
...
VOS_UINT32 l4a_FunPsNdisconnSetCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    VOS_MemCpy(&stEsmCnf.stPDNAddrInfo        ,&pstCnf->stPDNAddrInfo,      sizeof(APP_ESM_IP_ADDR_STRU));
    VOS_MemCpy(&stEsmCnf.stSubnetMask         ,&pstCnf->stSubnetMask,       sizeof(APP_ESM_IP_ADDR_STRU));       
    VOS_MemCpy(&stEsmCnf.stGateWayAddrInfo    ,&pstCnf->stGateWayAddrInfo,  sizeof(APP_ESM_IP_ADDR_STRU));  
    VOS_MemCpy(&stEsmCnf.stDnsPrimAddrInfo    ,&pstCnf->stDnsPrimAddrInfo,  sizeof(APP_ESM_IP_ADDR_STRU));  
    VOS_MemCpy(&stEsmCnf.stDnsSecAddrInfo     ,&pstCnf->stDnsSecAddrInfo,   sizeof(APP_ESM_IP_ADDR_STRU));   
    VOS_MemCpy(&stEsmCnf.stPCscfPrimAddrInfo  ,&pstCnf->stPCscfPrimAddrInfo,sizeof(APP_ESM_IP_ADDR_STRU));
    VOS_MemCpy(&stEsmCnf.stPCscfSecAddrInfo   ,&pstCnf->stPCscfSecAddrInfo, sizeof(APP_ESM_IP_ADDR_STRU));

    VOS_MemCpy(&stEsmCnf.stApnInfo            ,&pstCnf->stApnInfo,          sizeof(APP_ESM_APN_INFO_STRU));
    VOS_MemCpy(&stEsmCnf.stSdfQosInfo         ,&pstCnf->stSdfQosInfo,       sizeof(APP_ESM_EPS_QOS_INFO_STRU));

    /* 当结果值不为SUCC且bitOpRabId不为TRUE时，发送ACTIVATE_REJ，其他场景发送ACTIVATE_CNF */
    if((ERR_MSP_SUCCESS != pstCnf->ulRslt) && (0 == pstCnf->bitOpRabId))
    {
        return l4a_PktActivateRej2Aps((L4A_PS_MSG_STRU*)&stEsmCnf, ID_L4A_APS_PDP_ACTIVATE_REJ,usClientId);/*激活失败，上报REJ*/
    }
    else
    {
        return l4a_PktActivateCnf2Aps((L4A_PS_MSG_STRU*)&stEsmCnf, ID_L4A_APS_PDP_ACTIVATE_CNF,usClientId);/*激活成功，上报CNF*/
    }
}
...
VOS_UINT32 l4a_FunPsPPPDialSetReq(L4A_DATA_MSG_STRU* pData,VOS_UINT32 ulPsReqMsgId,VOS_UINT32 ulPsReqPid)
{
    ...
    VOS_MemCpy(&(stReq.stPppInfo), &(pstReq->stPppInfo), sizeof(APP_ESM_PPP_INFO_STRU));
...
VOS_UINT32 l4a_FunPsPPPDialSetCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    if((ERR_MSP_SUCCESS != pstCnf->ulRslt) && (0 == pstCnf->bitOpRabId))
    {
        return l4a_PktActivateRej2Aps(pData, ID_L4A_APS_PDP_ACTIVATE_REJ,usClientId);/*激活失败，上报REJ*/
    }
    else
    {
        return l4a_PktActivateCnf2Aps(pData, ID_L4A_APS_PDP_ACTIVATE_CNF,usClientId);/*激活成功，上报CNF*/
    }
}
...
VOS_UINT32 l4a_FunPsAuthDataSetReq(L4A_DATA_MSG_STRU* pData,VOS_UINT32 ulPsReqMsgId,VOS_UINT32 ulPsReqPid)
{
    ...
    if(0 != pstReq->stAuthData.usAuthType)
    {
        ...
        VOS_MemCpy(stReq.stGwAuthInfo.aucUserName,pstReq->stAuthData.aucUserName,APP_ESM_MAX_USER_NAME_LEN); /* [false alarm]: 屏蔽Fortify错误 */

        /* PASSWORD*/
        stReq.stGwAuthInfo.ucPwdLen = pstReq->stAuthData.ucPwdLen;
        VOS_MemCpy(stReq.stGwAuthInfo.aucPwd,pstReq->stAuthData.aucPassword,APP_ESM_MAX_PASSWORD_LEN); /* [false alarm]: 屏蔽Fortify错误 */
...
VOS_UINT32 l4a_FunPsCgdcontSetReq(L4A_DATA_MSG_STRU* pData,VOS_UINT32 ulPsReqMsgId,VOS_UINT32 ulPsReqPid)
{
    ...
    else
    {
        ...
        VOS_MemCpy((VOS_VOID*)(stReq.stApnInfo.aucApnName),
    ...
    else
    {
        ...
        if(1 == pstCgtftReq->stTftInfo.bitOpSrcIp)
        {
            if(APP_ESM_PDN_TYPE_IPV4 == pstCgtftReq->stTftInfo.stSourceIpaddr.enPdpType)
            {
                ...
                VOS_MemCpy((VOS_VOID*)(pstPsCgtftReq->stAppTftInfo.aucRmtIpv4Address),
                    (VOS_VOID*)(pstCgtftReq->stTftInfo.stSourceIpaddr.aucIpv4Addr), APP_MAX_IPV4_ADDR_LEN);

                VOS_MemCpy((VOS_VOID*)(pstPsCgtftReq->stAppTftInfo.aucRmtIpv4Mask),
            ...
            if(APP_ESM_PDN_TYPE_IPV6 == pstCgtftReq->stTftInfo.stSourceIpaddr.enPdpType)
            {
                ...
                VOS_MemCpy((VOS_VOID*)(pstPsCgtftReq->stAppTftInfo.aucRmtIpv6Address),
                    (VOS_VOID*)(pstCgtftReq->stTftInfo.stSourceIpaddr.aucIpv6Addr), APP_MAX_IPV6_ADDR_LEN);

                VOS_MemCpy((VOS_VOID*)(pstPsCgtftReq->stAppTftInfo.aucRmtIpv6Mask),
...
VOS_UINT32 l4a_FunPsCgactSetReq(L4A_DATA_MSG_STRU* pData,VOS_UINT32 ulPsReqMsgId,VOS_UINT32 ulPsReqPid)
{
    ...
    if(pstReq->enState == 1)
    {
        ...
        {
            l4a_AddCmdToListEx(pData, ID_APS_L4A_SET_CGACT_REQ, ID_APP_ESM_PDP_SETUP_CNF);
            return ERR_MSP_SUCCESS;
        }
    }
    else
    {
        ...
        if(ERR_MSP_WAIT_ASYNC == ret)
        {
            l4a_AddCmdToListEx(pData, ID_APS_L4A_SET_CGACT_REQ, ID_APP_ESM_PDP_RELEASE_CNF);
            return ERR_MSP_SUCCESS;
        }
    }
...
#endif
    VOS_MemSet((VOS_VOID*)(pstPrim->aucApn), 0, APS_L4A_MAX_APN_LEN + 1);

    VOS_MemCpy((VOS_VOID*)(pstPrim->aucApn),
        (VOS_VOID*)(pstPsPdpContCnf->stApnInfo.aucApnName), pstPsPdpContCnf->stApnInfo.ucApnLen);

    VOS_MemCpy((VOS_VOID*)&(pstPrim->stPdpAddr),
        (VOS_VOID*)&(pstPsPdpContCnf->stPDNAddrInfo), sizeof(APP_ESM_IP_ADDR_STRU));

    VOS_MemCpy((VOS_VOID*)&(pstPrim->stSubnetMask),
        (VOS_VOID*)&(pstPsPdpContCnf->stSubnetMask), sizeof(APP_ESM_IP_ADDR_STRU));

    VOS_MemCpy((VOS_VOID*)&(pstPrim->stGWAddr),
        (VOS_VOID*)&(pstPsPdpContCnf->stGateWayAddrInfo), sizeof(APP_ESM_IP_ADDR_STRU));

    VOS_MemCpy((VOS_VOID*)&(pstPrim->stDNSPrimAddr),
        (VOS_VOID*)&(pstPsPdpContCnf->stDnsPrimAddrInfo), sizeof(APP_ESM_IP_ADDR_STRU));

    VOS_MemCpy((VOS_VOID*)&(pstPrim->stDNSSecAddr),
        (VOS_VOID*)&(pstPsPdpContCnf->stDnsSecAddrInfo), sizeof(APP_ESM_IP_ADDR_STRU));

    VOS_MemCpy((VOS_VOID*)&(pstPrim->stPCSCFPrimAddr),
        (VOS_VOID*)&(pstPsPdpContCnf->stPCscfPrimAddrInfo), sizeof(APP_ESM_IP_ADDR_STRU));

    VOS_MemCpy((VOS_VOID*)&(pstPrim->stPCSCFSecAddr),
...
VOS_UINT32 l4a_FunPsCgcontrdpSetCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    if(0 == pNode->ulCidNum)
    {
        ...
        VOS_MemSet((VOS_VOID*)pstCnf, 0, ulsize);
    ...
    if(NULL == pData)
    {
        pstCnf->ulErrorCode = ERR_MSP_TIMEOUT;
    }
    else
    {
    ...
    for(i = 0, j = 0; i < pNode->ulCidNum; i++)
    {
        ...
        {
            VOS_MemCpy(&pstCnf->astPrimParaInfo[j++], &pstCnf->astPrimParaInfo[i], sizeof(APS_L4A_PRIM_PDP_DYNAMIC_EXT_STRU));
        }
    }
    ...
    VOS_MemFree(MSP_L4_L4A_PID, pstCnf);
...
VOS_UINT32 l4a_FunPsCgscontrdpSetCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    for(i = 0, j = 0; i < pNode->ulCidNum; i++)
    {
        ...
        {
            VOS_MemCpy(&pstCnf->astSecParaInfo[j++], &pstCnf->astSecParaInfo[i], sizeof(APS_L4A_SEC_PDP_DYNAMIC_EXT_STRU));
        }
    }
...
VOS_VOID l4a_FunCgtftrdpPkj(APP_ESM_INQ_DYNAMIC_TFT_CNF_STRU* pstPsInqTftCnf, APS_L4A_SDF_PF_TFT_STRU *psttft)
{
    ...
    for(j = 0; j < pstPsInqTftCnf->ulPfNum; j++)
    {
        ...
        if(pstPsInqTftCnf->astPfInfo[j].bitOpRmtIpv4AddrAndMask)
        {
            ...
            VOS_MemCpy((VOS_VOID*)(psttft->astTftInfo[j].stSourceIpaddr.aucIpv4Addr),
            (VOS_VOID*)(pstPsInqTftCnf->astPfInfo[j].aucRmtIpv4Address), APP_MAX_IPV4_ADDR_LEN);

            VOS_MemCpy((VOS_VOID*)(psttft->astTftInfo[j].stSourceIpMask.aucIpv4Addr),
        ...
        if(pstPsInqTftCnf->astPfInfo[j].bitOpRmtIpv6AddrAndMask)
        {
            ...
            VOS_MemCpy((VOS_VOID*)(psttft->astTftInfo[j].stSourceIpaddr.aucIpv6Addr),
            (VOS_VOID*)(pstPsInqTftCnf->astPfInfo[j].aucRmtIpv6Address), APP_MAX_IPV6_ADDR_LEN);

            VOS_MemCpy((VOS_VOID*)(psttft->astTftInfo[j].stSourceIpMask.aucIpv6Addr),
...
VOS_UINT32 l4a_FunPsCgtftrdpSetCnf(L4A_PS_MSG_STRU* pData,L4A_FW_MSG_FUN_TABLE_STRU* pstTable,VOS_UINT16 usClientId)
{
    ...
    if(NULL == pData)
    {
        pstCnf->ulErrorCode = ERR_MSP_TIMEOUT;  /* TODO: error code */
    }
    else
    {
...
VOS_UINT32 l4a_PktPdpModifyCnf2Aps(L4A_PS_MSG_STRU* pData, VOS_UINT32 ulAtCnfMsgId,VOS_UINT16 usClientId)
{
    ...
    VOS_MemCpy(&(stCnf.stEpsQosInfo), &(pstCnf->stSdfQosInfo), sizeof(APP_ESM_EPS_QOS_INFO_STRU));
...
VOS_UINT32 l4a_FunPsIpv6SetReq(L4A_DATA_MSG_STRU* pData,VOS_UINT32 ulPsReqMsgId,VOS_UINT32 ulPsReqPid)
{
    ...
    VOS_MemCpy(stReq.astIpv6PrefixArray, pstReq->astIpv6PrefixArray, \
...
VOS_UINT32 l4a_PktPdpModifyInd2Aps(L4A_PS_MSG_STRU* pData)
{
    ...
    VOS_MemCpy(&(stInd.stEpsQosInfo), &(pstInd->stSdfQosInfo), sizeof(APP_ESM_EPS_QOS_INFO_STRU));
...
VOS_UINT32 L4aPktSdfInfo(APS_L4A_SDF_PARA_STRU* pstSdfParaInfo,APP_ESM_SDF_PARA_STRU* pstSdfPara)
{
    ...
    VOS_MemCpy(&(pstSdfPara->stSdfQosInfo), &(pstSdfParaInfo->stSdfQosInfo), sizeof(APP_ESM_EPS_QOS_INFO_STRU));
    pstSdfPara->stApnInfo.ucApnLen       = pstSdfParaInfo->stApnInfo.ucApnLen;
    VOS_MemCpy(pstSdfPara->stApnInfo.aucApnName, pstSdfParaInfo->stApnInfo.aucApnName, APP_MAX_APN_LEN);

    /*VOS_MemCpy(&(pstSdfPara->stGwAuthInfo), &(pstSdfParaInfo->stGwAuthInfo), sizeof(APP_ESM_GW_AUTH_INFO_STRU));注意，这里两个USERNAME长度不同，有待确认    */
    ...
    VOS_MemCpy(pstSdfPara->stGwAuthInfo.auAccessNum, pstSdfParaInfo->stGwAuthInfo.auAccessNum, APP_ESM_MAX_ACCESS_NUM_LEN);
    VOS_MemCpy(pstSdfPara->stGwAuthInfo.aucUserName, pstSdfParaInfo->stGwAuthInfo.aucUserName, APP_ESM_MAX_USER_NAME_LEN);
    VOS_MemCpy(pstSdfPara->stGwAuthInfo.aucPwd, pstSdfParaInfo->stGwAuthInfo.aucPwd, APP_ESM_MAX_PASSWORD_LEN);

    for(i = 0; i < pstSdfParaInfo->ulPfNum; i++)
    {
        VOS_MemCpy(pstSdfPara->astCntxtTftInfo + i, pstSdfParaInfo->astCntxtTftInfo + i, sizeof(APP_ESM_TFT_INFO_STRU));
/*
...
VOS_UINT32 APP_GetSdfPara(VOS_UINT32 *pulSdfNum,APP_ESM_SDF_PARA_STRU *pstSdfPara)
{
    ...
    VOS_MemSet((VOS_VOID*)pstSdfParaInfo, 0, sizeof(APS_L4A_SDF_PARA_STRU));
    ...
    {
        L4aPktSdfInfo(pstSdfParaInfo,pstSdfPara);

        *pulSdfNum = 1;
        ret        = APP_SUCCESS;
    }

    VOS_MemFree(MSP_L4_L4A_PID, pstSdfParaInfo);
...
VOS_UINT32 APP_GetPdpManageInfo(APP_ESM_PDP_MANAGE_INFO_STRU *pstPdpManageInfo)
{
    ...
    if(VOS_ERR == ret)
    {
        VOS_MemFree(MSP_L4_L4A_PID, pstApsPdpMngInfo);
        return APP_FAILURE;
    }
