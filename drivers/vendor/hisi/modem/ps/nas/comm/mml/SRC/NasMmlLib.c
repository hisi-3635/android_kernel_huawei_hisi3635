...
)
{
    ...
    for (ulIndex = 0; ulIndex < pstPrioRatList->ucRatNum; ulIndex++)
    {
        ...
        if (VOS_FALSE == NAS_MML_IsPlmnIdWithRatInDestDisabledPlmnWithValidPeriodList(&stPlmnWithRat,
...
)
{
    ...
    {
        if (ulIdx == (VOS_UINT32)(pstForbPlmnInfo->ucForbPlmnNum -1))
        {
...
)
{
    ...
    {
        if (ulIdx == (VOS_UINT32)(pstForbPlmnInfo->ucForbGprsPlmnNum -1))
        {
...
)
{
    ...
    /* 复位时保存接入层的TCB信息 */
    DRV_SYSTEM_ERROR( NAS_REBOOT_MOD_ID_MML | NAS_MML_SAVE_OTHER_FID_TCB_FLAG | (VOS_INT)ucRebootScean
...
)
{
    ...
    /* 从第五个字节开始存放压缩后的数据 */
    ulRslt  = (VOS_UINT32)_uncompress(pucDest, (VOS_ULONG *)pulDestLen,
                                      pucSrc, *pulSrcLen);
    if (VOS_OK != ulRslt)
    {
...
)
{
    ...
    ulRslt         = (VOS_UINT32)compress(pucDest + sizeof(VOS_UINT32), (VOS_ULONG *)pulDestLen,
                                   pucSrc, ulSrcLen);
...
VOS_UINT32 NAS_MML_IsNeedAddPlmnInDisabledPlmnWithValidPeriodList(
...
)
{
    ...
    pstDisabledPlmnIdWithRatList = NAS_MML_GetDisabledRatPlmnCfg();


    ulIndex = pstDisabledPlmnIdWithRatList->ulDisabledRatPlmnNum;
    
    /* 如果当前PLMN已经在惩罚列表里面，则更新当前PLMN的惩罚属性信息，如DISABLE的原因，惩罚时间 */
    for ( i = 0 ; i < pstDisabledPlmnIdWithRatList->ulDisabledRatPlmnNum; i++ )
    {
        if ((VOS_TRUE               == NAS_MML_CompareBcchPlmnwithSimPlmn(&(pstPlmnWithRat->stPlmnId), &(pstDisabledPlmnIdWithRatList->astDisabledRatPlmnId[i].stPlmnWithRat.stPlmnId)))
         && (pstPlmnWithRat->enRat  == pstDisabledPlmnIdWithRatList->astDisabledRatPlmnId[i].stPlmnWithRat.enRat))
        {
            if ( enDisableLteReason == pstDisabledPlmnIdWithRatList->astDisabledRatPlmnId[i].enDisableLteReason )
            {
                return VOS_FALSE;
            }
...
VOS_UINT32 NAS_MML_GetDisabledPlmnWithValidPeriodTimeSlice(
    NAS_MML_PLMN_WITH_RAT_STRU         *pstPlmnWithRat
)
{
    ...
    if ((VOS_TRUE == NAS_MML_GetAddDamPlmnInDisablePlmnWithRatListFlag())
     && (VOS_TRUE == ulIsPlmnSupportDam))
    {
        ulExpiredTimeSlice = NAS_MML_ONE_SECOND_TIME_SLICES * (NAS_MML_GetT3402Len()/1000);
    }
...
VOS_VOID NAS_MML_UpdateDisabledPlmnWithValidPeriodList(
    NAS_MML_PLMN_WITH_RAT_STRU                             *pstPlmnWithRat,
    VOS_UINT32                                              ulPlmnDisabledExpiredSlice,
    MMC_LMM_DISABLE_LTE_REASON_ENUM_UINT32                  enDisableLteReason
)
{
    NAS_MML_DISABLED_RAT_PLMN_CFG_INFO_STRU                *pstDisabledPlmnIdWithRatList = VOS_NULL_PTR;
    ...
    pstDisabledPlmnIdWithRatList->astDisabledRatPlmnId[ulIndex].ulPlmnDisabledExpiredSlice   = ulPlmnDisabledExpiredSlice;
    ...
    /* 如果当前PLMN是尾部插入，则增加惩罚PLMN个数，否则只是更新PLMN的属性信息，不需要增加总个数 */
    if ( ulIndex == pstDisabledPlmnIdWithRatList->ulDisabledRatPlmnNum )
    {
...
VOS_UINT32 NAS_MML_IsNeedDeleteDisabledPlmnWithValidPeriodInPlmnList (
    NAS_MML_DISABLED_PLMN_WITH_VALID_PERIOD_STRU           *pstDisabledPlmnIdWithRat
)
{
    ...
    if (NAS_MML_MAX_TIME_SLICE == pstDisabledPlmnIdWithRat->ulPlmnDisabledExpiredSlice)
    {
        return VOS_FALSE;
    }
    ...
    if (ulDuration >= pstDisabledPlmnIdWithRat->ulPlmnDisabledExpiredSlice)
    {
        return VOS_TRUE;
    }
...
VOS_UINT32 NAS_MML_IsPlmnIdWithRatInDestDisabledPlmnWithValidPeriodList (
    NAS_MML_PLMN_WITH_RAT_STRU                             *pstSrcPlmnId,
    VOS_UINT32                                              ulDestPlmnNum,
    NAS_MML_DISABLED_PLMN_WITH_VALID_PERIOD_STRU           *pstDestPlmnIdList
)
{
    VOS_UINT32                          i;
    VOS_UINT32                          ulIndex;
...
VOS_VOID NAS_MML_AddDisabledRatPlmnWithValidPeriodList(
    NAS_MML_PLMN_WITH_RAT_STRU                             *pstPlmnWithRat,
    VOS_UINT32                                              ulPlmnDisabledExpiredSlice,
    MMC_LMM_DISABLE_LTE_REASON_ENUM_UINT32                  enDisableLteReason
)
{
    ...
    /* 惩罚PLMN最大个数异常保护 */
    if ( pstDisabledPlmnIdWithRatList->ulDisabledRatPlmnNum > NAS_MML_MAX_DISABLED_RAT_PLMN_NUM )
    {
        pstDisabledPlmnIdWithRatList->ulDisabledRatPlmnNum = NAS_MML_MAX_DISABLED_RAT_PLMN_NUM;
    }

    ulIndex                         = pstDisabledPlmnIdWithRatList->ulDisabledRatPlmnNum;

    /* 如果当前PLMN已经在惩罚列表里面，则不需要添加 */
    for ( i = 0 ; i < pstDisabledPlmnIdWithRatList->ulDisabledRatPlmnNum; i++ )
    {
        if ((VOS_TRUE               == NAS_MML_CompareBcchPlmnwithSimPlmn(&(pstPlmnWithRat->stPlmnId), &(pstDisabledPlmnIdWithRatList->astDisabledRatPlmnId[i].stPlmnWithRat.stPlmnId)))
         && (pstPlmnWithRat->enRat  == pstDisabledPlmnIdWithRatList->astDisabledRatPlmnId[i].stPlmnWithRat.enRat))
    ...
    /* 将PLMN加入到禁止接入技术列表里面 */        
    NAS_MML_UpdateDisabledPlmnWithValidPeriodList(pstPlmnWithRat, ulPlmnDisabledExpiredSlice, enDisableLteReason);
...
VOS_VOID NAS_MML_DelEqualPlmnsInDisableRatList(
    ...
    NAS_MML_DISABLED_PLMN_WITH_VALID_PERIOD_STRU           *pstDisabledRatPlmnIdList
)
{
    VOS_UINT8                           i;
    VOS_UINT8                           ucPlmnNum;
    ...
    for ( i = 0; i < ucPlmnNum;  )
    {
        ...
        ulRlst = NAS_MML_IsPlmnIdWithRatInDestDisabledPlmnWithValidPeriodList(&stPlmnWithRat,
