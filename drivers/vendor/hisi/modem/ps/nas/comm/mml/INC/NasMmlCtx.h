...
enum NAS_MML_REG_FAIL_CAUSE_ENUM
{
    ...
    NAS_MML_REG_FAIL_CAUSE_HO_SUCC                                  = 419,      /* HO succ之后，接入层未上报系统消息时带的原因值 */
...
typedef struct
{
    ...
    /* 记录禁止时间，该列表每次使用前需要判断是否有网络禁止时间到，
       如果禁止时间到动态删除,从nv获取的网络默认禁止时间为全F最大值 */
    VOS_UINT32                          ulPlmnDisabledExpiredSlice;

    MMC_LMM_DISABLE_LTE_REASON_ENUM_UINT32                  enDisableLteReason;
    
}NAS_MML_DISABLED_PLMN_WITH_VALID_PERIOD_STRU;
...
typedef struct
{
    ...
    NAS_MML_DISABLED_PLMN_WITH_VALID_PERIOD_STRU    astDisabledRatPlmnId[NAS_MML_MAX_DISABLED_RAT_PLMN_NUM];/* 禁止接入技术的PLMN和RAT信息 */
