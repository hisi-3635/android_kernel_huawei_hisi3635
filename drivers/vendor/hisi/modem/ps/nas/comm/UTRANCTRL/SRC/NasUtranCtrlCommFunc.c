...
)
{
    ...
    switch ( enCurrentUtranMode )
    {
        /* W模的处理 */
        case NAS_UTRANCTRL_UTRAN_MODE_FDD :
            WTTF_RLC_GetRbSuitableUlPduSize(ucRbId,
                                           ulPduSize,
                                           ucCrcSize,
                                           pulUlPduSize);
            break;
...
#if ( FEATURE_ON == FEATURE_UE_MODE_TDS )
        case NAS_UTRANCTRL_UTRAN_MODE_TDD :
            TDS_RLC_GetRbSuitableUlPduSize(ucRbId,
                                           ulPduSize,
                                           ucCrcSize,
                                           pulUlPduSize);
...
)
{
    ...
    /* 将usSndPid左移16位赋值给ulEventType */
    /*lint -e701*/
    ulEventType = usSndPid << NAS_UTRANCTRL_OCTET_MOVE_SIXTEEN_BITS;
    /*lint +e701*/

    ulEventType |= usMsgName;
...
)
{
    ...
    */
    if ( VOS_PID_TIMER == pstMsgHeader->ulSenderPid )
    {
        ...
        ulEventType  = NAS_UTRANCTRL_BuildEventType((VOS_UINT16)pstMsgHeader->ulSenderPid, (VOS_UINT16)pRcvTimerMsg->ulName);
    }
    else
    {
        ulEventType  = NAS_UTRANCTRL_BuildEventType((VOS_UINT16)pstMsgHeader->ulSenderPid, (VOS_UINT16)pstMsgHeader->ulMsgName);
    }
...
{
    NAS_MMC_StartTimer(enTimerId, ulLen);
}
...
)
{
    ...
    if ( VOS_NULL_PTR == pstMsg )
    {
        ...
        vos_printf("NAS_UTRANCTRL_SndGuAsPowerOffReq.memory alloc error.\n ");
        return ;
    }
    ...
    if ( VOS_OK != ulRet )
    {
        vos_printf("NAS_UTRANCTRL_SndGuAsPowerOffReq.Snd Msg Fail.\n ");

        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_SndGuAsPowerOffReq:ERROR:PS_SEND_MSG ");
    }

    vos_printf("NAS_UTRANCTRL_SndGuAsPowerOffReq.Snd Msg succ.ulReceiverPid = %d.\n ",ulReceiverPid);
...
)
{
    ...
    vos_printf("NAS_UTRANCTRL_SndGuAsStartReq, ulReceiverPid:%d, ulRet:%d. \n ", ulRcvPid, ulRet);
...
VOS_UINT32 NAS_UTRANCTRL_SndAnotherUtranModeMsg_SndUtranAsMsgPreProcessMsg(
    VOS_UINT32                          ulSndPid,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 NAS_UTRANCTRL_StartUtranTddModeMsgTimer(
    VOS_UINT32                          ulSndPid,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 NAS_UTRANCTRL_StartUtranFddModeMsgTimer(
    VOS_UINT32                          ulSndPid,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 NAS_UTRANCTRL_StopUtranTddModeMsgTimer(
...
VOS_UINT32 NAS_UTRANCTRL_StopUtranFddModeMsgTimer(
    VOS_UINT32                          ulMsgName
)
{
    switch ( ulMsgName )
...
VOS_UINT32 NAS_UTRANCTRL_StartUtranModeMsgTimer_SndUtranAsMsgPreProcessMsg(
    VOS_UINT32                          ulSndPid,
    struct MsgCB                       *pstMsg
)
{
