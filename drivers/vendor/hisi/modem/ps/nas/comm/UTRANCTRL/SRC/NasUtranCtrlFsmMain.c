...
)
{
    ...
    /* 如果指定搜网成功或列表搜网时或自动切换模式标记关闭时直接返回进入MMC状态机处理 */
    if ((NAS_UTRANCTRL_UTRAN_SWITCH_MODE_AUTO != NAS_UTRANCTRL_GetCurUtranSwitchMode())
     || (RRC_PLMN_SEARCH_RLT_SPEC             == pstSrchCnfMsg->ulSearchRlt)
     || (RRC_PLMN_SEARCH_RLT_LIST             == pstSrchCnfMsg->ulSearchRlt)
     || (RRC_PLMN_SEARCH_RLT_LIST_REJ         == pstSrchCnfMsg->ulSearchRlt)
     || (RRC_PLMN_SEARCH_RLT_LIST_FAIL        == pstSrchCnfMsg->ulSearchRlt))
    {
        /* 进入MMC状态机处理 */
        return VOS_FALSE;
    }
...
)
{
    ...
    /* 如果指定搜网成功或列表搜网时或自动切换模式标记关闭时直接返回进入MMC状态机处理 */
    if ((NAS_UTRANCTRL_UTRAN_SWITCH_MODE_AUTO != NAS_UTRANCTRL_GetCurUtranSwitchMode())
     || (MMC_LMM_PLMN_SRCH_RLT_SPEC_SUCC       == pstLmmSrchCnfMsg->enRlst)
     || (MMC_LMM_PLMN_SRCH_RLT_LIST_SUCC       == pstLmmSrchCnfMsg->enRlst)
     || (MMC_LMM_PLMN_SRCH_RLT_LIST_REJ        == pstLmmSrchCnfMsg->enRlst))
    {
        /* 进入MMC状态机处理 */
        return VOS_FALSE;
    }
