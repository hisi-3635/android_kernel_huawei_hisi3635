...
)
{
    ...
    /*add by lifuxin for Lnas est process re-construct*/
         ...
         EMM_OM_ERRLOG_TYPE_ENUM_UINT16    enErrType)
{
    ...
    {
        stAttachRslt.stAttachCnRej.enCnRejCause = NAS_LMM_CnCauseProc(pstCnAttachRej->ucCause);/* [false alarm]:代码保证了不是空指针 */
    ...
    {
        if (NAS_EMM_BIT_SLCT == pstCnAttachEpsOnly->ucBitOpEmmCau)/* [false alarm]:代码保证了不是空指针 */
        {
            stAttachRslt.stAttachCnRej.enCnRejCause = NAS_LMM_CnCauseProc(pstCnAttachEpsOnly->ucEMMCause);
        }
        else
        {
    ...
    {
        stAttachRslt.enEstCnfRslt = NAS_LMM_EstCnfFailRsltProc(pstEstCnfMsg->enResult);/* [false alarm]:代码保证了不是空指针 */
    }
    else if(EMM_OM_ERRLOG_TYPE_DATA_CNF_FAIL == stAttachRslt.enErrType)
    {
        stAttachRslt.enDataCnfRslt = NAS_LMM_DataCnfFailRsltProc(pRrcDataCnf->enSendRslt);/* [false alarm]:代码保证了不是空指针 */
    }
    else
    {
         ...
         EMM_OM_ERRLOG_TYPE_ENUM_UINT16    enErrType)
{
    ...
    {
        stTAURslt.stTauCnRej.enCnRejCause = NAS_LMM_CnCauseProc(pstCnTAURej->ucEMMCause);/* [false alarm]:代码保证了不是空指针 */
    ...
    {
        if (NAS_EMM_BIT_SLCT == pstCnTAUEpsOnly->ucBitOpEmmCau)/* [false alarm]:代码保证了不是空指针 */
        {
            stTAURslt.stTauCnRej.enCnRejCause = NAS_LMM_CnCauseProc(pstCnTAUEpsOnly->ucEMMCause);
        }
        else
        {
    ...
    {
        stTAURslt.enEstCnfRslt = NAS_LMM_EstCnfFailRsltProc(pstEstCnfMsg->enResult);/* [false alarm]:代码保证了不是空指针 */
    }
    else if(EMM_OM_ERRLOG_TYPE_DATA_CNF_FAIL == stTAURslt.enErrType)
    {
        stTAURslt.enDataCnfRslt = NAS_LMM_DataCnfFailRsltProc(pRrcDataCnf->enSendRslt);/* [false alarm]:代码保证了不是空指针 */
    }
    else
    {
...

{
    ...
    if(EMM_OM_ERRLOG_TYPE_CN_REJ == stNorServerRslt.enErrType)
    {
        stNorServerRslt.stNorServiceCnRej.enCnRejCause =
                                        NAS_LMM_CnCauseProc(pstCnSerRej->ucEMMCause);/* [false alarm]:代码保证了不是空指针 */
    }
    else if(EMM_OM_ERRLOG_TYPE_EST_CNF_FAIL == stNorServerRslt.enErrType)
    {
        stNorServerRslt.enEstCnfRslt = NAS_LMM_EstCnfFailRsltProc(pstEstCnfMsg->enResult);/* [false alarm]:代码保证了不是空指针 */
    }
    else if(EMM_OM_ERRLOG_TYPE_DATA_CNF_FAIL == stNorServerRslt.enErrType)
    {
        stNorServerRslt.enDataCnfRslt = NAS_LMM_DataCnfFailRsltProc(pRrcDataCnf->enSendRslt);/* [false alarm]:代码保证了不是空指针 */
    }
    else
    {
