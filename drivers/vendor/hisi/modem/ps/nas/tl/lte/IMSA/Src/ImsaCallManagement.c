...
VOS_UINT32 IMSA_CallCtxInit(VOS_VOID)
{
    ...
    pstCallCtx->stCMCCTcallTimer.phTimer     = VOS_NULL_PTR;
    pstCallCtx->stCMCCTcallTimer.ucMode      = VOS_RELTIMER_NOLOOP;
    pstCallCtx->stCMCCTcallTimer.usName      = TI_IMSA_CONFIG_PARA_CMCC_TCALL;
    pstCallCtx->stCMCCTcallTimer.ulTimerLen  = 0;
...
VOS_UINT32 IMSA_CallCtxDeinit(VOS_VOID)
{
    ...
    if (VOS_TRUE == IMSA_IsTimerRunning(&pstCallCtx->stCMCCTcallTimer))
    {
        IMSA_StopTimer(&pstCallCtx->stCMCCTcallTimer);
    }
...
VOS_VOID IMSA_CallProcTimeoutCMCCTCall(const VOS_VOID *pTimerMsg)
{
    VOS_UINT32 i = 0;
    ...
    if(IMSA_CALL_MAX_NUM == i)
    {
        IMSA_ERR_LOG("IMSA_CallProcTimeoutCMCCTCall: Call Entity not found.");
        return;
    }

    if(ulIsDialingTimeOut)
    {
        IMSA_INFO_LOG1("IMSA_CallProcTimeoutCMCCTCall: Hang up call id: ", pstCallCtx->astCallEntity[i].ucImscallId);
        (VOS_VOID)IMSA_CallImsCmdRel(pstCallCtx->astCallEntity[i].ucImscallId);
    }
    pstCallCtx->ucIsCmccTcallTimeOutProc = VOS_TRUE;
}


VOS_VOID IMSA_StartCMCCTcallTimer(const IMSA_CALL_ENTITY_STRU *pstCallEntity)
{
    IMSA_CALL_MANAGER_STRU    *pstCallCtx = IMSA_CallCtxGet();
    ...
    {
        /* 启动中国移动Tcall定时器 */
        pstCallCtx->stCMCCTcallTimer.ulTimerLen = IMSA_GetConfigParaAddress()->stCMCCCustomReq.ulTcallTimerLen;
        pstCallCtx->stCMCCTcallTimer.usPara = pstCallEntity->ucCallEntityIndex;
        IMSA_StartTimer(&pstCallCtx->stCMCCTcallTimer);
    }
}
