...
/*    {   EMM_SS_NULL_WAITING_USIM_READY  ,
    ...
    /*=================== MMC的子状态 ======================*/
    /*========== MMC_MS_NULL 专有 =========*/
    {   MMC_SS_WAIT_START_REQ           ,
            "SS:  MMC_SS_WAIT_START_REQ              "    },

    /*==== MMC_MS_TRYING_PLMN和MMC_MS_TRYING_PLMN_LIST共用子状态 ===*/
    {   MMC_SS_WAIT_FOR_EMM_PLMN_SRCH_REQ   ,
            "SS:  MMC_SS_WAIT_FOR_EMM_PLMN_SRCH_REQ  "    },
    {   MMC_SS_WAIT_FOR_RRC_PLMN_SRCH_RSLT       ,
            "SS:  MMC_SS_WAIT_FOR_RRC_PLMN_SRCH_RSLT "    },
    {   MMC_SS_WAIT_FOR_ACTION_RESULT   ,
            "SS:  MMC_SS_WAIT_FOR_ACTION_RESULT      "    },
    {   MMC_SS_WAIT_FOR_RRC_SUIT_CELL_SRCH_RSLT   ,
            "SS:  MMC_SS_WAIT_FOR_RRC_SUIT_CELL_SRCH_RSLT"    },

    /*========== MMC_MS_TRYING_PLMN_LIST专有 ===*/
    {   MMC_SS_WAIT_FOR_EMM_PLMN_LIST_SRCH_REQ   ,
            "SS:  MMC_SS_WAIT_FOR_EMM_PLMN_LIST_SRCH_REQ"    },
    {   MMC_SS_WAIT_FOR_RRC_PLMN_LIST_SRCH_RSLT            ,
            "SS:  MMC_SS_WAIT_FOR_RRC_PLMN_LIST_SRCH_RSLT"    },


    /*========== NAS_EMMC_MS_ON_PLMN专有 =======*/
    {   MMC_SS_ON_VPLMN                 ,
            "SS:  MMC_SS_ON_VPLMN                    "    },
    {   MMC_SS_ON_HPLMN                 ,
            "SS:  MMC_SS_ON_HPLMN                    "    },
    {   MMC_SS_ON_LIMITED_SERVICE                 ,
            "SS:  MMC_SS_ON_LIMITED_SERVICE          "    },

    /*========== MMC_MS_WAIT_FOR_PLMN专有 =====*/
    {   MMC_SS_WAIT_FOR_SYS_INFO        ,
            "SS:  MMC_SS_WAIT_FOR_SYS_INFO           "    },

    /*========== NAS_EMMC_MS_NOT_ON_PLMN专有 ===*/
    {   MMC_SS_WAIT_FOR_USR_SEL         ,
            "SS:  MMC_SS_WAIT_FOR_USR_SEL            "    },

    /*========== NAS_EMMC_MS_NO_SIM 专有 =======*/
    {   MMC_SS_WAIT_SIM_INSERT          ,
            "SS:  MMC_SS_WAIT_SIM_INSERT             "    },
    #endif
    /*=================== SECU的NEW状态 ===================*/
...
NAS_COMM_PRINT_MSG_LIST_STRU g_astMmMmcMsgIdArray[] =
{
    ...
    /*xiongxianghui00253310 add for st begin */
...
#if 0
    /*======================= 打印SIM卡状态 ==================================*/
    ilOutPutLen = NAS_LMM_PUBM_PrintUsimState(
                        pcBuff,
                        usTotalLen,
                        pstAuxFsm->ucUsimState);
    if ( 0 == ilOutPutLen )
    {
        NAS_LMM_PUBM_LOG_WARN("NAS_LMM_PUBM_PrintAuxFsm, NAS_LMM_PUBM_PrintSubState exception.");
        return 0;
    }
    usTotalLen += (VOS_UINT16)(ilOutPutLen);
...
/*    VOS_UINT16                          usLineNo            = 1;*/
    ...
    for (usLoop = 0; usLoop < usDataLen; usLoop++)
    {
        ...
        if ( (0 != usLoop) && (0 == (usLoop%NAS_COMM_PRINT_MAX_BYTES_PER_LINE)) )
        {
            ...
            if ( 0 == ilOutPutLen )
            {
                NAS_LMM_PUBM_LOG_WARN("NAS_LMM_PUBM_PrintData, Print LineNo exception.");
                return 0;
            }
            usTotalLen += (VOS_UINT16)ilOutPutLen;
            usLineNo++;
