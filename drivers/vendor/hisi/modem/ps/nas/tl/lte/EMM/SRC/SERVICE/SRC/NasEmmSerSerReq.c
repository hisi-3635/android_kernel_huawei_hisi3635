...
VOS_UINT32 NAS_EMM_SER_VerifyNormalCsfb(VOS_VOID)
{
    ...
    {
       NAS_EMM_SER_LOG_NORM("NAS_EMM_SER_VerifyNormalCsfb:cs is not registered!");
       return NAS_EMM_FAIL;
    }
...
)
{
    ...
    {
        NAS_EMM_SER_LOG_NORM("NAS_EMM_MsRegPreProcMmNormalCsfbNotifyMsg:REG+ATTEMPTING TO UPDATE!");
        /* 受限服务需要判断IMS电话，此处暂时先不考虑IMS */
        NAS_EMM_MmcSendSerResultIndOtherType(MMC_LMM_SERVICE_RSLT_FAILURE);
        NAS_EMM_RelReq(NAS_LMM_NOT_BARRED);
        return NAS_EMM_SUCC;
    }
...
)
{
    ...
    {
        NAS_EMM_SER_LOG_NORM("NAS_EMM_RcvMmNormalCsfbNotifyMsgProc:SMS only or T3442 is running.");
        NAS_EMM_MmcSendSerResultIndOtherType(MMC_LMM_SERVICE_RSLT_FAILURE);
        NAS_EMM_RelReq(NAS_LMM_NOT_BARRED);
        return NAS_LMM_MSG_HANDLED;
    }
...
)
{
    ...
    /* MT CSFB不能发起 */
    if ((MM_LMM_CSFB_SERVICE_MT_NORMAL == pstCsfbSerStartNotify->enCsfbSrvType)
      &&(NAS_EMM_SUCC == NAS_EMM_JudgeBarType(NAS_EMM_BAR_TYPE_MT)))
    {
        NAS_EMM_MmcSendSerResultIndOtherType(MMC_LMM_SERVICE_RSLT_FAILURE);
        return NAS_LMM_MSG_HANDLED;
    }
