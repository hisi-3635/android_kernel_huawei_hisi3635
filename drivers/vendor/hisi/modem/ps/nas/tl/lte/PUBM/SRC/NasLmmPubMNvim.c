...
static NAS_LMM_NV_ACT_STRU               g_astNvDataMap[] =
{
    ...
    {EN_NV_ID_DAM_CONFIG_PARA,       NAS_USIM_FILE_ID_INVALID,     NAS_LMM_DecodeOneNvItem,   NAS_LMM_EncodeOneNvItem},
    {EN_NV_ID_LNAS_COMM_CONFIG_PARA,       NAS_USIM_FILE_ID_INVALID,     NAS_LMM_DecodeOneNvItem,   NAS_LMM_EncodeOneNvItem},

    /*SIM数据*/
    {EN_NV_ID_MNC_LEN_FILE,         NAS_USIM_FILE_ID_MNC_LEN,     NAS_LMM_DecodeSimMncLen,     NAS_LMM_EncodeOneNvItem},
...
VOS_VOID  NAS_LMM_ReadLteUsimFile( VOS_VOID )
{
    ...
    /*6FAD MNC长度*/
    ulRslt = NAS_LMM_ReadSimMncLen();
    if(NAS_LMM_ERR_CODE_HARD_USIM == ulRslt)
    {   /* 返回值 NAS_LMM_ERR_CODE_HARD_USIM 意味着读硬SUIM卡 */
        ulReadHardUsimFileNum++;
    }
