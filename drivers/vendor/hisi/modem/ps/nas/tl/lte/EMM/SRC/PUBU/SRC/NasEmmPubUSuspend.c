...
{
    NAS_EMM_PUBU_LOG_INFO("NAS_EMM_ProcInterSystemTransferBearerInfo: GET MML PS BEARER INFO:");
                                                             ...
                                                             VOS_VOID   *pMsgStru   )
{
    ...
    NAS_EMM_PUBU_LOG_INFO("NAS_EMM_PreProcMmcSuspendRsp: enter. ");
    ...
    {
        NAS_EMM_PUBU_LOG1_INFO("NAS_EMM_PreProcMmcSuspendRsp fail enter: MMC enRslt = ",
                               pMmcEmmRusRsp->ulRst);
    ...
    {
        /* 挂起资源 */
        NAS_EMM_PUBU_LOG_INFO("NAS_EMM_PreProcMmcSuspendRsp: succ enter.");
...
VOS_VOID  NAS_EMM_GuCombinedAttachPsOnlySucc( MsgBlock * pMsg  )
{
    MMC_LMM_ACTION_RESULT_REQ_STRU     *pMmcActResult = VOS_NULL_PTR;

    pMmcActResult = (MMC_LMM_ACTION_RESULT_REQ_STRU *)pMsg;

    /*GU在CN REJ时一定会将bitOpCnCause置为TURE,此处不再容错*/
    switch(pMmcActResult->ucCnCause)
    {
        /* #2,16,17,22进入REG.NoCellAvail状态*/
...
#if 0
VOS_VOID  NAS_EMM_GuCmbRauPsOnlyRej( MsgBlock * pMsg  )
{
    MMC_LMM_ACTION_RESULT_REQ_STRU     *pMmcActResult = VOS_NULL_PTR;
    ...
    switch(pMmcActResult->ucCnCause)
    {
        /* #2,16,17,22,18进入REG.NoCellAvail状态*/
        case NAS_LMM_CAUSE_IMSI_UNKNOWN_IN_HSS:
        case NAS_LMM_CAUSE_MSC_UNREACHABLE:
        case NAS_LMM_CAUSE_NETWORK_FAILURE:
        case NAS_LMM_CAUSE_PROCEDURE_CONGESTION:
        ...
        /*#95,96,97,99,101,GU下处理将RauAttempCounter置为5*/
        case NAS_LMM_CAUSE_SEMANTICALLY_INCORRECT_MSG:
        case NAS_LMM_CAUSE_INVALID_MANDATORY_INF:
        case NAS_LMM_CAUSE_MSG_NONEXIST_NOTIMPLEMENTE:
        case NAS_LMM_CAUSE_IE_NONEXIST_NOTIMPLEMENTED:
        case NAS_LMM_CAUSE_MSG_NOT_COMPATIBLE:
            ...
            /* 这种情况下RauAttempCounter为5*/
            NAS_EMM_ProcGuRauAttemptCounter(pMsg);
            break;

        /*其余原因值处理，GU将RauAttempCounter加1，按RauAttempCounter的处理*/
        default    :
            NAS_EMM_ProcGuRauAttemptCounter(pMsg);
            break;
    }
...
VOS_UINT32  NAS_EMM_MsTauSerWaitRrcRelRcvLrrcSyscfgCnfProc(VOS_VOID)
{
    /* 打印进入函数 */
    NAS_EMM_PUBU_LOG_INFO("NAS_EMM_MsTauSerWaitRrcRelRcvLrrcSyscfgCnfProc is entered !");

    /* 无论是GU变还是L变，无论是TAU还是SER，均统一处理 */
    if (NAS_LMM_UE_RADIO_CAP_NOT_CHG != NAS_LMM_GetEmmInfoUeRadioCapChgFlag())
    {
        NAS_EMM_PUBU_LOG_NORM("NAS_EMM_MsTauSerWaitRrcRelRcvLrrcSyscfgCnfProc: UE radio capability is changed, send RRC release.");

        /* 主动释放, 压栈处理后自动在状态机中处理TAU和SER的结果 */
        NAS_EMM_RelReq(NAS_LMM_NOT_BARRED);
    }
    else
    {
