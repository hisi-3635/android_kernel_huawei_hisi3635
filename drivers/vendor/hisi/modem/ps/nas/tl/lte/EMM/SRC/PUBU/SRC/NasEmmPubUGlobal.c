...
VOS_UINT32  NAS_LMM_ReadSimMncLen(VOS_VOID)
{
    VOS_VOID                           *pData;
    VOS_UINT32                          ulRslt;
    VOS_UINT16                          usDataLen;
    VOS_UINT8                           aucSimMncFile[NAS_LMM_NVIM_MNCLEN_FILE_MIN_LEN];

    pData  = aucSimMncFile;
    usDataLen = NAS_LMM_NVIM_MNCLEN_FILE_MIN_LEN;
    ulRslt = NAS_LMM_NvimRead(EN_NV_ID_MNC_LEN_FILE,
                              pData,
                              &usDataLen);

    if (NAS_LMM_NVIM_HARD_USIM == ulRslt)
    {
        NAS_LMM_PUBM_LOG_NORM("NAS_LMM_ReadSimMncLen: read usim MNC LEN!");
                               ...
                               MMC_LMM_LTE_NO_SUBSCRIBE_PLMN_LIST_STRU  *pstLteNoSubsPLmnList)
{
    ...
    NAS_LMM_SendMmcEutranNotAllowNotify(NAS_LMM_WITH_EXT_CAUSE, ulTimerLen, pstLteNoSubsPLmnList);
                        ...
                        MMC_LMM_LTE_NO_SUBSCRIBE_PLMN_LIST_STRU  *pstLteNoSubsPLmnList)
{
    ...
    if(NAS_EMM_YES == NAS_LMM_LteNoSubscribeAtLeastHaveOneHplmn(pstLteNoSubsPLmnList))
    {
        ...
        {
            NAS_LMM_SendMmcEutranNotAllowNotify(
