...
)
{
    ...
    else if(ulCurEmmStat == NAS_LMM_PUB_COMP_EMMSTATE( EMM_MS_REG, EMM_SS_REG_LIMITED_SERVICE))
    {
        NAS_EMM_SER_RcvRrcCsPagingInd(enPagingUeId);
    }
                                    ...
                                    VOS_VOID               *pMsg )
{
    ...
    if(NAS_EMM_AUTO_ATTACH_NOT_ALLOW == NAS_EMM_GetPsAttachAllowFlg())
    {
        ...
        if (EMMC_EMM_NO_CHANGE != pstMmcSysInfoInd->ulChangeInfo)
        {
            NAS_EMM_AppSendAttRstDefaultReqType(MMC_LMM_ATT_RSLT_PS_ATT_NOT_ALLOW);
        }
        return NAS_LMM_MSG_HANDLED;
    }
