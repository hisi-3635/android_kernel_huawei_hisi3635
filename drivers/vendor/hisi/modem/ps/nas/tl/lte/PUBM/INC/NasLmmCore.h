...
#if 0
/*****************************************************************************
结构名称    :NAS_LMM_EPS_BEARER_CONTEXT_STRU
使用说明    :MM的EPS承载上下文
             根据 ESM层的消息，维护EPS承载上下文的信息
             ucStatusChange:TRUE 认为UE侧的 EPS承载上下文与网侧不一致，需要在下次TAU时通知网侧
                            FALSE 认为UE侧的 EPS承载上下文与网侧一致
             ulEpsIdNum:   当前已激活的EPSID个数
             aulEsmEpsId[EMM_ESM_MAX_EPS_BEARER_NUM]:已激活的EPS BID
*****************************************************************************/
typedef struct
{
    VOS_UINT32                          ulEpsIdNum;
    VOS_UINT32                          aulEsmEpsId[EMM_ESM_MAX_EPS_BEARER_NUM];
}NAS_EMM_EPS_CONTEXT_STATUS;

typedef struct
{
    NAS_EMM_EPS_BEARER_CONTEXT_STATUS_ENUM_UINT32   enStatusChange;
    NAS_EMM_EPS_CONTEXT_STATUS           stEpsContextStatus;
}NAS_LMM_EPS_BEARER_CONTEXT_STRU;
typedef NAS_LMM_EPS_BEARER_CONTEXT_STRU  NAS_EMM_EPS_BEARER_CONTEXT_STRU;
typedef NAS_LMM_EPS_BEARER_CONTEXT_STRU  NAS_EMMC_EPS_BEARER_CONTEXT_STRU;
#endif
