...
)
{
    ...
    NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_EST_IND, TI_NAS_MMC_WAIT_EST_IND_LEN);
...
)
{
    ...
    switch(pstRrconnInfo->ulRrConnFlg)
    {
        ...
        case MMC_MM_RR_CONN_ABSENT:
            ...
            NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_AS_SUSPEND_IND, TI_NAS_MMC_WAIT_AS_SUSPEND_IND_LEN);
...
)
{
    ...
    /* 目前流程中G下注册过程中WRR会发一条假的EST_CNF消息到GMM，GMM处理时会发PS信令链接存在
       指示给MMC，此时MMC应该忽略该消息,G下应该以TBF链路建好为CCO退出的标准 */
    if ( NAS_MML_NET_RAT_TYPE_GSM == NAS_MML_GetCurrNetRatType())
    {
