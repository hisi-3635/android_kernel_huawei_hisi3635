...
typedef struct
{
    ...
    VOS_UINT32                               ulTimerRunMask;                         /* Timer启动标记(有哪些Timer在运行)         */
    VOS_UINT32                               ulTimerSusMask;                         /* Timer挂起标记(有哪些Timer被挂起)         */

    VOS_UINT32                               ulTimerSusTcMask;                       /* Timer在TC中挂起标记(有哪些Timer被挂起)   */

    VOS_UINT32                               ulTimerValMask;                         /* Timer时长更新标记(有哪些Timer时长被更新) */
    struct{
...
typedef struct
{
    VOS_UINT16                             usOptionalIeMask;                        /* 可选IE的MASK                             */

    VOS_UINT8                              aucIeOffset[16];                        /* IE的偏移量的存储空间                     */
