...
VOS_VOID Mm_ComTmsiReloc()
{
                                                    ...
                                                    MM_MS_ID_IMSI_PRESENT ) )
    {
        ...
        Mm_SndAgentUsimUpdateFileReq(MM_READ_LOCA_INFO_FILE_ID);                /* 向USIM发送AGENT_USIM_UPDATE_FILE_REQ     */
    }
    else
    {
...
VOS_VOID Mm_ComInit ()
{
    ...
    PS_MEM_SET( &gstMmTimer[0], 0, ( MM_TIMER_NUM * sizeof( MM_TIMER_STRU ) ) );/* 初始化                                   */
...
VOS_VOID Mm_ComSetMmState(NAS_MM_STATE_ENUM_UINT8 ulState)
{
    ...
    switch(ulState)
    {
        ...
        case MM_CONNECTION_ACTIVE:
            ...
            if (VOS_OK == ulRet)
            {
                NAS_MM_SndRrcTransactionStatusNotify(ulRcvPid, RRMM_TRANSACTION_CONNECTION_EST_SUCC);
            }
            break;
...
)
{
    ...
    /* 如果当前是SYSCFG触发的DETACH,则设置保护定时器的时长为15s */
...
VOS_VOID  NAS_MM_LogMmCtxInfo(VOS_VOID)
{
    ...
    pstMsg->stMmlCtxInfo.enCallMode                           = 2;
    ...
    /* Added by c00318887 for AT&T phaseI, 2015-2-4, begin */
    pstMsg->stMmlCtxInfo.ucPlmnSupportDam                       = (VOS_UINT8)NAS_MML_IsPlmnSupportDam(NAS_MML_GetCurrCampPlmnId());
    /* Added by c00318887 for AT&T phaseI, 2015-2-4, end */
    ...
    pstMsg->stMmTimerInfo.ucT3242TimerStatus                  = 0;
    pstMsg->stMmTimerInfo.ucT3243TimerStatus                  = 0;
...
)
{
    ...
    if (VOS_TRUE == NAS_MML_GetErrLogEstCnfCsfbMtFailRecordFlag())
    {
        return;
    }
...
VOS_VOID NAS_MM_ProcCsfbMtFailRecord(VOS_VOID)
{
    if (NAS_MML_CSFB_SERVICE_STATUS_MT_EXIST  == NAS_MML_GetCsfbServiceStatus())
    {
        ...
        /* 清除CSFB MT的状态 */
        NAS_MML_SetErrLogCsfbMtState(NAS_ERR_LOG_CSFB_MT_STATE_NULL);

        if (1 == NAS_MML_GetErrLogWaitForRrConnRcvEstCnfFlag())
        {
            NAS_MML_SetErrLogEstCnfCsfbMtFailRecordFlag(1);
        }
    }
