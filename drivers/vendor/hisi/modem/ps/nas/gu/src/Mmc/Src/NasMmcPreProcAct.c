...
)
{
    ...
    if (VOS_TRUE == NAS_MML_IsAcInfoChanged(&stOldCsRestrictInfo, pstCurCsRestrictInfo))
    {
         ...
         && (VOS_TRUE  == pstWasAcInfoChangeInd->stCsRestriction.enRestrictRegister))
        {
            ...
            {
                NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_CS, NAS_MMC_LIMITED_SERVICE);
    ...
    if (VOS_TRUE == NAS_MML_IsAcInfoChanged(&stOldPsRestrictInfo, pstCurPsRestrictInfo))
    {
         ...
         && (VOS_TRUE == pstWasAcInfoChangeInd->stPsRestriction.enRestrictRegister))
        {
            ...
            {
                NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_PS, NAS_MMC_LIMITED_SERVICE);
...
)
{
    ...
    ulIsPlmnIdInDestPlmnWithRatList = NAS_MML_IsPlmnIdWithRatInDestDisabledPlmnWithValidPeriodList(&stPlmnWithRat,
    ...
    for ( i = 0; i < pstEquPlmnList->ucEquPlmnNum; i++ )
    {
        ...
        if (VOS_TRUE == NAS_MML_IsPlmnIdWithRatInDestDisabledPlmnWithValidPeriodList(&stPlmnWithRat,
                                                                     pstDisabledRatPlmnCfg->ulDisabledRatPlmnNum,
                                                                     pstDisabledRatPlmnCfg->astDisabledRatPlmnId))
        {
            continue;
...
VOS_VOID NAS_MMC_BuildRefreshFileList(
    PS_USIM_REFRESH_IND_STRU           *pstUsimRefreshInd,
    PS_USIM_REFRESH_FILE_CONTENT_STRU  *pstRefreshFile
)
{
    VOS_UINT32      ulLoop;
    VOS_UINT16      usFileId;
    ...
    if (USIMM_REFRESH_FILE_LIST == pstUsimRefreshInd->enRefreshType)
    {
        ...
        {
            PS_MEM_CPY(&pstRefreshFile->astRefreshFile[ulLoop], &pstUsimRefreshInd->astEfId[ulLoop], sizeof(PS_USIM_REFRESH_FILE_STRU)); 
        }
    ...
    /* ALL FILE 类型时，认为FILE ID 是开机时读的所有文件 */    
    ...
    for (ulLoop = 0; ulLoop < usNumber; ulLoop++)
    {
        ...
        pstRefreshFile->astRefreshFile[usIndex].enAppType    = USIMM_UNLIMIT_APP; 
        usIndex++;
...
/*****************************************************************************
 ...
 1.日    期   : 2015年5月8日
   作    者   : c00318887
   修改内容   : for file refresh需要触发背景搜 增加0x6F7B的处理
...
)
{
    ...
    if ((NAS_MML_READ_UPLMN_SEL_FILE_ID == usFileId)
     || ( (VOS_FALSE == ulNvOplmnAvail)
         && (NAS_MML_READ_OPLMN_SEL_FILE_ID == usFileId) )
    || (NAS_MML_READ_PLMN_SEL_FILE_ID == usFileId)
    || (NAS_MML_READ_FORBIDDEN_PLMN_LIST_FILE_ID == usFileId))
    {
        return VOS_TRUE;
    }
...
)
{
    ...
    PS_USIM_REFRESH_FILE_CONTENT_STRU    stRefreshFileContent;
    /* Modified by c00318887 for file refresh需要触发背景搜, 2015-4-28, end */

    VOS_UINT8                           ucBufferRefreshFlg;

    /* Deleted by c00318887 for file refresh需要触发背景搜, 2015-5-8, begin */
    /* Deleted by c00318887 for file refresh需要触发背景搜, 2015-5-8, end */
    ...
    /* Modified by c00318887 for file refresh需要触发背景搜, 2015-4-28, begin */
    PS_MEM_SET(&stRefreshFileContent, 0x0, sizeof(PS_USIM_REFRESH_FILE_CONTENT_STRU));
    NAS_MMC_BuildRefreshFileList(pstUsimRefreshInd, &stRefreshFileContent);
    /* Modified by c00318887 for file refresh需要触发背景搜, 2015-4-28, end */
    ...
    /* 一次遍历USIM上报的文件ID */
    /* Modified by c00318887 for file refresh需要触发背景搜, 2015-4-28, begin */
    for (i = 0; i < stRefreshFileContent.usEfNum; i++ )
    {
        usEfId = stRefreshFileContent.astRefreshFile[i].usFileId;

        if ((NAS_MML_READ_ACTING_HPLMN_FILE_ID == usEfId)
         && (USIMM_ATT_APP == stRefreshFileContent.astRefreshFile[i].enAppType))
        {
        ...
        if ( (NAS_MML_READ_CS_LOC_FILE_ID == usEfId)
           ||(NAS_MML_READ_PS_LOC_FILE_ID == usEfId))
        {
            continue;
        }
        ...
        /* 调用API读取USIM文件 */
        if (USIMM_API_SUCCESS != NAS_USIMMAPI_GetCachedFile(usEfId, &ulEfLen, &pucEf, stRefreshFileContent.astRefreshFile[i].enAppType))
        {
            /*读当前文件失败，读取下一个文件*/
        ...
        /* Modified by c00318887 for file refresh需要触发背景搜, 2015-5-8, begin */
        if (VOS_TRUE == NAS_MMC_IsHighPrioPlmnFileRefreshed(usEfId))
        /* Modified by c00318887 for file refresh需要触发背景搜, 2015-5-8, end */
        {
...
)
{
    switch (pstLmmAttachCnf->ulReqType)
    {
        case MMC_LMM_ATT_REQ_TYPE_PS_ONLY:
            ...
            NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_PS, NAS_MMC_NORMAL_SERVICE);

            break;

        case MMC_LMM_ATT_REQ_TYPE_CS_ONLY:
             NAS_MMC_UpdateCsRegStateCsRegSucc();
            NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_CS, NAS_MMC_NORMAL_SERVICE);
        ...
        case MMC_LMM_ATT_REQ_TYPE_CS_PS:
            NAS_MMC_UpdateCsRegStateCsRegSucc();
            NAS_MMC_UpdatePsRegStatePsRegSucc();
            NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_CS_PS,NAS_MMC_NORMAL_SERVICE);
...
)
{
    ...
    NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_CS_PS,NAS_MMC_LIMITED_SERVICE);
}
/*****************************************************************************
...
)
{
    ...
    if ( MMC_LMM_CONNECTED_DATA == pstLmmStatusIndMsg->ulConnState )
    {
        ...
        NAS_MMC_SndMmaPsServiceConnStatusInd(VOS_TRUE);
    }
    else
    {
        NAS_MMC_SndMmaDataTranAttri(MMA_MMC_DATATRAN_ATTRI_LTE_IDLE);
        NAS_MMC_SndMmaPsServiceConnStatusInd(VOS_FALSE);
    }
...
)
{
    ...
    MMA_MMC_LMM_CELL_SIGN_INFO_STRU     stLteCellSignInfo;
    MMA_MMC_SCELL_MEAS_REPORT_TYPE_UN   unMeasReportType;

    PS_MEM_SET(((VOS_UINT8*)&unMeasReportType), 0, sizeof(MMA_MMC_SCELL_MEAS_REPORT_TYPE_UN));
    unMeasReportType.ucMeasReportType |= MMA_MMC_SCELL_MEAS_TYPE_RXLEV;

    PS_MEM_SET(((VOS_UINT8*)&stLteCellSignInfo), 0, sizeof(MMA_MMC_LMM_CELL_SIGN_INFO_STRU));
    ...
    NAS_MMC_SndMmaMsgLmmCellSignInfoInd(&unMeasReportType, &stLteCellSignInfo);
...
)
{
    ...
    if  ((MM_MMC_LU_RESULT_SUCCESS       == pstCsRegRslt->enLuResult)
      && (NAS_MML_REG_FAIL_CAUSE_HO_SUCC == pstCsRegRslt->enRegFailCause))
    {
...
)
{
    ...
    if ((GMM_MMC_ACTION_RESULT_SUCCESS  == pstPsRegRslt->enActionResult)
     && (NAS_MML_REG_FAIL_CAUSE_HO_SUCC == pstPsRegRslt->enRegFailCause))
    {
...
)
{
    ...
    {
        NAS_MMC_SndMmaSysInfo_BgPlmnSearch();
    }
...
)
{
    ...
    pstDisabledPlmnInfo               = NAS_MML_GetDisabledRatPlmnCfg();
            ...
            if (ulIndex < pstDisabledPlmnInfo->ulDisabledRatPlmnNum - 1)
            {
                PS_MEM_MOVE(&(pstDisabledPlmnInfo->astDisabledRatPlmnId[ulIndex]),
                            &(pstDisabledPlmnInfo->astDisabledRatPlmnId[ulIndex+1]),
                             (pstDisabledPlmnInfo->ulDisabledRatPlmnNum-ulIndex-1)*sizeof(NAS_MML_DISABLED_PLMN_WITH_VALID_PERIOD_STRU));
            }
    ...
    {
        if (NAS_MML_MAX_TIME_SLICE != pstDisabledPlmnInfo->astDisabledRatPlmnId[ulIndex].ulPlmnDisabledExpiredSlice)
        {
            NAS_MMC_StartTimer(TI_NAS_MMC_PERIOD_DELETE_DISABLED_PLMN_WITH_RAT_TIMER,
    ...
    if ((VOS_TRUE == ulIsNeedDeleteDisabledPlmnWithRat)
...
)
{
    ...
    for (i = 0; i < pstEutanNotAllowNotifyMsg->ucPlmnNum; i++)
    {
        ...
        NAS_MML_AddDisabledRatPlmnWithValidPeriodList(&stPlmnWithRat, 
    ...
    if (VOS_TRUE == NAS_MMC_IsEutranNotAllowedNeedDisableL())
    {
        ...
        /* 启动ENABLE 定时器 */
        NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_ENABLE_LTE_TIMER, pstEutanNotAllowNotifyMsg->ulTimerLen * NAS_MML_ONE_THOUSAND_MILLISECOND);
    }
...
VOS_UINT32 NAS_MMC_RcvMmaAutoReselSet_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MMA_MMC_AUTO_RESEL_SET_REQ_STRU    *pstRcvMsg   = VOS_NULL_PTR;

    pstRcvMsg   = (MMA_MMC_AUTO_RESEL_SET_REQ_STRU *)pstMsg;
