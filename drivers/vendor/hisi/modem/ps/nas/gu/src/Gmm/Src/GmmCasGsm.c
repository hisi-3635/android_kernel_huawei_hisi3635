...
VOS_VOID GMM_RcvGasSysInfoInd_SuspWaitSys(VOS_VOID *pRcvMsg)
{
    ...
    {                                                                           /* 原来SIM无效                              */
        ...
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_NORMAL, "GMM_RcvGasSysInfoInd_SuspWaitSys:NORMAL: STATUS IS GMM_DEREGISTERED_NO_IMSI");
        return;
    }
    if (VOS_FALSE == NAS_MML_GetPsAttachAllowFlg())
    {
        g_GmmGlobalCtrl.ucCvrgAreaLostFlg = GMM_FALSE;                          /* 清除出覆盖区标志                         */
        Gmm_ComStaChg(GMM_DEREGISTERED_NORMAL_SERVICE);                         /* 调用状态的公共处理                       */
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "GMM_RcvGasSysInfoInd_SuspWaitSys:WARNING: Not allow Attach");
...
VOS_UINT32 GMM_RcvGasTdsRadioAccessCapa(
    VOS_VOID                           *pstRcvMsg
)
{
    VOS_UINT8                                               ucLen;
...
)
{
    ...
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "NAS_GMM_SaveGsmSysInfo:WARNING: Mem alloc fail.");
        return;
    }
