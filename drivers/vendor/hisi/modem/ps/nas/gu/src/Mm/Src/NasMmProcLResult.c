...
)
{
    ...
    - if the update status is U1 UPDATED and the tracking area updating attempt counter is less than 5, then the UE
    shall keep the update status to U1 UPDATED, the new MM state is MM IDLE substate NORMAL SERVICE;
    - if the tracking area updating attempt counter is less than 5 and, additionally, the update status is different from
    U1 UPDATED UE shall delete any LAI, TMSI, ciphering key sequence number and list of equivalent PLMNs
...
#if (FEATURE_ON == FEATURE_PTM)
    /* 记录异常状态为CSFB Paging已经接收 */
    NAS_MML_SetErrLogCsfbMtState(NAS_ERR_LOG_CSFB_MT_STATE_PAGING_RECEIVED);
#endif
