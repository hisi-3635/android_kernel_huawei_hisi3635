...
VOS_UINT32  NAS_GMM_GetRoutingAreaUpdateRequestMsgIELength( VOS_VOID )
{
    ...
    /* 除 MS Radio Access capability IE外的固定IE 长度 */
    ulMsgLen = 9;

    pucRadioCpb = NAS_GMM_GetUeIdRadioCpbAddr();
    cVersion = NAS_Common_Get_Supported_3GPP_Version(MM_COM_SRVDOMAIN_PS);

    /* 固定IE: MS Radio Access capability IE */
    ulMsgLen += pucRadioCpb[0] + 1;
    ...
    if(PS_PTL_VER_PRE_R99 != cVersion)
    {
        ...
        /* 可选IE: MS Network Capability IE */
        ulMsgLen += pstMsCapability->stMsNetworkCapability.ucNetworkCapabilityLen + 2;
...
#if (FEATURE_ON == FEATURE_LTE)
    VOS_UINT32                          ulIsSupportedLTE;

    VOS_UINT32                          ulIsLteCapabilityDisabled;

    ulIsSupportedLTE = NAS_MML_IsNetRatSupported(NAS_MML_NET_RAT_TYPE_LTE);
#endif
...
NAS_MSG_STRU *Gmm_RoutingAreaUpdateCompleteMsgMake(VOS_VOID)
{
    ...
    if ((VOS_TRUE == g_GmmInterRatInfoCtrl.ucEutranInterRatInfoRequestedFlag)
     && (0 != ucEUeCapDataLen))
    {
        pRauComplete->ulNasMsgSize             += 2 + ucEUeCapDataLen;
        pRauComplete->aucNasMsg[usIeOffset++]   = GMM_IEI_E_UTRAN_INTER_RAT_HANDOVER_INFORMATION;
        pRauComplete->aucNasMsg[usIeOffset++]   = ucEUeCapDataLen;
        Gmm_MemCpy(&(pRauComplete->aucNasMsg[usIeOffset]),
                   aucEUeCapData, ucEUeCapDataLen);
    }
    /*lint +e961*/
#endif
...
VOS_VOID Gmm_RoutingAreaUpdateRejectCause13(VOS_BOOL   bUpdateSimFlg)
{
    if (VOS_TRUE == bUpdateSimFlg)
    {
        ...
        Gmm_SndAgentUsimUpdateFileReq(GMM_USIM_FILE_PS_LOC_INFO);                   /* 通知AGENT更新PSlocation information      */
    }
                                        ...
                                        )
{
    ...
    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateRejectCause11:INFO: PLMN not allowed");
    ...
    *pucAttachFlg = GMM_TRUE;
    ...
    else
    {
        ...
        if ( NAS_MML_TIN_TYPE_GUTI == NAS_MML_GetTinType())
        {
            ...
            Gmm_SndAgentUsimUpdateFileReq(GMM_USIM_FILE_PS_KEY);
                                       ...
                                       )
{
    ...
    {                                                                           /* 消息中含有OP项                           */
        ...
        {                                                                       /* 消息中含有T3302 value                    */
            ...
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateRejectMsg:WARNING: T3302 of RAU REJECT is omitted");
            }
            else
            {
                if (1 != pMsg->aucNasMsg[GMM_MSG_LEN_RAU_REJECT + 1])
                {
                    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateRejectMsg:WARNING: RAU REJECT is semantically incorrect(IE:T3302)");
...
#endif
    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateRejectMsg:WARNING: GMM cause is Abnormal");
...
VOS_VOID Gmm_RcvRoutingAreaUpdateRejectMsg_AttachFlg_Handling(VOS_VOID)
{
    ...
    {                                                                           /* 不是用户模式A且网络模式I                 */
        ...
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RcvRoutingAreaUpdateRejectMsg_AttachFlg_Handling:INFO: normal attach procedure start");
    }
        ...
        && (GMM_FALSE == g_GmmGlobalCtrl.CsInfo.ucCsTransFlg))
    {
        ...
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RcvRoutingAreaUpdateRejectMsg_AttachFlg_Handling:INFO: combined attach procedure started");
    }
    else
    {                                                                           /* SIM无效，或者存在CS业务                                  */
        g_GmmGlobalCtrl.ucSpecProc = GMM_ATTACH_NORMAL;                         /* 存储当前进行的specific流程               */
...
)
{
    switch (enGmmRauType)
    {
        ...
        default:
            PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "NAS_GMM_ConvertGmmRauTypeToStkRauType:WARNING: The input in invalid");
...
TAF_PH_RAT_TYPE_ENUM_UINT8 NAS_GMM_ConvertGmmRatTypeToStkRatType (
...
)
{
    switch (enGmmRatType)
    {
        ...
        default:
            PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "NAS_GMM_ConvertGmmRatTypeToStkRatType:WARNING: The input in invalid");
...
VOS_VOID NAS_GMM_SndStkRauRej(VOS_UINT8 ucCause)
{
    ...
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL_PTR, LOG_LEVEL_ERROR, "NAS_GMM_ReportStkRauRej: Alloc Msg");
        return;
    }
    ...
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL_PTR, LOG_LEVEL_WARNING, "NAS_GMM_ReportStkRauRej: PS_SEND_MSG ERROR");
    }
...
)
{
    ...
    if (GMM_ROUTING_AREA_UPDATING_INITIATED != g_GmmGlobalCtrl.ucState)
    {
        ...
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateRejectMsg:WARNING: RAU REJECT is unexpected");
    ...
    if (GMM_MSG_LEN_RAU_REJECT > pMsg->ulNasMsgSize)
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateRejectMsg:WARNING: The length of RAU REJECT is invalid");
    ...
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateRejectMsg:WARNING: RAU REJECT is semantically incorrect (IE:Force to Standby)");
        gstGmmCasGlobalCtrl.ucFtsFlg = GMM_FALSE;
    }
    ...
    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RcvRoutingAreaUpdateRejectMsg:INFO: specific procedure ended");
                                         ...
                                         )
{
    ...
    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateAttemptCounter:INFO: specific procedure ended");
...
)
{
    ...
    if (GMM_RAU_ACCEPT_IE_GMM_CAUSE_FLG
        == (pRauAcceptIe->usOptionalIeMask & GMM_RAU_ACCEPT_IE_GMM_CAUSE_FLG))
    {
    ...
    {                                                                           /* combined流程                             */
            ...
            && (NAS_MML_REG_FAIL_CAUSE_NULL == enCause))
        {
            ...
            PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdataAcceptRaOnly:INFO: specific procedure ended");
            g_GmmRauCtrl.ucPeriodicRauFlg = GMM_FALSE;                          /* 清除当前RAU是否是周期RAU标志             */
            return;
        }
                                     ...
                                     )
{
    ...
    {                                                                           /* combined流程                             */
        ...
        g_GmmGlobalCtrl.ucSpecProc    = GMM_NULL_PROCEDURE;                     /* 清除当前进行的specific 流程标志          */
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdataAcceptCombined:INFO: specific procedure ended");
        g_GmmRauCtrl.ucPeriodicRauFlg = GMM_FALSE;                              /* 清除当前RAU是否是周期RAU标志             */
                                    ...
                                    )
{
    ...
    {                                                                           /* 存储IE的偏移量小于空口消息的长度         */
        ...
        case GMM_IEI_P_TMSI_SIGNATURE:                                          /* P-TMSI signature                         */
            ...
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"P-TMSI signature\" in RAU ACCEPT is invalid");
                return;
            }
        ...
        case GMM_IEI_ALLOCATED_PTMSI:                                           /* Allocated P-TMSI                         */
            ...
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"P-TMSI\" in RAU ACCEPT is invalid");
                return;
            }
            if (GMM_RAU_ACCEPT_IE_PTMSI_FLG ==
                (pRauAcceptIe->usOptionalIeMask & GMM_RAU_ACCEPT_IE_PTMSI_FLG))
            {
                ucIeOffset += 7;
                break;
            }
        ...
        case GMM_IEI_MS_IDENTITY:                                               /* MS identity                              */
            ...
            {                                                                   /* IE中是IMSI                               */
                if ((ucIeOffset + 10) > (VOS_UINT8)pMsg->ulNasMsgSize)
                {
                    ...
                    pRauAcceptIe->aucIeOffset[GMM_RAU_ACCEPT_IE_MS_IDENTITY]
                        = ucIeOffset;
                    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"MS ID(IMSI)\" in RAU ACCEPT is invalid");
                    return;
                }
            ...
            else
            {
                if ((ucIeOffset + 7) > (VOS_UINT8)pMsg->ulNasMsgSize)
                {
                    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"MS ID(TMSI)\" in RAU ACCEPT is invalid");
                    return;
                }
                if (GMM_RAU_ACCEPT_IE_MS_IDENTITY_FLG ==
                    (pRauAcceptIe->usOptionalIeMask
        ...
        case GMM_IEI_LIST_OF_RECEIVE_N_PDU_NUMBERS:                             /* Allocated P-TMSI                         */
            ...
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"N-PDU\" in RAU ACCEPT is invalid");
                return;
            }
        ...
        case GMM_IEI_NEGOTIATED_READY_TIMER_VALUE:                              /* Negotiated READY timervalue              */
            ...
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"READY TIMER value\" in RAU ACCEPT is invalid");
                return;
            }
        ...
        case GMM_IEI_GMM_CAUSE:                                                 /* GMM cause                                */
            if ((ucIeOffset + 2) > (VOS_UINT8)pMsg->ulNasMsgSize)
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"GMM cause\" in RAU ACCEPT is invalid");
                return;
            }
            if (GMM_RAU_ACCEPT_IE_GMM_CAUSE_FLG ==
                (pRauAcceptIe->usOptionalIeMask
        ...
        case GMM_IEI_T3302_VALUE:                                               /* T3302 value                              */
            ...
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"T3302 value\" in RAU ACCEPT is invalid");
                return;
            }
        ...
        case GMM_IEI_CELL_NOTIFICATION:                                         /* Cell Notification                        */
            ...
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"Cell notification\" in RAU ACCEPT is invalid");
                return;
            }
        ...
        case GMM_IEI_EQUIVALENT_PLMNS:                                          /* Equivalent PLMNs                         */
            if ((ucIeOffset + (2 + pMsg->aucNasMsg[ucIeOffset + 1]))
                 > (VOS_UINT8)pMsg->ulNasMsgSize)
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"Equivalent PLMNs\" in RAU ACCEPT is invalid");
                return;
            }
        ...
        case GMM_IEI_EMERGENCY_NUMBER_LIST:                                     /* Emergency Number List                    */
            if ( (ucIeOffset + (2 + pMsg->aucNasMsg[ucIeOffset + 1]))
                 > (VOS_UINT8)pMsg->ulNasMsgSize )
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateResolveIe:WARNING: IE\"emergency number\" length error in ATTACH ACCEPT");
                return;
            }
            pRauAcceptIe->usOptionalIeMask
            ...
            case NAS_GMM_IEI_T3323_VALUE:
                ...
                if (GMM_RAU_ACCEPT_IE_T3323_VALUE_FLG == (pRauAcceptIe->usOptionalIeMask & GMM_RAU_ACCEPT_IE_T3323_VALUE_FLG))
                {
                    ucIeOffset += 3;
                    break;
                }
                ...
                pRauAcceptIe->usOptionalIeMask |= GMM_RAU_ACCEPT_IE_T3323_VALUE_FLG;
        ...
        default:
            ...
            /* TLV类型的IE */
            /*lint -e961*/
            else if ((VOS_UINT32)(ucIeOffset + 2 + pMsg->aucNasMsg[ucIeOffset + 1]) < pMsg->ulNasMsgSize)
            /*lint +e961*/
            {
                ucIeOffset += (2 + pMsg->aucNasMsg[ucIeOffset + 1]);
            }
                                 ...
                                 )
{
    ...
    if (VOS_FALSE == bTlliUpdateFlag)
    {
        if (GMM_TRUE == gstGmmCasGlobalCtrl.ucRauCmpFlg)
        {
            ...
            PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateHandle_Under_Gsm:INFO: specific procedure ended");
    ...
    */
    ...
    else
    {
        ...
        g_GmmGlobalCtrl.ucSpecProc    = GMM_NULL_PROCEDURE;
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateHandle:INFO: specific procedure ended");
        g_GmmRauCtrl.ucPeriodicRauFlg = GMM_FALSE;
                                                    ...
                                                    NAS_MSG_FOR_PCLINT_STRU        *pMsg)
{
    ...
    if (GMM_RAU_ACCEPT_IE_T3302_VALUE_FLG
        == (pRauAcceptIe->usOptionalIeMask & GMM_RAU_ACCEPT_IE_T3302_VALUE_FLG))
    {                                                                           /* 有T3302IE                                */
        /* 24.008: 9.4.15.7
...
)
{
    ...
    if (GMM_RAU_ACCEPT_IE_EQUIVALENT_PLMN_FLG
        != (pstRauAcceptIe->usOptionalIeMask
        & GMM_RAU_ACCEPT_IE_EQUIVALENT_PLMN_FLG))
    {
...
{
    if (GMM_RAU_ACCEPT_IE_READY_TIMER_FLG
        == (pRauAcceptIe->usOptionalIeMask & GMM_RAU_ACCEPT_IE_READY_TIMER_FLG))
    {
...
)
{
    ...
    if (GMM_RAU_ACCEPT_IE_NPDU_NUMBER_FLG
        == (pRauAcceptIe->usOptionalIeMask & GMM_RAU_ACCEPT_IE_NPDU_NUMBER_FLG))
    {                                                                           /* 空口消息中有N-PDU Numbers                */
    ...
    if (GMM_RAU_ACCEPT_IE_PDP_CONTEXT_STATUS_FLG
        == (pRauAcceptIe->usOptionalIeMask & GMM_RAU_ACCEPT_IE_PDP_CONTEXT_STATUS_FLG))
    {
...
VOS_VOID Gmm_RoutingAreaUpdateHandleFollowOn(VOS_VOID)
{
    ...
    case GMM_SERVICE_REQUEST_SIGNALLING:
        if (GMM_TRUE == g_GmmServiceCtrl.ucSmsFlg)
        {
            ...
            else
            {
                ...
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_NORMAL, "Gmm_RoutingAreaUpdateHandleFollowOn:NORMAL: STATUS IS GMM_SERVICE_REQUEST_INITIATED");
                ...
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateHandleFollowOn:INFO: SR(signaling) procedure started");
    ...
    g_GmmGlobalCtrl.ucSpecProcHold = GMM_NULL_PROCEDURE;                        /* 清除保留的specific流程标志               */
    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateHandleFollowOn:INFO: specific procedure ended");
}
                                       ...
                                       )
{
    ...
    {                                                                           /* GMM状态不是GMM_RAU_INITIATED             */
        ...
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateAcceptMsg:WARNING: RAU ACCEPT is unexpected");
    ...
    {                                                                           /* 当前的specific过程不是RAU过程            */
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateAcceptMsg:WARNING: Current Specific Process is not RAU process");
        return;
    }
    else
    {
    ...
    {                                                                           /* 消息过短                                 */
        ...
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateAcceptMsg:WARNING: RAU ACCEPT is too short");
        pGmmStatus = Gmm_GmmStatusMsgMake(NAS_MML_REG_FAIL_CAUSE_INVALID_MANDATORY_INF);
    ...
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateAcceptMsg:WARNING: RAU ACCEPT is semantically incorrect");
        pGmmStatus =
    ...
    {                                                                           /* RAU ACCEPT中的RAI与系统信息的RAI不一致   */
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RcvRoutingAreaUpdateAcceptMsg:WARNING: RAI in RAU ACCEPT is different with current RAI");
        pGmmStatus =
            Gmm_GmmStatusMsgMake(NAS_MML_REG_FAIL_CAUSE_SEMANTICALLY_INCORRECT_MSG);
                                 ...
                                 )
{
    ...
    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_NORMAL, "Gmm_SndRoutingAreaUpdateReq:NORMAL: STATUS IS GMM_ROUTING_AREA_UPDATING_INITIATED");
    ...
    {                                                                           /* CS域SIM有效                              */
        ...
        {                                                                       /* CS域已经注册                             */
            ...
            {                                                                   /* CS域在通信中                             */
                ...
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_PeriodRoutingAreaUpdateType:INFO: normal RAU procedure during CS transaction started");
            ...
            else
            {
                ...
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_PeriodRoutingAreaUpdateType:INFO: combined RAU procedure started");
            }
        }
        else
        {                                                                       /* CS域没注册                               */
            ...
            {                                                                   /* CS域ATTACH被禁止                         */
                ...
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_PeriodRoutingAreaUpdateType:INFO: normal RAU procedure started");
            }
            else
            {
                g_GmmGlobalCtrl.ucSpecProc = GMM_RAU_WITH_IMSI_ATTACH;          /* 存储当前进行的specific流程               */
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_PeriodRoutingAreaUpdateType:INFO: combined RAU with IMSI attach procedure started");
            }
        }
    }
    else
                                   ...
                                   )
{
    ...
    {                                                                           /* PS域没有注册                             */
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateInitiate:WARNING: PS domain hasn't register");
        return;
    }
    ...
    {                                                                           /* 输入参数有效                             */
        ...
        {                                                                       /* 周期性的RAU                              */
            ...
            {                                                                   /* 不是用户模式A且网络模式I                 */
                g_GmmGlobalCtrl.ucSpecProc = GMM_RAU_NORMAL;                    /* 存储当前进行的specific流程               */
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateInitiate:INFO: normal RAU procedure started");
            }
            else
            {
        ...
        else
        {
            ...
            case GMM_COMBINED_RALA_UPDATING:
                ...
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateInitiate:INFO: combined RAU procedure started");
                break;
            case GMM_COMBINED_RALAU_WITH_IMSI_ATTACH:
                g_GmmGlobalCtrl.ucSpecProc = GMM_RAU_WITH_IMSI_ATTACH;
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateInitiate:INFO: combined RAU with IMSI attach procedure started");
                break;
            default:
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateInitiate:WARNING: Update Type is Abnormal");
                break;
            }
        }
    ...
    {                                                                           /* CS域SIM有效                              */
        ...
        {                                                                       /* CS域已经注册                             */
            ...
            {                                                                   /* CS域在通信中                             */
                g_GmmGlobalCtrl.ucSpecProc = GMM_RAU_NORMAL_CS_TRANS;           /* 存储当前进行的specific流程               */
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateInitiate:INFO: normal RAU procedure during CS transaction started");
                g_GmmGlobalCtrl.ucSpecProcInCsTrans = GMM_NULL_PROCEDURE;       /* 清除CS通信中的流程                       */
                                                  ...
                                                  VOS_UINT8    *pNeedReturn)
{
    ...
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RauAccept_Mandatory_Ie_Check:ERROR: Null Pointer");
        return GMM_FAILURE;
    }
    ...
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RauAccept_Mandatory_Ie_Check:WARNING: IE\"Force to standby\" is invalid in RAU ACCEPT");
        gstGmmCasGlobalCtrl.ucFtsFlg = GMM_FALSE;
    }
    ...
    if ((GMM_RA_UPDATED != ucUpdateResultValue)
     ...
     && (GMM_COMBINED_RALA_UPDATED_ISR_ACTIVE != ucUpdateResultValue))
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RauAccept_Mandatory_Ie_Check:WARNING: IE\"Update result\" is invalid in RAU ACCEPT");
        return GMM_FAILURE;
    }
                                                  ...
                                                  VOS_UINT8    *pNeedReturn)
{
    if ((VOS_NULL == pMsg) || (VOS_NULL == pNeedReturn))
    {
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_IeCheck_Npdu_Numbers:ERROR: Null Pointer");
        return GMM_FAILURE;
    }
...
VOS_UINT8 Gmm_IECheck_RauAccept(NAS_MSG_FOR_PCLINT_STRU *pMsg)
{
    ...
    {                                                                           /* 存储IE的偏移量小于空口消息的长度         */
        ...
        case GMM_IEI_EMERGENCY_NUMBER_LIST:                                     /* Emergency Number List                    */
            /*lint -e961*/
            if ((VOS_UINT32)(ucIeOffset + 2 + pMsg->aucNasMsg[ucIeOffset + 1]) <= pMsg->ulNasMsgSize)
            /*lint +e961*/    
            {
                ucIeOffset += (2 + pMsg->aucNasMsg[ucIeOffset + 1]);
            }
...
VOS_VOID Gmm_RoutingAreaUpdateInitiateWithReEstRR()
{
    ...
    if ((GMM_REGISTERED_INITIATED == g_GmmGlobalCtrl.ucState)
        ...
        || (GMM_NULL == g_GmmGlobalCtrl.ucState))
    {                                                                           /* PS域没有注册                             */
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_WARNING, "Gmm_RoutingAreaUpdateInitiateWithReEstRR:WARNING: PS domain hasn't register");
        return;
    }
    ...
    {                                                                           /* 不是用户模式A且网络模式I                 */
        g_GmmGlobalCtrl.ucSpecProc = GMM_RAU_NORMAL;                            /* 存储当前进行的specific流程               */
        PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateInitiateWithReEstRR:INFO: normal RAU procedure started");
    ...
    {                                                                           /* CS域SIM有效                              */
        ...
        {                                                                       /* CS域已经注册                             */
            ...
            {                                                                   /* CS域在通信中                             */
                g_GmmGlobalCtrl.ucSpecProc = GMM_RAU_NORMAL_CS_TRANS;           /* 存储当前进行的specific流程               */
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateInitiateWithReEstRR:INFO: normal RAU procedure during CS transaction started");
                g_GmmGlobalCtrl.ucSpecProcInCsTrans = GMM_NULL_PROCEDURE;       /* 清除CS通信中的流程                       */
                Gmm_SndRoutingAreaUpdateReqWithReEstRR(GMM_RA_UPDATING);        /* 调用函数发送RAU REQUEST                  */
            }
            else
            {
                PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateInitiateWithReEstRR:INFO: combined RAU procedure started");
        ...
        {                                                                       /* CS域没注册                               */
            ...
            {                                                                   /* CS域ATTACH没被禁止                       */
                ...
                {
                    g_GmmGlobalCtrl.ucSpecProc = GMM_RAU_WITH_IMSI_ATTACH;      /* 存储当前进行的specific流程               */
                    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_INFO, "Gmm_RoutingAreaUpdateInitiateWithReEstRR:INFO: combined RAU with IMSI attach procedure started");
                                 ...
                                 )
{
    ...
    PS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_PRINT_NORMAL, "Gmm_SndRoutingAreaUpdateReqWithReEstRR:NORMAL: STATUS IS GMM_ROUTING_AREA_UPDATING_INITIATED");
