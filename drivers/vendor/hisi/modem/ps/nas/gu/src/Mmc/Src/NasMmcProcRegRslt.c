...
NAS_MMC_REG_FAIL_PROC_ENTITY             gastMmcProcCsRegFailCause[] =
{
    ...
    {NAS_MML_REG_FAIL_CAUSE_HO_SUCC,                                  NAS_MMC_ProcCsRegFailCause419HoSucc},
};
...
NAS_MMC_REG_FAIL_PROC_ENTITY             gastMmcProcPsRegFailCause[] =
{
    ...
    {NAS_MML_REG_FAIL_CAUSE_HO_SUCC,                                  NAS_MMC_ProcPsRegFailCause419HoSucc},
};
#if   (FEATURE_ON == FEATURE_LTE)
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcCsRegFailCause12LaNotAllow(VOS_VOID)
{
    ...
    NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_CS, NAS_MMC_LIMITED_SERVICE_REGION);
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcCsRegFailCause13RoamNotAllow(VOS_VOID)
{
    ...
    /* 上报对应的CS服务状态MMA_MMC_SERVICE_STATUS_NO_SERVICE */
    NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_CS, NAS_MMC_LIMITED_SERVICE);
    NAS_MMC_ChangeCsRegState(NAS_MML_REG_NOT_REGISTERED_SEARCHING);
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcCsRegFailCause419HoSucc(VOS_VOID)
{
    ...
    /* 通知TAF当前的服务状态 */
    NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_CS, NAS_MMC_GetCsServiceStatus());
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcPsRegFailCause12LaNotAllow(VOS_VOID)
{
    ...
    /* 更新服务状态 */
    NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_PS, NAS_MMC_LIMITED_SERVICE_REGION);
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcPsRegFailCause419HoSucc(VOS_VOID)
{
    NAS_MML_CAMP_PLMN_INFO_STRU        *pstCampPlmnInfo     = VOS_NULL_PTR;
    NAS_MML_NET_RAT_TYPE_ENUM_UINT8     enRat;
    ...
    /* 通知TAF当前的服务状态 */
    NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_PS, NAS_MMC_GetPsServiceStatus());
...
)
{
    ...
    *penCsRegAdditionalAction = NAS_MMC_ADDITIONAL_ACTION_CELL_SELECTION;
    ...
    /* Add by l00305157 for Service_State_Optimize_PhaseII, 2015-02-11, begin */
    /*mmc收到COMBINED ATU錜EJ(#12),上报注册状态和服务状态*/
    NAS_MMC_ChangeCsRegState(NAS_MML_REG_NOT_REGISTERED_NOT_SEARCH);
    NAS_MMC_ChangePsRegState(NAS_MML_REG_NOT_REGISTERED_NOT_SEARCH);
    NAS_MMC_ChangeServiceState(MMA_MMC_SRVDOMAIN_CS_PS,NAS_MMC_LIMITED_SERVICE_REGION);
    /* Add by l00305157 for Service_State_Optimize_PhaseII, 2015-02-11, end */
...
)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedDisableL_LteRejCause14())
    {
        ...
        NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_ENABLE_LTE_TIMER, NAS_MML_GetLteRejCause14EnableLteTimerLen());
...
)
{
    ...
    else
    {
        ...
        if ( VOS_TRUE == NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes() )
        {
            ...
            if (VOS_TRUE == NAS_MML_IsNeedAddPlmnInDisabledPlmnWithValidPeriodList(&stPlmnWithRat, MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE))
            {
                NAS_MML_UpdateDisabledPlmnWithValidPeriodList(&stPlmnWithRat,
                                                              NAS_MML_GetDisabledPlmnWithValidPeriodTimeSlice(&stPlmnWithRat),
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcEpsServiceRequestFailCause18CsDomainNotAvailable(VOS_VOID)
{
    ...
    /* Deleted by l00305157 for Service_State_Optimize_PhaseII, 2015-02-11, begin */
    /* LMM Service Reject结果上报(cause=#3)*/
...
)
{
    ...
    else
    {
        ...
        *penCsAddition = NAS_MMC_ADDITIONAL_ACTION_NORMAL_CAMP_ON;
         ...
         /* Add by l00305157 for Service_State_Optimize_PhaseII, 2015-05-11, begin */
         NAS_MMC_UpdatePsRegStatePsRegSucc();
         /* Add by l00305157 for Service_State_Optimize_PhaseII, 2015-05-11, end */
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_RegFailCause22AttmptCntMaxTimes(VOS_VOID)
{
    ...
    /* ATT的卡如果注册被拒#22达最大次数的网络在支持DAM特性网络列表中则disable lte，否则无需disable lte，
       非ATT的卡注册被拒#22达最大次数需要disable lte，R11协议升级对这块有修改被拒#22达5次无需disable lte
       暂不调整因为没有协议版本nv控制*/
...
{
    NAS_MML_LTE_UE_OPERATION_MODE_ENUM_UINT8                enLUeMode;
    VOS_UINT32                                              ulCsfbServiceStatus;

    ulCsfbServiceStatus   = NAS_MML_IsCsfbServiceStatusExist();
    enLUeMode             = NAS_MML_GetLteUeOperationMode();
    ...
    /* PS业务service请求T3417超时达最大次数时需要disable lte,
       csfb service请求EXT T3417超时也会上报超时无需disable lte */
    if ((NAS_MML_REG_FAIL_CAUSE_TIMER_TIMEOUT == enRegRsltCause)
     && (VOS_FALSE == ulCsfbServiceStatus))
    {
        return VOS_TRUE;
    }
