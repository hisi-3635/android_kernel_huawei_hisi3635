...
)
{
    ...
    {
        /* 目前代码中成功的假流程可以填两种原因值: NAS_MML_REG_FAIL_CAUSE_LOCAL_NULL和NAS_MML_REG_FAIL_CAUSE_HO_SUCC */
        /* 除NAS_MML_REG_FAIL_CAUSE_HO_SUCC之外，其他原因值统一填为NAS_MML_REG_FAIL_CAUSE_LOCAL_NULL */
        if ( MM_MMC_LU_RESULT_SUCCESS == ulLuResult )
        {
            if (NAS_MML_REG_FAIL_CAUSE_HO_SUCC != ulCause)
            {
