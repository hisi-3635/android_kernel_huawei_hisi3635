...
VOS_VOID NAS_MMC_SaveCardStatus_SwitchOn(MMA_MMC_CARD_STATUS_ENUM_UINT8 enCardStatus)
{
    NAS_MML_SIM_STATUS_STRU            *pstSimStatus;

    pstSimStatus    = NAS_MML_GetSimStatus();

    if ( MMA_MMC_CARD_STATUS_SIM_PRESENT == enCardStatus )
    {
        pstSimStatus->enSimType = NAS_MML_SIM_TYPE_SIM;

        NAS_MML_SetSimPresentStatus(VOS_TRUE);
    }
    else if ( MMA_MMC_CARD_STATUS_USIM_PRESENT == enCardStatus)
    {
...
{
    MMA_MMC_START_REQ_STRU              *pstStartReq     = VOS_NULL_PTR;
    ...
    /* 保存SIM卡状态信息 */
    NAS_MMC_SaveCardStatus_SwitchOn(pstStartReq->enCardStatus);
    /* 开机初始化EHPLMN 信息，开机会从9102NV/6FD9卡文件/4F34卡文件中获取用户配置的EHPLMN */
    NAS_MML_InitSimEhPlmnInfo(NAS_MML_GetSimEhplmnList());
    ...
    /* 在DSDS宏开启的时候，需要判断是否需要通知RRM模块进行开机注册消息 */
    ...
    if ( VOS_TRUE == NAS_MML_GetSimPresentStatus())
    {
        ...
        NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_READ_SIM_FILES,TI_NAS_MMC_WAIT_READ_SIM_FILES_LEN);
    }
    /* SIM卡不存在 */
    else
    {
        ...
        NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_MM_START_CNF,TI_NAS_MMC_WAIT_MM_START_CNF_LEN);
    }
...
)
{
    ...
    NAS_MMC_ClearWaitSimFilesCnfFlg_SwitchOn_WaitSimFilesCnf(pstRcvMsg->usEfId);
    ...
    /* 删除FPLMN中的无效PLMN */
    NAS_MML_DelInvalidPlmnFromList(pstForbidPlmnInfo->ucForbPlmnNum, pstForbidPlmnInfo->astForbPlmnIdList);
...
)
{
    ...
    /* 开机顺序设置完成后，对异常情况进行判断 */
    /* 异常情况：下发开机的接入层个数为0，打印异常，给MMA回复开机失败，退状态机 */
    if (0 == pstSwitchOnSettingRecord->ucTotalSwitchOnRatNum)
    {
        ...
        /* 给TAF发送开机失败 */
        NAS_Mmc_SndMmaStartCnf(TAF_FAILURE);
...
)
{
    ...
    vos_printf("NAS_MMC_RcvGasStartCnf_SwitchOn_WaitGasStartCnf.ulResult = %d.\n ", pstRrStartCnfMsg->ulResult);
    ...
    NAS_Mmc_SndMmaStartCnf(TAF_SUCCESS);
...
)
{
    ...
    vos_printf("NAS_MMC_RcvWasStartCnf_SwitchOn_WaitWasStartCnf.ulResult = %d.tick=0x%x\n ", pstRrStartCnfMsg->ulResult,OM_GetSlice());
    ...
    /* 开机完成，向TAF发送成功的TAFMMC_START_CNF */
    NAS_Mmc_SndMmaStartCnf(MMC_START_SUCCESS);
...
)
{
    ...
    vos_printf("NAS_MMC_RcvLmmStartCnf_SwitchOn_WaitLStartCnf.ulResult = %d,tick=0x%x\r\n ", pstRrStartCnfMsg->ulRst,OM_GetSlice());
...
)
{
    switch (enRegDomain)
    {
        case MMA_MMC_ALLOWED_REG_DOMAIN_PS:
            NAS_MML_SetPsAttachAllowFlg(VOS_TRUE);
            break;

        case MMA_MMC_ALLOWED_REG_DOMAIN_CS:
            NAS_MML_SetCsAttachAllowFlg(VOS_TRUE);
            break;

        case MMA_MMC_ALLOWED_REG_DOMAIN_CS_PS:
            NAS_MML_SetCsAttachAllowFlg(VOS_TRUE);
            NAS_MML_SetPsAttachAllowFlg(VOS_TRUE);
            break;
...
)
{
    ...
    switch ( enCurrSwitchOnRat )
    {
        ...
        case NAS_MML_NET_RAT_TYPE_LTE:
            ...
            /* 启动等待回复定时器 */
            NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_LMM_START_CNF, TI_NAS_MMC_WAIT_LMM_START_CNF_LEN);
            break;
#endif
        ...
        case NAS_MML_NET_RAT_TYPE_WCDMA:
            ...
            /* 启动等待回复定时器 */
            NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_WAS_START_CNF, TI_NAS_MMC_WAIT_WAS_START_CNF_LEN);
            break;
        ...
        case NAS_MML_NET_RAT_TYPE_GSM:
            ...
            /* 启动等待回复定时器 */
            NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_GAS_START_CNF, TI_NAS_MMC_WAIT_GAS_START_CNF_LEN);
            break;
...
VOS_VOID NAS_MMC_ProcRrmRegister_SwitchOn(VOS_VOID)
{
    ...
    /* 如果当前NV未开启DSDS,则不向RRM发送开机注册 */
    if ( VOS_FALSE == NAS_MML_GetDsdsRfShareFlg() )
    {
        return;
    }

    /* 在MODEM0时候，向RRM发送W下的注册信息 */
    if ( MODEM_ID_0 == usModemId )
    {
        NAS_MMC_SndRrmRegisterInd(MODEM_ID_0,
    ...
    /* 在MODEM1时候，向RRM发送G下的注册信息 */
    if ( MODEM_ID_1 == usModemId )
    {
        NAS_MMC_SndRrmRegisterInd(MODEM_ID_1,
                                  RRM_PS_TASK_TYPE_NAS_SEARCH,
                                  RRM_PS_RAT_TYPE_GSM);
    }
