...
VOS_VOID MM_SaveMsg(const VOS_VOID* pMsg, VOS_UINT8 ucEventId, VOS_UINT32 ulLength)
{
    ...
    NAS_LOG1(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_NORMAL,
                    "STATE MM_SUSPEND_STATE RECEIVE Msg Id",ucEventId);

    /* 消息长度异常，丢弃此条消息 */
    if (ulLength > MM_MAX_MSG_LENGTH)
    {
        NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_ERROR,
...
VOS_VOID MM_RetransferMsg()
{
    ...
    for (i = 0; i < NAS_RRC_MAX_RE_TX_MSG_NUM; i ++)
    {
        ...
        {
            NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_INFO,
        ...
        else if (NAS_RRC_RE_TX_EST_REQ_MSG == g_MmGlobalInfo.stSuspendShare.astReTxMsg[i].enReTxMsgType)
        {
            ...
            case WAIT_FOR_RR_CONNECTION_LOCATION_UPDATING :
                ...
                if (VOS_OK != Result)
                {
                    PS_LOG(WUEPS_PID_MM, VOS_NULL, PS_PRINT_ERROR, "MM_RetransferMsg: Failed RrEstReqFunc.");
                }
        ...
        else
        {
            g_MmGlobalInfo.stSuspendShare.astReTxMsg[i].enReTxMsgType
                    = NAS_RRC_NULL_RE_TX_MSG;
            NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_WARNING,
...
VOS_VOID MM_Restore()
{
    ...
    for ( ; i > MM_CONST_NUM_0; i --)
    {
        ...
        {
            NAS_LOG1(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_INFO,
...
VOS_VOID MM_SuspendTimeout()
{
    ...
    if (MM_SUSPEND_STATE == g_MmGlobalInfo.stSuspendShare.ucSuspendFlg)
    {
        ...
        {
            gstMmTimerSuspend.ucTimerStatus = MM_TIMER_STOPPED;
            NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_ERROR,"restart MM_TIMER_PROTECT_SUSPEND fail");
        }
        else
        {
            NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_INFO,"restart MM_TIMER_PROTECT_SUSPEND success");
        }
    }
    else /* 非挂起状态，恢复缓存队列消息 */
    {
...
VOS_VOID Mm_Cell_S3_E84( VOS_VOID* pMsg )
{
    ...
    {
        gstMmTimerSuspend.ucTimerStatus = MM_TIMER_STOPPED;
        NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_ERROR,"start MM_TIMER_PROTECT_SUSPEND fail");
    }
    else
    {
        gstMmTimerSuspend.ucTimerStatus = MM_TIMER_START;
        NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_INFO,"start MM_TIMER_PROTECT_SUSPEND success");
    }
...
)
{
    ...
    else if (TEST_CONTROL_ACTIVE !=  g_MmGlobalInfo.ucSuspendPreState)
    {
        ...
        Mm_ComSetMmState(g_MmGlobalInfo.ucSuspendPreState);
        PS_LOG1(WUEPS_PID_MM, VOS_NULL, PS_PRINT_NORMAL, "NAS_MM_ResumeSuspendInfo:NORMAL: ucState = ", g_MmGlobalInfo.ucState);
    }
    else
    {
        g_MmGlobalInfo.ucPreState = g_MmGlobalInfo.ucState;
    ...
    if ( MM_TIMER_START == gstMmTimerSuspend.ucTimerStatus )
    {
        ...
        {
            NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_ERROR,"Stop MM_TIMER_PROTECT_SUSPEND fail.");
        }
        else
        {
            NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_INFO,"Stop MM_TIMER_PROTECT_SUSPEND success");
        }
        gstMmTimerSuspend.ucTimerStatus = MM_TIMER_STOPPED;
    }
...
)
{
      ...
      && (g_MmGlobalInfo.ucCsSigConnFlg != MM_CS_SIG_CONN_PRESENT)))
    {
        ...
        {
            /* 内存申请失败                             */
             PS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_ERROR, "NAS_MM_ProcResumeIndInterRatChangedToGU:ERROR: MALLOC RRMM_EST_CNF_STRU ERROR!");
             return;
        }
...
)
{
    ...
    {
        if ( (VOS_TRUE == NAS_MML_IsCsfbServiceStatusExist())
          && (NAS_MML_CSFB_SERVICE_STATUS_BUTT != enCsfbStatus) )
        {
...
VOS_VOID Mm_Cell_S32_E85( VOS_VOID* pMsg )
{
    ...
    switch ( g_MmGlobalInfo.stSuspendShare.ucSuspendCause )
    {
        ...
        case MMC_SUSPEND_CAUSE_PLMNSEARCH:
            ...
            {
                NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_NORMAL,"MM:HANDOVER Resume Failure.");
            ...
            {
                NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_NORMAL,"Resume Success");
        ...
        default:
            NAS_LOG(WUEPS_PID_MM, MM_SUSPEND, PS_PRINT_WARNING,"SUSPEND cause is error");
            break;
    }
