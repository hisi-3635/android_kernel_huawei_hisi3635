                      ...
                      )
{
    ...
    if(SMC_MT_WAIT_FOR_RP_ACK == g_SmcPsEnt.SmcMt.ucState)
    {
        ...
        if (NAS_GMM_NET_RAT_TYPE_LTE == GMM_GetCurNetwork())
        {
            ...
            /* 启动timer tc1m */
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_SmrApiDataReq: TimerStart: tc1m" );
            ...
            g_SmcPsEnt.SmcMt.ucState                 = SMC_MT_WAIT_FOR_CP_ACK;  /* 状态迁移                                 */
    ...
    {                                                                           /* CS域的实体在等待RP-ACK                   */
        {                                                                       /* Message Reference相等                    */
           ...
           TempAddr = (VOS_UINT32)(&(((MMSMS_DATA_REQ_STRU *)aucSndMsg)->SmsMsg.aucNasMsg[2]));
            ...
            g_SmcCsEnt.SmcMt.ucState =  SMC_MT_WAIT_FOR_CP_ACK;
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_SmrApiDataReq:NORMAL:SMS state = SMC_MT_WAIT_FOR_CP_ACK");
        }
    }
    else if(SMR_WAIT_FOR_RP_ACK == g_SmrEnt.SmrMo.ucState)
    {
        ...
        else
        {
            ...
            if (NAS_GMM_NET_RAT_TYPE_LTE == GMM_GetCurNetwork())
            {
                ...
                /* 发送CP-DATA消息 */
                NAS_SMS_SndLmmCpDataReq(aucSndMsg, usMsgLen);
            }
            else
#endif
            {
    ...
    {
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_ERROR, "SMC_SmrApiDataReq:ERROR:Invalid SMS-R state ");
    }
}
...
)
{
    ...
    SMS_LOG(PS_PRINT_NORMAL, "SMC_SmrApiEstReq: step into function.");
    SMS_LOG1(PS_PRINT_NORMAL, "SMC_SmrApiEstReq: ucCurSendDomain.", ucCurSendDomain);
    /*****************************进行实际的发送******************************/
    ...
    SMS_LOG1(PS_PRINT_NORMAL, "SMC_SmrApiEstReq: g_SmcCsEnt.SmcMo.ucState.", g_SmcCsEnt.SmcMo.ucState);
    SMS_LOG1(PS_PRINT_NORMAL, "SMC_SmrApiEstReq: g_SmcPsEnt.SmcMo.ucState.", g_SmcPsEnt.SmcMo.ucState);
    ...
    {   /*如果是在PS域发送*/
        if (SMC_MO_WAIT_TO_SND_CP_ACK == g_SmcCsEnt.SmcMo.ucState)
        {
            ...
            SMS_LOG(PS_PRINT_NORMAL, "SMC_SmrApiEstReq: SMS state = SMC_MO_IDLE");
            g_SmcCsEnt.SmcMo.ucState = SMC_MO_IDLE;
            SMS_SendMnMoLinkCloseInd();
        }

        SMS_LOG1(PS_PRINT_NORMAL,
        ...
        g_SmcPsEnt.SmcMo.ucState = SMC_MO_GMM_CONN_PENDING;                 /* 状态迁移                                 */
        SMS_LOG( PS_PRINT_NORMAL,
    ...
    {   /*如果是在CS域发送*/
        ...
        /* 状态迁移                                 */
        g_SmcCsEnt.SmcMo.ucState = SMC_MO_MM_CONN_PENDING;
        SMS_LOG( PS_PRINT_NORMAL,
                        ...
                        )
{
    ...
    {                                                                           /* 原因值为TR1M超时                         */
        if(SMC_MO_IDLE != g_SmcCsEnt.SmcMo.ucState)
        {
            ...
            else
            {
                ...
                /* 停止timer tc1m */
                SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_SmrApiAbortReq: TimerStop: TC1M" );
            ...
            g_SmcCsEnt.SmcMo.ucState = SMC_MO_IDLE;                             /* 状态迁移                                 */
        }
        else
        {                                                                       /* PS域的SMC实体状态为非空                  */
    ...
    {                                                                           /* 原因值为TR2M超时                         */
        ...
        {                                                                       /* CS域的SMC实体状态为SMC_MT_MM_CONN_EST    */
            ...
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_SmrApiAbortReq:NORMAL:SMS state = SMC_MT_IDLE");
            g_SmcCsEnt.SmcMt.ucState = SMC_MT_IDLE;                             /* 状态迁移到空闲                           */
        }
        else if(SMC_MT_WAIT_FOR_RP_ACK == g_SmcCsEnt.SmcMt.ucState)
        {
                      ...
                      )
{
    ...
    {                                                                           /* 释放mo过程中的mm连接                     */
        ...
        {                                                                       /* 实体在等待发送CP_ACK                     */
            ...
            /* 停止timer tc1m */
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_SmrApiRelReq: TimerStop: TC1M" );
            if ( SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_CS_TC1M_MO ) )
            {
                    SMS_LOG( PS_PRINT_NORMAL, "SMS:SMC_SmrApiRelReq: fail to stop TC1M." );
            }
        ...
        else if(SMC_MO_IDLE != g_SmcPsEnt.SmcMo.ucState)
        {
            ...
            else
            {
                ...
                PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_SmrApiRelReq:NORMAL:SMS state = SMC_MO_IDLE");
                g_SmcPsEnt.SmcMo.ucState = SMC_MO_IDLE;                         /* 状态转移                                 */
            }
        }
        else
        {
    ...
    else
    {
        ...
        {                                                                       /* 实体在等待高层的确认                     */
            ...
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMC_SmrApiRelReq:NORMAL:SMS state = SMC_MT_IDLE");
            g_SmcCsEnt.SmcMt.ucState = SMC_MT_IDLE;                             /* 状态转移                                 */
        }
                      ...
                      )
{
    ...
    {                                                                       /* 不存在强制IE                             */
        ...
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_WARNING, "SMR_SmcApiDataInd:WARNING:NW->SMS SMS ERROR Len is too short!");
    ...
    {                                                                           /* 实体在等待RP_ACK                         */
        ...
        case SMR_NTM_DATA_TYPE_RP_ACK:
            ...
            /* 停止timer TR1M */
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMR_SmcApiDataInd: TimerStop: TR1M" );
            if ( SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_TR1M ) )
            {
                SMS_LOG( PS_PRINT_NORMAL, "SMS:SMR_SmcApiDataInd: TimerStop: TR1M Failed" );
            }
                      ...
                      )
{
    ...
    {                                                                           /* 未通过消息检查                           */
        ...
        PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_WARNING, "SMR_SmcApiEstInd:WARNING:ucChkError != SMR_MSG_CHK_SUCCESS ");
        return ucChkError;
    }
    ...
    if(SMR_IDLE == g_SmrEnt.SmrMt.ucState)
    {
        ...
        {
            PS_LOG(WUEPS_PID_SMS, VOS_NULL, PS_PRINT_NORMAL, "SMR_SmcApiEstInd:NORMAL:SMS state = SMR_WAIT_TO_SND_RP_ACK");
            g_SmrEnt.SmrMt.ucState = SMR_WAIT_TO_SND_RP_ACK;                    /* 实体状态迁移                             */
            g_SmrEnt.SmrMt.ucMr    = pucData[1];

            /* 启动定时器TR2M */
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMR_SmcApiEstInd: TimerStart: TR2M" );
            if ( SMS_FALSE == SMS_TimerStart( SMC_TIMER_ID_TR2M ) )
            {
                SMS_LOG( PS_PRINT_NORMAL, "SMS:SMR_SmcApiEstInd: TimerStart: TR2M failed" );
            }
            /* 关闭定时器TRRM */
            if( SMS_TIMER_STATUS_RUN == g_SmrEnt.SmrMt.RelTimerInfo.ucTimerSta )
            {
                SMS_LOG( PS_PRINT_NORMAL, "SMS:SMR_SmcApiEstInd: TimerStop: TRRM" );
            }
...
)
{
    ...
    {                                                                           /* 在MO过程                                 */
        if (SMR_IDLE != g_SmrEnt.SmrMo.ucState)
        {
            ...
            /* 停止timer TR1M */
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMR_SmcApiErrorInd: TimerStop: TR1M" );
            if ( SMS_FALSE == SMS_TimerStop( SMC_TIMER_ID_TR1M ) )
            {
                return;
            }
            ...
            /* 停止timer TRAM */
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMR_SmcApiErrorInd: TimerStop: TRAM" );
    ...
    else
    {
        if (SMR_WAIT_TO_SND_RP_ACK == g_SmrEnt.SmrMt.ucState)
        {
            ...
            /* 停止timer TR2M */
            SMS_LOG( PS_PRINT_NORMAL, "SMS:SMR_SmcApiErrorInd: TimerStop: TR2M" );
