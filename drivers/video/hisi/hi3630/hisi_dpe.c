...
static void dpe_interrupt_clear(struct hisi_fb_data_type *hisifd)
{
	...
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		clear = ~0;
		clear &= ~BIT_VACTIVE0_START_INT;
		outp32(dss_base + PDP_LDI_CPU_IRQ_CLR, clear);
	} else if (hisifd->index == EXTERNAL_PANEL_IDX) {
		clear = ~0;
		clear &= ~BIT_VACTIVE0_START_INT;
		outp32(dss_base + SDP_LDI_CPU_IRQ_CLR, clear);
	} else if (hisifd->index == AUXILIARY_PANEL_IDX) {
		clear = ~0;
		outp32(dss_base + DSS_GLB_OFFLINE_S2_CPU_IRQ_CLR, clear);
	} else {
		HISI_FB_ERR("fb%d, not support this device!\n", hisifd->index);
	}
}
