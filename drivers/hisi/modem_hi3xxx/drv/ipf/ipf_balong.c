...
static unsigned long long dma_mask = 0xffffffffULL;
...
static inline void ipf_pm_print_packet(void *buf, size_t len)
{
	...
	if (g_stIPFDebugInfo->ipf_wakeup_acore_intr_count){
...
BSP_VOID BSP_IPF_GetDlRd(BSP_U32* pu32Num, IPF_RD_DESC_S *pstRd)
{
    ...
    for(i = 0; i < u32Num; i++)
    {
        ...
        *(g_stIpfDl.u32IpfCdRptr) = u32CdqRptr;
		...
		ipf_pm_print_packet((void *)(unsigned long)pstRd[i].u32OutPtr, pstRd[i].u16PktLen);
        /* 更新RD读指针 */
...
static void get_ipf_int_stat(void)
{
	...
	status = ipf_readl(HI_IPF_REGBASE_ADDR_VIRT + HI_IPF_INT1_OFFSET); 
	if (status & (IPF_DL_RPT_INT1 \
					| IPF_DL_TIMEOUT_INT1 \
					| IPF_DL_ADQ0_EPTY_INT1 \
					| IPF_DL_ADQ1_EPTY_INT1)) {
		g_stIPFDebugInfo->ipf_wakeup_acore_intr_count++;
	}
}
