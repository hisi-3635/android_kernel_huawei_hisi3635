...
static const char *chip_text[] = {
    ...
    "Signal-I fpc1020a",     /* FPC1020_CHIP_1020A */
...
static int fpc1020_nav_write_sensor_1021_setup(fpc1020_data_t *fpc1020);
/* -------------------------------------------------------------------- */
...
void send_msg_to_dsm(struct dsm_client *dsm_client, int error, int dsm)
{
    ...
    if (!dsm_client_ocuppy(dsm_client)) {
        if(dsm == DSM_FINGERPRINT_SPISYNC_ERROR_NO){
            ...
            switch(error){
                case -ESHUTDOWN:
                    err_dsm = DSM_FINGERPRINT_SPI_ESHUTDOWN_ERROR_NO;
                    break;
                case -EBUSY:
                ...
                case -EIO:
                    ...
                    break;
                case -EINPROGRESS:
                    err_dsm = DSM_FINGERPRINT_SPI_EINPROGRESS_ERROR_NO;
                    break;
                default:
            ...
            switch(error){
                case FPC_1020_MIN_CHECKER_DIFF_ERROR:
                    err_dsm = DSM_FINGERPRINT_DIFF_DEADPIXELS_ERROR_NO;
                    break;
                case FPC_1020_TOO_MANY_DEADPIXEL_ERROR:
                    err_dsm = DSM_FINGERPRINT_MANY_DEADPIXELS_ERROR_NO;
                    break;
                default:
                    err_dsm = DSM_FINGERPRINT_TEST_DEADPIXELS_ERROR_NO;
                    break;
            }
        }
    else
...
int fpc1020_setup_defaults(fpc1020_data_t *fpc1020)
{
    ...
    error = (fpc1020->chip.type == FPC1020_CHIP_1020A) ||
        ...
        (fpc1020->chip.type == FPC1020_CHIP_1150B) ? 0 : -EINVAL;
    ...
    if (fpc1020->chip.type == FPC1020_CHIP_1020A) {
        if (gpio_is_valid(fpc1020->moduleID_gpio))
        {
            ...
            if(gpio_get_value_cansleep(fpc1020->moduleID_gpio))
            {
                ...
                ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_CT_a1a2 :
                    (fpc1020->chip.revision == 2) ? &fpc1020_setup_default_CT_a1a2 :
                    (fpc1020->chip.revision == 3) ? &fpc1020_setup_default_CT_a3a4 :
                    (fpc1020->chip.revision == 4) ? &fpc1020_setup_default_CT_a3a4 :
                    NULL;
            }
            else
            {
                printk("fingerprint module LO low \n");
                ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_LO_a1a2 :
                    (fpc1020->chip.revision == 2) ? &fpc1020_setup_default_LO_a1a2 :
                    (fpc1020->chip.revision == 3) ? &fpc1020_setup_default_LO_a3a4 :
                    (fpc1020->chip.revision == 4) ? &fpc1020_setup_default_LO_a3a4 :
                    NULL;
            }
        ...
        ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_1021_a2b1 :
            NULL;
    }else if (fpc1020->chip.type == FPC1020_CHIP_1021F) {
        ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_1021_a2b1 :
            NULL;
    }else if (fpc1020->chip.type == FPC1020_CHIP_1150A) {
        ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_1150_a1 :
            NULL;
    } else if (fpc1020->chip.type == FPC1020_CHIP_1150B){
        ptr = (fpc1020->chip.revision == 1) ? &fpc1020_setup_default_1150_a1 :
            NULL;
    } else {
        ptr = NULL;
...
/* -------------------------------------------------------------------- */
int fpc1020_nav_write_sensor_setup(fpc1020_data_t *fpc1020)
{
    switch (fpc1020->chip.type)
    {
        case FPC1020_CHIP_1020A:
        ...
        case FPC1020_CHIP_1021B:
        case FPC1020_CHIP_1021F:
        return fpc1020_nav_write_sensor_1021_setup(fpc1020);
...
static int fpc1020_write_sensor_1021_setup(fpc1020_data_t *fpc1020)
{
    ...
    temp_u64 = 0x363636363D3D3D3D/*0x0808080814141414*/;
    FPC1020_MK_REG_WRITE(reg, FPC102X_REG_SAMPLE_PX_DLY, &temp_u64);
    error = fpc1020_reg_access(fpc1020, &reg);
    if (error)
        goto out;
...
/* -------------------------------------------------------------------- */
int fpc1020_write_sensor_checkerboard_setup_1020(fpc1020_data_t *fpc1020,   u16 pattern)
{
    int error = 0;
    u8 temp_u8;
    u16 temp_u16;
...
/* -------------------------------------------------------------------- */
int fpc1020_write_sensor_checkerboard_setup_1150(fpc1020_data_t *fpc1020,   u16 pattern)
{
    int error = 0;
    u8 temp_u8;
    u16 temp_u16;
    ...
    temp_u16 = (pattern==0x55aa) ? 0x0f1B : 0x0f0f;
    FPC1020_MK_REG_WRITE(reg, FPC102X_REG_PXL_CTRL, &temp_u16);
    error = fpc1020_reg_access(fpc1020, &reg);
    if (error)
        goto out;
    ...
    do {
     error = spi_sync(fpc1020->spi, &msg);
        ...
        if (0 == error) {
            ...
            dev_err(&fpc1020->spi->dev, "spi_sync failed error=%d.\n", error);
            msleep(100);
            spi_failed_times++;
        }
    } while (spi_failed_times < SPI_MAX_TRY_TIMES);
    if (error) {
        dev_err(&fpc1020->spi->dev, "%s %d: sync error reached MAX FAILed times\n", __func__, __LINE__);
...
/* -------------------------------------------------------------------- */
int fpc1020_check_finger_present_sum(fpc1020_data_t *fpc1020, int *sub)
{
    int zones = 0;
                ...
                size_t buff_size)
{
    ...
    if (!error) {
     ...
     error = spi_sync(fpc1020->spi, &msg);
        ...
        memmove(buffer + offset, fpc1020->single_buff , image_size_bytes);
