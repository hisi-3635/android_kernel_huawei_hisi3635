...
#define FPC1020_NAV_ALGO_SUBZONES	3
			...
			int subarea_bits)
{
    ...
    if (subarea_bits == 0x111 || subarea_bits == 0x101 || subarea_bits == 0x110 || subarea_bits == 0x011
		|| subarea_bits == 0x100 || subarea_bits == 0x010 || subarea_bits == 0x001) {
        xG = -10;
    }
    *pCoordX = xG;
    *pCoordY = yG;
...
void init_enhanced_navi_setting(fpc1020_data_t* fpc1020)
{
    ...
    switch (fpc1020->nav.input_mode)
    {
        ...
        case FPC1020_INPUTMODE_DPAD:
            ...
            fpc1020->nav.threshold_key_start_down = 96;
            fpc1020->nav.threshold_key_start_left = 90;
            fpc1020->nav.threshold_key_start_right = 90;
...
/* -------------------------------------------------------*/
static void get_movement_fast(fpc1020_data_t *fpc1020)
{
#ifdef FPC1020_NAV_DPAD_DBG
 	pr_info("[FPC/DPAD] report touch down :%d, up=%d \n", fpc1020->nav.touch_down_x, fpc1020->nav.touch_up_x);
#endif
	if (fpc1020->nav.touch_up_x - fpc1020->nav.touch_down_x < -10 && fpc1020->nav.throw_event == 0) /*move fast down*/
	{
                pr_info("[FPC/DPAD] report fast key down\n");
	...
	if (fpc1020->nav.touch_up_x - fpc1020->nav.touch_down_x > 10 && fpc1020->nav.throw_event == 0) /*move fast down*/
	{
                pr_info("[FPC/DPAD] report fast key up\n");
...
static void dispatch_dpad_event(fpc1020_data_t* fpc1020, int x, int y, int finger_status)
{
    ...
    switch (finger_status)
    {
        ...
        case FNGR_ST_LOST:
	    get_movement_fast(fpc1020);/*workaround*/
            if (fpc1020->nav.move_down_cnt == 1 && fpc1020->nav.throw_event == 0)
            {
                if (fpc1020->nav.tap_status != FNGR_ST_TAP)
                {
#ifdef FPC1020_NAV_DPAD_DBG
                    pr_info("[FPC/DPAD] report m_single click\n");
#endif
...
#ifdef FPC1020_NAV_DPAD_DBG
            pr_info("[FPC/DPAD] report finger up last\n");
#endif
            break;
...
//#endif
        ...
        case FNGR_ST_MOVING:
...
#ifdef FPC1020_NAV_DPAD_DBG
            printk("[FPC/DPAD] report Navigation:active=%d, sum x:%d,y:%d, nav_imgs=%d, changes=%d\n",
                    fpc1020->nav.throw_event, fpc1020->nav.sum_x, fpc1020->nav.sum_y,
                    fpc1020->nav.nav_imgs,fpc1020->nav.touch_change_count);
#endif
            ...
            //if (!active) {
            if (fpc1020->nav.throw_event == 0 && fpc1020->nav.nav_imgs > 1
                  /*&& fpc1020->nav.touch_change_count > 0*/)
            {
                if (abs_x > abs_y) {
                	if (fpc1020->nav.sum_x > fpc1020->nav.threshold_key_start_right)
                	{
                    		key_code = KEY_RIGHT;
                	...
                	else if (fpc1020->nav.sum_x < -fpc1020->nav.threshold_key_start_left)
                	{
                    		key_code = KEY_LEFT;
                    		//fpc1020->nav.sum_x += fpc1020->nav.threshold_key_start;
                    		fpc1020->nav.sum_x = 0;
                ...
                if (key_code)
                {
                    active = 1;
#ifdef FPC1020_NAV_DPAD_DBG
                    pr_info("[FPC/DPAD] move up count=%d, down count=%d\n",fpc1020->nav.move_up_cnt, fpc1020->nav.move_down_cnt);
#endif
                    {
			if (fpc1020->nav.move_up_cnt > 0 && key_code == KEY_UP) {
				pr_info("[FPC/DPAD] report key up\n");
                        	input_report_key(fpc1020->touch_pad_dev, KEY_UP, 1);
                        	input_sync(fpc1020->touch_pad_dev);
                        	input_report_key(fpc1020->touch_pad_dev, KEY_UP, 0);
                        	input_sync(fpc1020->touch_pad_dev);
			...
			else if (fpc1020->nav.move_down_cnt > 0 && key_code == KEY_DOWN) {
				pr_info("[FPC/DPAD] report key down\n");
                        	input_report_key(fpc1020->touch_pad_dev, KEY_DOWN, 1);
                        	input_sync(fpc1020->touch_pad_dev);
                        	input_report_key(fpc1020->touch_pad_dev, KEY_DOWN, 0);
                        	input_sync(fpc1020->touch_pad_dev);
				...
				pr_info("[FPC/DPAD] report key left\n");
                        	input_report_key(fpc1020->touch_pad_dev, KEY_LEFT, 1);
                        	input_sync(fpc1020->touch_pad_dev);
                        	input_report_key(fpc1020->touch_pad_dev, KEY_LEFT, 0);
                        	input_sync(fpc1020->touch_pad_dev);
                        	fpc1020->nav.throw_event = 1;
                                fpc1020->nav.throw_event_move = 1;
                        } else if (key_code == KEY_RIGHT) {
				pr_info("[FPC/DPAD] report key right\n");
                                input_report_key(fpc1020->touch_pad_dev, KEY_RIGHT, 1);
                                input_sync(fpc1020->touch_pad_dev);
                                input_report_key(fpc1020->touch_pad_dev, KEY_RIGHT, 0);
                                input_sync(fpc1020->touch_pad_dev);
                                fpc1020->nav.throw_event = 1;
                                fpc1020->nav.throw_event_move = 1;
                        }
                        fpc1020->nav.report_key = key_code;
//#ifdef FPC1020_NAV_DPAD_DBG
                        switch (key_code)
                        {
                            case KEY_UP:
                                pr_info("[FPC/DPAD] is key up\n");
                                break;
                            case KEY_DOWN:
                                pr_info("[FPC/DPAD] is key down\n");
                                break;
                            case KEY_LEFT:
                                pr_info("[FPC/DPAD] is key left\n");
                                break;
                            case KEY_RIGHT:
                                pr_info("[FPC/DPAD] is key right\n");
                                break;
                            default:
                                break;
                        }
...
static void dispatch_touch_event(fpc1020_data_t* fpc1020, int x, int y, int finger_status)
{
    ...
    switch (finger_status)
    {
        ...
        case FNGR_ST_LOST:
            ...
            pr_info("[FPC/TOUCH] finger lost, nav_sum_x=%d,nav_sum_y=%d, filer=%d\n",
...
#ifdef FPC1020_NAV_TOUCH_DBG
            pr_info("[FPC/TOUCH] %s:touch moving sum x,y(%d, %d)/sum_x,sum_y(%d,%d)\n",
...
static void process_navi_event(fpc1020_data_t* fpc1020, int dx, int dy, int finger_status)
{
    ...
    const int THRESHOLD_RANGE_MIN_TAP = 500;
	...
	//if (duration<=30&&finger_status!=FNGR_ST_LOST&&(dx!=0 || dy!=0)) {
	//	dx = 0;
	//	dy = 0;
	//}
        deviation_x += dx;
        ...
        pr_info("[FPC] %s: click deviation=%d, duration=%d, finger status=%d, thraw =%d, double interval%d, zones=%d\n",
                __func__, deviation, duration, fpc1020->nav.tap_status, fpc1020->nav.throw_event,
               tick_curr - fpc1020->nav.tap_start,fpc1020->nav.last_zones);
#endif
        ...
        else
        {
            if (duration < THRESHOLD_DURATION_TAP)
            {
                ...
                {
                    if (fpc1020->nav.last_zones > 0)
                    {
                          if (fpc1020->nav.tap_status == FNGR_ST_TAP
				&& tick_curr - fpc1020->nav.tap_start <= THRESHOLD_DURATION_DTAP
				&& ((deviation <= THRESHOLD_RANGE_DOUBLE_TAP )||((dy == fpc1020->nav.move_pre_y)&& (dx==fpc1020->nav.move_pre_x)) || fpc1020->nav.touch_change_count == 0)
				&& fpc1020->nav.throw_event == 0)
                          {
                        ...
                        pr_info("[FPC] %s:prepare report double click\n", __func__);
                          ...
                          else if ((deviation <= THRESHOLD_RANGE_DOUBLE_TAP || ((dy == fpc1020->nav.move_pre_y)&& (dx==fpc1020->nav.move_pre_x)) || fpc1020->nav.touch_change_count == 0)
...
{
    int error = 0;
    int nav_image_width = NAV_IMAGE_WIDTH;
    int nav_image_height = NAV_IMAGE_HEIGHT;
           ...
           fpc1020->nav.enabled && (error >= 0))
    {
...
#endif
        if (error < 0 /* && fpc1020->down == false*/)
        { break; }
        error = fpc1020_check_finger_present_raw(fpc1020);
#ifdef FPC1020_NAV_DBG
        pr_info("[FPC] report finger is down\n");
#endif
        get_finger_position(&touch_x, &touch_y, error);
       ...
       // queue_work(fpc1020->fpc_nav_workqueue, &fpc1020->fpc_nav_work);
        ...
        while (!fpc1020->worker.stop_request && (error >= 0))
        {
            ...
            {
#ifdef  FPC1020_NAV_DBG
                pr_info("[FPC] prepare report finger up, last_zones=%d\n", fpc1020->nav.last_zones);
#endif

#if 0
                click_diffTime = abs(jiffies - fpc1020->nav.click_start);
                click_diffTime = click_diffTime * 1000000 / HZ;
                if (click_diffTime <= FPC1020_CLICK_INTERVAL&&fpc1020->nav.last_zones == 0)
                {
                    if (fpc1020->nav.tap_status == FNGR_ST_TAP &&
...
#endif
#ifdef FPC1020_NAV_DBG
		printk("[FPC] report fingerup line=%d, down=%d\n", __LINE__, fpc1020->down);
#endif
...
#ifdef FPC1020_NAV_DBG
            printk("[FPC] touch sub=0x%x,x=%d, y=%d\n", sub, touch_x, touch_y);
#endif
#ifdef FPC1020_NAV_HEAP_BUF
            if (isReverse)
            {
            ...
            {
#ifdef	FPC1020_NAV_DBG
		pr_info("[FPC] prepare report finger up, last zones=%d\n", fpc1020->nav.last_zones);
#endif
            ...
            get_finger_position(&touch_x, &touch_y, sub);
            fpc1020->nav.touch_up_x = touch_x;
            fpc1020->nav.poll_count++;
            if (fpc1020->nav.touch_pre_x != touch_x
                  || fpc1020->nav.touch_pre_y != touch_y)
                  fpc1020->nav.touch_change_count++; 
#ifdef FPC1020_NAV_DBG
            printk("[FPC] sub=0x%x, touch x=%d, y=%d\n", sub, touch_x, touch_y);
#endif
#ifdef FPC1020_NAV_HEAP_BUF
            isReverse = !isReverse;
#endif
            ...
            fpc1020->nav.sumy += dy;
#ifdef FPC1020_NAV_DBG
            pr_info("[FPC] report get_movement dx=%d, dy=%d, sumx=%d, sumy=%d,zones=%d, orig\n",
				dx, dy, sumX, sumY, fpc1020->nav.detect_zones);
#endif
            /*workaround to filter data*/
            abs_x = dx > 0 ? dx:-dx;
            abs_y = dy > 0 ? dy:-dy;
       
	    if (((abs_x != 0 || abs_y != 0 )&& fpc1020->nav.detect_zones < FPC1020_NAV_ALGO_SUBZONES) ||
			(abs_x == abs_y))
            {
#ifdef FPC1020_NAV_DBG
		pr_info("[FPC] report move reset\n");
#endif
               	dx = 0;
                dy = 0;
            }
		...
		dx = 0;
                dy = 0;
#ifdef FPC1020_NAV_DBG
                pr_info("[FPC] report move reset@end\n");
#endif
	    }
		
            if (fpc1020->nav.sumx == 0 && fpc1020->nav.sumy == 0&& fpc1020->nav.nav_imgs > 38)
            {
                 dx = 0;
                 dy = 0;
#ifdef FPC1020_NAV_DBG
                 printk("[FPC] report reset for long press\n");
#endif
            }
            sumX += dx;
            sumY += dy;
#ifdef FPC1020_NAV_DBG
	    if (dx!=0 || dy !=0)
            	pr_info("[FPC] report get_movement dx=%d, dy=%d, sumx=%d, sumy=%d\n", dx, dy, sumX, sumY);
#endif
