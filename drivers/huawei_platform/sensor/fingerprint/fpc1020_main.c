...
static void fpc1020_spi_cs_set(u32 control)
{
    int ret = 0;
    if(!fpc1020)
    {
        pr_err("%s: fpc1020 is NULL\n", __func__);
        return;
    }
    if (SSP_CHIP_SELECT == control) {
        if(fpc1020->spi)
        {
            ret = spi_setup(fpc1020->spi);
            if(ret < 0)
            {
                dev_err(&fpc1020->spi->dev,"%s: fail to setup spi\n",__func__);
            }
        }
        ret = gpio_direction_output(fpc1020->cs_gpio, control);
    } else {
        ret = gpio_direction_output(fpc1020->cs_gpio, control);
    }
    if (ret < 0)
        dev_err(&fpc1020->spi->dev,"%s: fail to set cs gpio,control=%d result = %d\n", __func__,control,ret);
}
...
int fpc1020_get_dts_data(struct device *dev, fpc1020_data_t *fpc1020_data)
{
    ...
    int ret = 0;
    if (!dev || !dev->of_node) {
        pr_err("%s dev or dev node is NULL", __func__);
        return -EINVAL;
    }
    np = dev->of_node;

    ret = of_property_read_u32(np, "fpc1020,cs_gpio",
        &fpc1020_data->cs_gpio);
    if(ret)
    {
        fpc1020_data->cs_gpio = -EINVAL;
        dev_err(dev,"failed to get cs gpio from device tree\n");
    }

    ret = of_property_read_u32(np, "fpc1020,reset_gpio",
        &fpc1020_data->reset_gpio);
    if(ret)
    {
        fpc1020_data->reset_gpio = -EINVAL;
        dev_err(dev,"failed to get reset gpio from device tree\n");
    }

    ret = of_property_read_u32(np, "fpc1020,power_gpio",
        &fpc1020_data->power_gpio);
    if(ret || (fpc1020_data->power_gpio == 0))
    {
        fpc1020_data->power_gpio = -EINVAL;
        dev_err(dev,"failed to get power gpio from device tree\n");
    }

    ret = of_property_read_u32(np, "fpc1020,irq_gpio",
        &fpc1020_data->irq_gpio);
    if(ret)
    {
        fpc1020_data->irq_gpio = -EINVAL;
    ...
    of_property_read_u32(np, "pl022,interface",
        &fpc1020_data->spidev0_chip_info.iface);
    of_property_read_u32(np, "pl022,com-mode",
        &fpc1020_data->spidev0_chip_info.com_mode);
    of_property_read_u32(np, "pl022,rx-level-trig",
        &fpc1020_data->spidev0_chip_info.rx_lev_trig);
    of_property_read_u32(np, "pl022,tx-level-trig",
        &fpc1020_data->spidev0_chip_info.tx_lev_trig);
    of_property_read_u32(np, "pl022,ctrl-len",
        &fpc1020_data->spidev0_chip_info.ctrl_len);
    of_property_read_u32(np, "pl022,wait-state",
        &fpc1020_data->spidev0_chip_info.wait_state);
    of_property_read_u32(np, "pl022,duplex",
        &fpc1020_data->spidev0_chip_info.duplex);
#if defined(CONFIG_ARCH_KIRIN)
    fpc1020_data->spidev0_chip_info.slave_tx_disable =
        of_property_read_bool(np, "pl022,slave-tx-disable");
#endif

    /* use gpio cs control */
    fpc1020_data->spidev0_chip_info.cs_control = fpc1020_spi_cs_set;
    return 0;
...
static int fpc1020_probe(struct spi_device *spi)
{
    ...
    printk(KERN_INFO "%s v1.7 add auto thredhold \n", __func__);
    ...
    if (error){
             platform_device_del(fpc1020->pf_dev);
               goto err;
                ...
                if (error) {
                         dev_err(&fpc1020->spi->dev,"sysf_create_group fingerprint_attr_group failed.\n");
                         goto err;
                }
   }
...
*/
    ...
    if (error != 0){
        dev_err(&fpc1020->spi->dev, "%s %d: test_deadpixels error=%d \n", __func__, __LINE__,error);
...
static int fpc1020_reset_init(fpc1020_data_t *fpc1020)
{
    ...
    if (gpio_is_valid(fpc1020->moduleID_gpio)) {
        ...
        error = gpio_request(fpc1020->moduleID_gpio, "fingerprint_moduleID_gpio");
...
static int fpc1020_power_init(fpc1020_data_t *fpc1020)
{
    ...
    if (gpio_is_valid(fpc1020->power_gpio)) {
        ...
        error = gpio_request(fpc1020->power_gpio, "fpc1020_power");

        if (error) {
            dev_err(&fpc1020->spi->dev,
                "gpio_request (power) failed.\n");
            return error;
        }

        error = gpio_direction_output(fpc1020->power_gpio, 0);

        if (error) {
            dev_err(&fpc1020->spi->dev,
            "gpio_direction_output(power) failed.\n");
            return error;
        }
    }
    return error;
...
static int fpc1020_spi_setup(fpc1020_data_t *fpc1020)
{
    ...
    fpc1020->spi->controller_data = &fpc1020->spidev0_chip_info;
...
static int fpc1020_check_for_deadPixels(fpc1020_data_t *fpc1020, u8* ripPixels, bool bCB)
{
   ...
   switch (fpc1020->chip.type) {
        ...
        for (x = 0; x < check_cols; x += 2) {
            ...
            if (dev1 > config_max_deviation) {
                if (deadpixels < config_max_dead_pixels_to_list) {
                           printk("%s: Dead pixel found @ ImgXY[%d, %d]  value[%d]\n", __func__, (x + ((odd) ? 1 : 0)), y,buff[i1]);
                }
                deadpixels++;
                ripPixels[i1] = 1;
            }
    ...
    if (deadpixels > config_max_dead_pixels_to_list) {
         printk("%s: More dead pixels found... Not listing all, deadpixel num =%d\n", __func__,deadpixels);
    }
    ...
    if (!dsm_client_ocuppy(fpc1020->dsm_fingerprint_client)) {
        dsm_client_record(fpc1020->dsm_fingerprint_client, "test deadpixels=%d\n", deadpixels);
...
static int fpc1020_worker_function(void *_fpc1020)
{
    ...
    while (!kthread_should_stop()) {
        ...
        dev_info(&fpc1020->spi->dev,"work in %s mode = %d  capture = %d\n", __func__,fpc1020->worker.req_mode,fpc1020->capture.current_mode);
