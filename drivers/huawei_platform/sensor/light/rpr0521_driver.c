...
static int Rpr0521_i2c_write(struct i2c_client*client, u8 reg, u16 value,bool flag)
{
	...
	while(loop) {
		...
		if(flag == RPR0521_I2C_BYTE)
		{
			...
			err = i2c_smbus_write_byte_data(client, reg, (u8)value);//grace modify in 2014.7.28
		}
		else if(flag == RPR0521_I2C_WORD)
		{
			//err = i2c_smbus_write_word_data(client, CMD_WORD|reg, value);
			err = i2c_smbus_write_word_data(client, reg, value);//grace modify in 2014.7.28
		}
		else
		{
...
static int Rpr0521_i2c_read(struct i2c_client*client, u8 reg,bool flag)
{
	...
	while(loop) {
		...
		if(flag == RPR0521_I2C_BYTE)
		{
			...
			err = i2c_smbus_read_byte_data(client, reg); //grace modify in 2014.7.28
		}
		else if(flag == RPR0521_I2C_WORD)
		{
			//err = i2c_smbus_read_word_data(client, CMD_WORD|reg); //grace modify in 2014.7.28
...
static void rpr0521_change_ps_threshold(struct i2c_client *client)
{
	...
	int ret; //grace modify in 2014.9.18
	unsigned char ps_info[3]; //grace modify in 2014.9.18
...
{
//grace modify in 2014.6.18 begin	
#if 0 
	...
	return rpr521_set_enable(client, PS_ALS_SET_MODE_CONTROL|PS_EN);//grace modify in 2014.5.7
