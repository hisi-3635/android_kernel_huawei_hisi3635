...
static int enable_boost(bool enable)
{
    ...
    if (enable) {
        if (!pdata->boost_flag) {
            ...
            if (ret != 0) {
                pr_err("%s:bulk enable fail err = %d\n", __FUNCTION__, ret);
                return ret;
            }
#endif
        ...
        if (pdata->boost_flag) {
            ...
            if (ret != 0) {
                pr_err("%s:bulk disable fail err = %d\n", __FUNCTION__, ret);
                return ret;
            }
#endif
...
static void anc_hs_adc_calibration(void)
{
    ...
    while (loop) {
        ...
        if (adc_h < 0) {
            pr_err("[anc_hs]get adc fail,adc_h:%d\n", adc_h);
            fail_count ++;
            continue;
        }
        adc_l = hisi_adc_get_value(pdata->channel_pwl_l);
        if (adc_l < 0) {
            pr_err("[anc_hs]get adc fail,adc_l:%d\n", adc_l);
            fail_count ++;
            continue;
        }
    ...
    if (count == 0) {
        ...
        pr_info("anc_hs:calibration_base = %d with %d times\n", pdata->adc_calibration_base, count);
...
{
    struct anc_dev *pdev = pdata->dev;
    struct anc_codec_ops *fops = &pdev->ops;

    /* userspace prohibit charging with using hs-mic pin */
    if (ANC_HS_DISABLE_CHARGE == pdata->force_charge_ctl) {
        pr_info("%s(%u) : charge is occupied by app level\n", __FUNCTION__, __LINE__);
    ...
    pr_info("%s: delay %d ms to wait anc chip up!\n", __FUNCTION__, pdata->sleep_time);
        ...
        if (pdata->mic_used) {
            ...
            if (!fops->check_headset_in(pdata->private_data)) {
                ...
                if(pdata->anc_hs_mode == ANC_HS_CHARGE_OFF){
                    ...
                    anc_hs_enable_irq();
                    pr_info("%s(%u) : resume charging for anc hs!\n", __FUNCTION__, __LINE__);
                }
                mutex_unlock(&pdata->charge_lock);
            }
...
void anc_hs_charge_detect(int saradc_value, int headset_type)
{
    ...
    if (headset_type == ANC_HS_4POLE) {
        /* 4-pole headset maybe an anc headset*/
        pr_debug("%s : start anc hs charge judge\n", __FUNCTION__);
        anc_hs_charge_judge();
...
int anc_hs_dev_register(struct anc_dev *dev, void * codec_data)
{
    ...
    if(pdata->registered) {
        pr_err("one codec has registered, no more permit\n");
        return -EEXIST;
    }
    if(!dev->ops.check_headset_in ||
        !dev->ops.btn_report ||
        !dev->ops.codec_resume_lock) {
        pr_err("codec ops funtion must be all registed\n");
        return -EINVAL;
    }
    ...
    pr_info("%s(%u) : anc hs has been register sucessful!\n", __FUNCTION__, __LINE__);
...
static void anc_hs_btn_judge(struct work_struct* work)
{
    ...
    if(pdata->anc_hs_mode == ANC_HS_CHARGE_OFF) {
        pr_info("%s(%u):ignore this irq\n", __FUNCTION__, __LINE__);
        if(pdata->button_pressed == 1) {
            btn_report = 0;
            pdata->button_pressed = 0;
            fops->btn_report(btn_report, ANC_BTN_MASK);
            pr_info("%s:irq level:%d", __FUNCTION__, anc_hs_gpio_get_value(pdata->mic_irq_gpio));
        }
        return;
    }

    pr_info("%s(%u):deal with button irq event!\n", __FUNCTION__, __LINE__);
                         ...
                         unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case IOCTL_ANC_HS_GET_HEADSET_CMD:
            ...
            ret = put_user((__u32)(charge_mode), pUser);
            break;
        case IOCTL_ANC_HS_GET_TYPE_CMD:
            ret = put_user((__u32)(pdata->anc_hs_mode), pUser);
            break;
        default:
            pr_err("unsupport cmd\n");
            ret = -EINVAL;
            break;
    }
...
static DEVICE_ATTR(hs_info, 0660, anc_hs_info_show,
                   NULL);
...
static int anc_hs_probe(struct platform_device* pdev)
{
    ...
    if (0 != ret) {
        pr_err("couldn't get regulators(boost5v) %d\n", ret);
        return -ENODEV;
    }
#endif
    ...
    if (pdata->mic_irq_gpio < 0) {
        pr_err("mic_irq_gpio is unvalid!\n");
        return -ENOENT;
    }
    ...
    if (ret) {
        pr_err("error request GPIO for mic_irq_gpio fail %d\n", ret);
        return -ENODEV;
    }
    ...
    if (ret) {
        pr_err("error request GPIO for mic_sw fail %d\n", ret);
    ...
    if (ret) {
        pr_err("%s: anc hs misc device register failed", __FUNCTION__);
        goto anc_hs_irq_err;
    }
