...
int inputhub_mcu_write_cmd_nolock(const void *buf, unsigned long length)
{
    int ret = 0;
...
void iom3_log_handle(struct work_struct  *work)
{
    ...
    if(pkt->level == LOG_LEVEL_FATAL){
		...
		if(memcmp(pkt->log,"dmd_acc",7) == 0){
			hwlog_err("[fatal]acc err\n");
			p = strstr(pkt->log, "exceed");
			if (NULL == p) {
				if(memcmp(pkt->log,"dmd_acc_data_abnormal",21) == 0) {

					iom3_need_recovery();
				}
			} else {
				if(memcmp(pkt->log,"dmd_acc_data_abnormal_exceed",28) == 0) {

					iom3_need_recovery();
				}
			}
		}
		else if(memcmp(pkt->log,"dmd_mag",7) == 0){
			hwlog_err("[fatal]mag err\n");
			if(memcmp(pkt->log,"dmd_mag_data_abnormal",21) == 0) {
				iom3_need_recovery();
			}
		}
		else if(memcmp(pkt->log,"dmd_gyro",8) == 0){
			hwlog_err("[fatal]gyro err\n");
			if(memcmp(pkt->log,"dmd_gyro_data_abnormal",22) == 0) {

				iom3_need_recovery();
			}
		}
		else if(memcmp(pkt->log,"dmd_light",9) == 0){
			hwlog_err("[fatal]light err\n");
			dmd_log_report(DSM_SHB_ERR_IOM3_LIGHT, __func__, "light error");
		}
		else if (memcmp(pkt->log,"dmd_inv",7) == 0) {
			hwlog_err("[fatal]dmd_inv err\n");
			if(memcmp(pkt->log,"dmd_inv_get_fifo_all",20) == 0) {

				iom3_need_recovery();
			}
			if (memcmp(pkt->log,"dmd_inv_all_zero_header",23) == 0) {

				iom3_need_recovery();
			}

		}
		else if(memcmp(pkt->log,"dmd_other",9) == 0){
			hwlog_err("[fatal]other err\n");
			dmd_log_report(DSM_SHB_ERR_IOM3_OTHER, __func__, "other error");
		}
		else{
			hwlog_err("[fatal]unknow err\n");
			dmd_log_report(DSM_SHB_ERR_IOM3_UNKNOW, __func__, "unknow error");
		}
    }
...
hwlog_err("%s \n", __func__);
	...
	while(retry_count--){//send retry 3 times
		...
		if (inputhub_mcu_write_cmd(buf, sizeof(pkt_header_t) + wr->wr_len)){ 
			hwlog_err("send cmd to mcu failed in %s, retry %d\n", __func__, retry_count);
		}else if (!wait_for_completion_timeout(&type_record.resp_complete, msecs_to_jiffies(2000))) {
			hwlog_err("wait for response timeout in %s, retry %d\n", __func__, retry_count);
			...
			//hwlog_info("wait for response success in %s, retry %d\n", __func__, retry_count);
...
int inputhub_route_cmd(unsigned short port, unsigned int cmd, unsigned long arg)
{
    switch (port) {
        case ROUTE_SHB_PORT:
        {
            if(flag_for_sensor_test) return 0;
