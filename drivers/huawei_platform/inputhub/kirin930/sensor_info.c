...
#define HANDPRESS_DATA_MAX_LEN				512
#define WHITE	0xE1
#define BLACK	0xD2
#define PINK	0xC3
#define RED		0xB4
...
#define CARRERA  10 // decimal base
...
typedef struct _BH1745_ALS_PARA_TABLE{
	...
	uint8_t tp_color;
	s16 bh745_para[17]; //give to bh1745 rgb sensor use,output lux and cct will use these para
...
static int set_fingersense_enable_internal(unsigned long enable, bool lock)
{
	...
	if(enable) {
        ...
        if(check_sensor_info(TAG_ACCEL, CMD_CMN_OPEN_REQ, 0)) {
            if(lock) {
        ...
        if(check_sensor_info(TAG_ACCEL, CMD_CMN_INTERVAL_REQ, 200)) {
            if(lock) {
    ...
    if(!enable) {
        if(check_sensor_info(TAG_ACCEL, CMD_CMN_CLOSE_REQ, 0)) {
            if(lock) {
...
int check_sensor_info(int tag, obj_cmd_t cmd, int delay)
{
	...
	if (TAG_SENSOR_BEGIN <= tag && tag < TAG_SENSOR_END) {
		...
		if ((CMD_CMN_OPEN_REQ == cmd) &&(sensor_status.opened[tag])) {
			...
			if ((TAG_PS != tag) && (sensor_status.delay[tag] != 0)&&(sensor_status.delay[tag]\
				<= delay)){
				ret = 0;
			}
		}
...
static int init_sensors_cfg_data_from_dts(void)
{
	...
	for_each_node_with_property(dn, "sensor_type")
	{
		...
		if(ret)
		{
			hwlog_err("%s:sensor_type =%s read status error\n", __func__, sensor_ty);
		}
		else
		{
			if(strncmp(status, "ok", sizeof("ok")))
			{
				hwlog_err("%s:sensor_type =%s sensor status is disable\n", __func__, sensor_ty);
				continue;
			}
		}
		...
		}else if(!strncmp(sensor_ty, "mag", sizeof("mag")))
		{
			...
			hwlog_info("magnetic sensor DC(%s):\n", str_soft_para);
		...
		else if(!strncmp(sensor_ty, "als", sizeof("als")))
		{
			...
			ret=of_property_read_u32(dn, "als_phone_type", &als_phone_type);
			if(ret) hwlog_err("%s:read als_phone_type fail\n",__func__);
			else als_data.als_phone_type=(uint8_t)als_phone_type;

			ret=of_property_read_u32(dn, "als_phone_version", &als_phone_version);
			if(ret) hwlog_err("%s:read als_phone_version fail\n",__func__);
			else als_data.als_phone_version=(uint8_t)als_phone_version;
...
#ifdef CONFIG_HUAWEI_HW_DEV_DCT
			set_hw_dev_flag(DEV_I2C_PS_SENSOR);
#endif
			}
...
static ssize_t show_##TAG##_selfTest_result(struct device *dev,\
				struct device_attribute *attr, char *buf)\
{\
	hwlog_err("%s , result is %s\n", __func__, sensor_status.TAG##_selfTest_result);\
...
{
	if (strlen(sensor_chip_info[HANDPRESS]) != 0)
		return snprintf(buf, MAX_STR_SIZE, "%u\n", handpress_data_flag);
	else
		return -1;
}

static ssize_t store_hp_sensor_switch(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size)
{
	unsigned long source = 0;
	...
	if (strlen(sensor_chip_info[HANDPRESS]) != 0) {
		source = simple_strtoul(buf, NULL, 10);
		handpress_data_flag = (unsigned int)source;
	}
	return size;
}

static int hp_sensor_get_data(unsigned int *data, int *data_len)
{
	unsigned char hp_data_cfg[][4] = {
		{0x38, 0x01, 0x01, 0x03},	// button status
		{0x2C, 0x02, 0x05, 0x06},	// raw count data0
		{0x2E, 0x02, 0x05, 0x06},	// raw count data1
		{0x30, 0x02, 0x05, 0x02},	// base line data0
		{0x32, 0x02, 0x05, 0x02},	// base line data1
		{0x34, 0x02, 0x05, 0x03},	// signal data0
		{0x36, 0x02, 0x05, 0x03},	// signal data1
	};
	int i=0,j=0;
	int ret = 0;
	int chip_count = 0;
	int reg_count = 0;
	int times = 1;
	uint8_t currunt_addr  = 0x00;
	uint8_t buff[2] = {0x01, 0x00};
	unsigned char temp_buff[2] = {0x00};
	int length = 0;
	reg_count = sizeof(hp_data_cfg)/(sizeof(hp_data_cfg[0][0])*4);
	length = 1;
	for (;chip_count<CYPRESS_CHIPS; chip_count++) {
		currunt_addr = handpress_data.i2c_address[chip_count];
		// make sure the chip is work in normal
		ret = mcu_i2c_rw(0,currunt_addr, 0x00, CMD_I2C_WRITE_REQ, 0x01, &buff[0]);
		if (ret < 0) {
			hwlog_err("i2c write err!!ret:%d(set normal mode)\n", ret);
			break;
		}
		msleep(15);
		ret = mcu_i2c_rw(0, currunt_addr, hp_data_cfg[0][0], CMD_I2C_READ_REQ, hp_data_cfg[0][1], temp_buff);
		if (ret < 0) {
			hwlog_err("i2c read err!!ret:%d\n", ret);
			break;
		}
		data[0] |= temp_buff[0]<<(chip_count*2);
		memset(temp_buff, 0x00, sizeof(temp_buff));
		for (i=1; i<reg_count; i++) {
			if (!(handpress_data_flag&hp_data_cfg[i][3]))
				continue;
			if (handpress_data_flag == 2)
				 times = hp_data_cfg[i][2];
			for(j=0; j<times; j++) {
				ret = mcu_i2c_rw(0, currunt_addr, hp_data_cfg[i][0], CMD_I2C_READ_REQ, hp_data_cfg[i][1], temp_buff);
				if (ret < 0) {
					hwlog_err("i2c read err!!ret:%d\n", ret);
					continue;
				}
				data[length] = (hp_data_cfg[i][1] ==2) ? ((unsigned int)temp_buff[1]<<8 |(unsigned int)temp_buff[0]):(unsigned int)temp_buff[0];
				length++;
				memset(temp_buff, 0x00, sizeof(temp_buff));
				msleep(15);
			}
		}
		// make sure the chip is work in idle
		ret = mcu_i2c_rw(0,currunt_addr, 0x00, CMD_I2C_WRITE_REQ, 0x01, &buff[1]);
		if (ret < 0) {
			hwlog_err("i2c write err!!ret:%d(set idle mode)\n", ret);
			break;
		}
	}
	*data_len = length;
...
static ssize_t show_hp_sensor_data_cfg(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int i = 0;
	unsigned int data_len = 0;
	ssize_t count = 0;
	unsigned int data[HANDPRESS_DATA_MAX_LEN] = {0x00};

	if (strlen(sensor_chip_info[HANDPRESS]) == 0)
		return -1;

	if (hp_sensor_get_data(data, &data_len) < 0)
		return -1;

	for (i=0;i<data_len; i++) {
		if (count >= MAX_STR_SIZE-16)
			break;
		count += snprintf(buf+count, (MAX_STR_SIZE - count), "%u ", data[i]);
	}
	count += snprintf(buf+count, (MAX_STR_SIZE-count), "\n");
	handpress_data_flag = 0;
	return count;
}

static DEVICE_ATTR(handpress_switch, 0660, show_hp_sensor_switch, store_hp_sensor_switch);
static DEVICE_ATTR(handpress_data, 0440, show_hp_sensor_data_cfg, NULL);
...
static struct attribute *sensor_attributes[] = {
	...
	&dev_attr_handpress_switch.attr,
	&dev_attr_handpress_data.attr,
