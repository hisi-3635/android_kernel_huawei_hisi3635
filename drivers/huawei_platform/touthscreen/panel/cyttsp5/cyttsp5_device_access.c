...
static int cyttsp5_suspend_scan_cmd_(struct device *dev)
{
    ...
    if (rc < 0)
        TS_LOG_ERR("%s: Suspend scan failed r=%d\n",
            __func__, rc);
    return rc;
}
...
static int cyttsp5_resume_scan_cmd_(struct device *dev)
{
    ...
    if (rc < 0)
        TS_LOG_ERR("%s: Resume scan failed r=%d\n",
            __func__, rc);
    return rc;
}
        ...
        u16 *actual_read_len, u8 *data)
{
    ...
    if (rc < 0)
        TS_LOG_ERR("%s: Get data structure failed r=%d\n",
                __func__, rc);
    return rc;
}
        ...
        u8 *results_available)
{
    ...
    rc = cmd->nonhid_cmd->run_selftest(dev, 0, test_id,
            ...
            results_available);
    if (rc < 0)
        TS_LOG_ERR("%s: Run self test failed r=%d\n",
                __func__, rc);
    return rc;
}
        ...
        u16 *actual_read_len, u8 *data)
{
    ...
    if (rc < 0)
        TS_LOG_ERR("%s: Get self test result failed r=%d\n",
                __func__, rc);
    return rc;
}
        ...
        struct cyttsp5_attribute *attr, char *buf)
{
    ...
    if (rc < 0) {
        TS_LOG_ERR("%s: Error on execute panel scan r=%d\n",
                __func__, rc);
        goto resume_scan;
    }
    ...
    if (rc < 0) {
        TS_LOG_ERR("%s: Error on retrieve panel scan r=%d\n",
                __func__, rc);
        goto resume_scan;
    }
        ...
        struct cyttsp5_attribute *attr, char *buf)
{
    ...
    if (rc < 0) {
        TS_LOG_ERR("%s: Error on get data structure r=%d\n",
                __func__, rc);
        goto resume_scan;
    }
        ...
        struct cyttsp5_attribute *attr, char *buf)
{
    ...
    if (rc < 0) {
        TS_LOG_ERR("%s: Error on run self test r=%d\n",
                __func__, rc);
        goto resume_scan;
    }
        ...
        struct cyttsp5_attribute *attr, char *buf)
{
    ...
    rc = _cyttsp5_calibrate_idacs_cmd(dev, dad->calibrate_sensing_mode,
            &dad->ic_buf[0]);
    if (rc < 0) {
        TS_LOG_ERR("%s: Error on calibrate idacs r=%d\n",
                __func__, rc);
        goto resume_scan;
    }
    ...
    if (dad->calibrate_initialize_baselines) {
        ...
        rc = _cyttsp5_initialize_baselines_cmd(dev, CY_IB_SM_MUTCAP |
                ...
                &dad->ic_buf[length]);
        if (rc < 0) {
            TS_LOG_ERR("%s: Error on initialize baselines r=%d\n",
                    __func__, rc);
            goto resume_scan;
        }
...
exit:
    mutex_unlock(&dad->debugfs_lock);
    TS_LOG_DEBUG("%s: return count=%zu\n", __func__, count);
    return count;
}
...
static int cyttsp5_device_access_probe(struct device *dev)
{
    ...
    if (dad->si) {
        rc = cyttsp5_setup_sysfs(dev);
        if (rc) {
            TS_LOG_INFO( "%s:Fail at setup sys, rc:%d.\n", __func__, rc);
...
static int __init cyttsp5_device_access_init(void)
{
    ...
    if (!cmd) {
        TS_LOG_INFO("%s:failed to get commands\n", __func__);
        return -EINVAL;
    }
    ...
    for (i = 0; i < num_core_ids; i++) {
        ...
        TS_LOG_INFO("%s: Registering device access module for core_id: %s\n",
            __func__, core_ids[i]);
    ...
    TS_LOG_INFO("%s: Cypress TTSP Device Access Driver (Built %s) rc=%d\n",
...
fail_unregister_devices:
    for (i--; i >= 0; i--) {
        ...
        TS_LOG_INFO("%s: Unregistering device access module for core_id: %s\n",
            __func__, core_ids[i]);
    }
    return rc;
}
