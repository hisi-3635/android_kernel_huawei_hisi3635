...
#define S3718_HOLSTER_SWITCH_OFFSET    12
...
/*only greater version can update, when set check dts and boot update fw, this data set to 1.*/
int g_boot_fwv_greater_check = 0;
...
static struct touch_settings *synaptics_sett_param_regs = &synaptics_sett_param_regs_map[9];
...
static int synaptics_chip_get_info(struct ts_chip_info_param *info)
{
	...
	if(!g_ts_data.get_info_flag)
	{
		if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type) {
			retval = synaptics_get_configid(buf, sizeof(buf));
			if (retval < 0)
				TS_LOG_ERR("failed to get configid\n");
		} else {
			retval = synaptics_get_s3718_configid(buf, sizeof(buf));
			if (retval < 0)
				TS_LOG_ERR("failed to get configid\n");
		}
	}
...
static int synaptics_parse_dts(struct device_node *device,struct ts_device_data *chip_data)
{
	...
	retval = of_property_read_u32(device, SYNAPTICS_FWV_G_UPDATE_CHECK, &chip_data->fwv_g_update_check);
	if (retval) {
		TS_LOG_INFO("get device SYNAPTICS_FWV_G_UPDATE_CHECK null\n");
		chip_data->fwv_g_update_check = 0;
		retval = 0;
	}
...
static int synaptics_fw_update_boot(char *file_name)
{
	...
	if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type) {
		...
		if (retval) {
			...
			#if defined (CONFIG_HUAWEI_DSM)
			g_ts_data.dsm_info.constraints_UPDATE_status = fwu_fw_data_init_error;
			#endif
			goto data_release;
		}
		strncat(file_name, rmi4_data->rmi4_mod_info.product_id_string, PRODUCT_ID_FW_LEN);
		...
		if (retval) {
			...
			#if defined (CONFIG_HUAWEI_DSM)
			g_ts_data.dsm_info.constraints_UPDATE_status = fwu_get_fw_data_fail;
			#endif
			goto data_release;
		}

		if(rmi4_data->synaptics_chip_data->fwv_g_update_check) {
			g_boot_fwv_greater_check = 1;
		}
		...
		if (rmi4_data->force_update || need_update) {
			...
			if (retval) {
				...
				#if defined (CONFIG_HUAWEI_DSM)
				g_ts_data.dsm_info.constraints_UPDATE_status = fwu_fw_update_fail;
				#endif
			} else {
				TS_LOG_INFO("successfully update fw\n");
			}
		}
		g_boot_fwv_greater_check = 0;
	} else {
...
data_release:
	...
	{
		/* Refresh configid */
		retval = synaptics_fw_configid(rmi4_data);
		if (retval) {
			TS_LOG_ERR("failed read config id !\n");
		} else {
			TS_LOG_INFO("successfully read config id !\n");
		}
...
static int synaptics_set_holster_switch(u8 holster_switch)
{
	...
	TS_LOG_INFO("holster_enable_addr = 0x%4x, holster switch is %d \n", holster_enable_addr, holster_switch);
