...
#endif 
#ifdef DHD_REOPEN_HW
static void hw_dhd_hang_process(struct work_struct *work);
#endif
...
#ifdef DHD_REOPEN_HW
	struct workqueue_struct* dhd_hang_wq;
...

{
	...
	SMP_RD_BARRIER_DEPENDS();
	DHD_ERROR(("### get sema and reload firmware\n"));
	dhd_reopen_dev(dev);
	//dhd_stop(dev);
	//dhd_open(dev);

	complete_and_exit(&tsk->completed, 0);
	DHD_ERROR(("### get sema and reload firmware exit\n"));
}
#endif
...
#ifdef DHD_DEBUG
void print_skt_buf(dhd_pub_t *pub, void *skb)
{
	struct sk_buff *skb_print = (struct sk_buff *)(skb);

	if ( skb_print->protocol != htons(ETHER_TYPE_802_1X) ) {
		return;
	}

	// proto: ETH_P_IP / ETH_P_IPV6
	DHD_ERROR(("skb-protocol=%x, proto=%x, skt_buf_len=%x\n", (skb_print->protocol),
		htons(eth_hdr(skb_print)->h_proto), skb_print->len));

	if ( (DHD_BYTES_ON() && DHD_DATA_ON()) ) {
		prhex("Rx 8021x Data:", PKTDATA(pub->osh, skb), PKTLEN(pub->osh, skb));
	}
}
#endif
...
#ifdef DHD_DEBUG
				print_skt_buf(pub, skb);
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 0)
				ret_val = netif_rx_ni(skb);
#else
...
#endif
				if ( ret_val != NET_RX_SUCCESS ) {
					DHD_ERROR(("netif_rx_ni return= %d, protocol=%x\n",
						ret_val, ((struct sk_buff *)skb)->protocol));
				}
				skb = skbnext;
			}
#if defined(WAIT_DEQUEUE)
...
exit:
	...
	if (ret){
		...
		hw_wifi_dsm_client_notify(DSM_WIFI_OPEN_ERROR,
		             "%s: open wifi fail, goto dhd_stop, ret:%d\n", __FUNCTION__, ret);
#endif
	...
	dhd_open_stop_if_mutex_unlock(dhd);
#endif
	HW_PRINT((WIFI_TAG"Exit: %s, ret:%d\n", __FUNCTION__, ret));
	return ret;
}
...
#endif
#ifdef DHD_REOPEN_HW
	dhd->dhd_hang_wq = create_singlethread_workqueue("dhd_hang_wq");
	INIT_WORK(&dhd->work_hang, hw_dhd_hang_process);
#endif
...
#endif /* #if defined(SOFTAP) && defined(HW_SOFTAP_MANAGEMENT)*/
...
dhd_module_init(void)
{
	...
	if (firmware_path[0] != '\0') {
		...
		DHD_INFO(("%s firmware_path=%s\n", __FUNCTION__, firmware_path));
	}

	if (nvram_path[0] != '\0') {
		strncpy(nv_bak_path, nvram_path, MOD_PARAM_PATHLEN);
		nv_bak_path[MOD_PARAM_PATHLEN-1] = '\0';
		DHD_INFO(("%s nvram_path=%s\n", __FUNCTION__, nvram_path));
	}
	...
	do {
		...
		if (!err) {
			...
			DHD_ERROR(("%s: retry to load wifi driver, firmware_path=%s, nvram_path=%s\n",
	...
	if(0 != err)
    {
		...
		hw_wifi_dsm_client_notify(DSM_WIFI_MODULE_INIT_ERROR,
		          "init wifi driver failed in dhd_module_init\n");
#endif
...
#endif
#else
		PROC_START(dhd_reopen_thread, dev, &dhd->thr_reopen_ctl, 0, "dhd_reload");
#endif
	}
}
...
static void hw_dhd_hang_process(struct work_struct *work)
{
	...
	if (dev) {
		DHD_ERROR(("dhd hang was occured, begin to dhd recovery\n"));
		cfg80211_drv_recovery(dev, GFP_KERNEL);
	}
}
#endif
...
#ifdef DHD_REOPEN_HW
			queue_work(dhdp->info->dhd_hang_wq, &dhdp->info->work_hang);
#else
...
int dhd_reopen_dev(struct net_device *dev)
{
    ...
    //avoid recovery due to CRC error  when driver suspend
    if (dhd_mmc_suspend) {
        DHD_ERROR(("in suspend, cancel recovery\n"));
        dhd->pub.hang_was_sent = 0;
        return 0;
    }
	if(dhd->pub.op_mode == DHD_FLAG_HOSTAP_MODE) {
		...
		DHD_ERROR(("net_os_send_hang_message enter2\n"));
		dhd_stop(dev);
		dhd_open(dev);
		wl_cfg80211_hang(dev, 0);
	}
	return 0;
}
#endif
