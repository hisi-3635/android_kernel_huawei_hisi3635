	...
	struct vif_params *params)
{
	...
	HW_PRINT((WIFI_TAG"Enter: %s, if name: %s, type: %d\n", __FUNCTION__, name, type));
	...
	struct cfg80211_ibss_params *params)
{
	...
	if (IBSS_INITIAL_SCAN_ALLOWED == FALSE) {
		...
		if (unlikely(err)) {
			...
			hw_wifi_dsm_client_notify(DSM_WIFI_WLC_SET_SCANSUPPRESS_ERROR,
			          "Scan Suppress Setting Failed (%d)\n", err);
#endif
			return err;
		}
	}
...
#ifdef HW_WIFI_DMD_LOG
		hw_wifi_dsm_client_notify(DSM_WIFI_SET_CIPHER_ERROR,
		          "Invalid ciper\n");
#endif
		return err;
	}
...
#ifdef HW_WIFI_DMD_LOG
		hw_wifi_dsm_client_notify(DSM_WIFI_SET_KEY_MGMT_ERROR,
		          "Invalid key mgmt\n");
#endif
		return err;
	}
...
#ifdef HW_WIFI_DMD_LOG
		hw_wifi_dsm_client_notify(DSM_WIFI_SET_SHAREDKEY_ERROR,
		          "Invalid shared key\n");
#endif
		return err;
	}
...
set_ssid:
	...
	if (err) {
		WL_ERR(("error (%d)\n", err));
#ifdef HW_WIFI_DMD_LOG
		hw_wifi_dsm_client_notify(DSM_WIFI_WLC_SET_SSID_ERROR,
		          "error (%d)\n", err);
#endif
		wl_clr_drv_status(cfg, CONNECTING, dev);
	}
...
set_channel:
        ...
        switch (channel) {
                ...
                case 9:
                case 13:
#ifdef HW_SOFTAP_SPECIAL_CHN_HT20
	...
	u32 dev_role, s32 bssidx)
{
	...
	u16 center_freq = 0;
	int center_chan = 0;
	int disable_nmode_channel1 = 9;
	int disable_nmode_channel2 = 13;
...
#ifdef HW_SOFTAP_DISABLE_NMODE
		center_freq = (dev->ieee80211_ptr == NULL)? 2412: (dev->ieee80211_ptr->preset_chandef.chan->center_freq);
		center_chan = ieee80211_frequency_to_channel(center_freq);
		//if channel equals 9 or 11, disable 11N
		if ((center_chan == disable_nmode_channel1) || (center_chan == disable_nmode_channel2)) {
			WL_ERR(("disable nmode for %d channel in AP mode\n", center_chan));
			if ((err = wldev_iovar_setint(dev, "nmode", 0)) != BCME_OK) {
				WL_ERR(("fail to disable nmode, err =%d\n", err));
				goto exit;
			}
		}
#endif
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (unlikely(err)) {
		WL_ERR(("scan busy (%d)\n", err));
#ifdef HW_WIFI_DMD_LOG
		hw_wifi_dsm_client_notify(DSM_WIFI_WLC_GET_CHANNEL_ERROR,
		          "scan busy (%d)\n", err);
#endif
		goto scan_done_out;
	}
...
#ifdef HW_WIFI_DMD_LOG
		hw_wifi_dsm_client_notify(DSM_WIFI_WLC_SCAN_RESULTS_ERROR,
	...
	void *ndev)
{
	...
	switch (state) {
		case NETDEV_DOWN:
		{
			WL_ERR(("WIFI_DBG: receive NETDEV_DOWN do nothing.\n"));
...
wl_cfg80211_event(struct net_device *ndev, const wl_event_msg_t * e, void *data)
{
	...
	if (cfg == NULL)
	{
		WL_ERR(("cfg is NULL, ignore event %d\n", event_type));
		return;
	}
...
static s32 __wl_cfg80211_up(struct bcm_cfg80211 *cfg)
{
	...
	WL_DBG(("In\n"));
	HW_PRINT((WIFI_TAG"Enter %s, nl80211_iftype:%d\n",__FUNCTION__, wdev->iftype));
