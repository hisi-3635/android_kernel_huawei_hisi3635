                                                    ...
                                                    unsigned int msg_priority )
{
    ...
    case HI_CHN_MSG_PCM_TRIGGER :
        pmsg = (struct hifi_chn_pcm_trigger*)pmsg_body;
		loge("hi6210_mailbox_send_data data_addr = 0x%x, mst_type = %d \r\n", pmsg->data_addr, pmsg->msg_type);
		break;
	case HI_CHN_MSG_PCM_SET_BUF:
		pbuf = (struct hifi_channel_set_buffer*)pmsg_body;
		loge("hi6210_mailbox_send_data data_addr = 0x%x, mst_type = %d \r\n", pbuf->data_addr, pbuf->msg_type);
		break;
    default:
		loge("!!!!!!!!!!!!!!!!!!!!==========================================22222222222222 \r\n");
		break;
...
STATIC int hi6210_pcm_hifi_open(struct snd_pcm_substream *substream)
{
    ...
    prtd = kzalloc(sizeof(struct hi6210_runtime_data), GFP_KERNEL);
    if (NULL == prtd)
    {
...
static int hi6210_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
    ...
    else
    {
        logi("pcm-device = %d\n", pcm->device);
        ret = snd_pcm_lib_preallocate_pages_for_all(pcm, SNDRV_DMA_TYPE_DEV,
                pcm->card->dev, HI6210_BUFFER_SIZE_MM, HI6210_BUFFER_SIZE_MM);
        if (ret)
        {
            loge("snd_pcm_lib_preallocate_pages_for_all error : %d\n", ret);
            return ret;
